...
| Tailnet invoke latency (LAN) | < 20ms / < 100ms | Host-backed invoke stub benchmark with injected direct-path RTT plus `fcp-tailnet-invoke-evidence`; real records require a configured tailnet-class `/rpc/invoke` endpoint and live Tailscale direct-route telemetry from `tailscale status --json` or LocalAPI, while automatic two-node mesh harnessing is still pending |
| Tailnet invoke latency (DERP) | < 150ms / < 500ms | Host-backed invoke stub benchmark with injected DERP RTT plus `fcp-tailnet-invoke-evidence`; real records require a configured tailnet-class `/rpc/invoke` endpoint and live Tailscale DERP/fallback telemetry from `tailscale status --json` or LocalAPI, while automatic two-node mesh harnessing is still pending |
