//! FCP `Algolia` Connector - Main entrypoint
//!
//! An `Algolia` search connector implementing the Flywheel Connector Protocol.
//! Provides access to search, indices, and records.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Algolia Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut AlgoliaConnector, message: &str) -> serde_json::Value {
