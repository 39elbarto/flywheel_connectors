...
> **Primary upstream**: https://platform.claude.com/docs/en/api/overview and https://code.claude.com/docs/en/authentication
...
- Bearer-token mode sends `Authorization: Bearer ...`; direct Anthropic API use is reserved for short-lived WIF access tokens, while gateway bearer tokens must terminate behind the approved provider boundary.
- Values shaped like Anthropic API keys are rejected from `auth_token`/`bearer_token`; use `api_key` so the connector sends `x-api-key`.
...
- `setup_token` is a compatibility alias for the long-lived `CLAUDE_CODE_OAUTH_TOKEN` produced by `claude setup-token`; the connector does not run `claude`, read Claude Code auth env vars, persist the token, or refresh it locally.
...
- Authentication mechanisms: Anthropic API key, WIF/gateway bearer token, Claude Code OAuth token, setup-token OAuth alias, or host-injected credential ID.
- Direct Anthropic API calls use `api_key`, `credential_id`, or a short-lived WIF bearer token; Claude Code OAuth and setup-token credentials must be routed through a host-managed Claude CLI/provider boundary or a localhost verification fixture.
- Claude Code auth precedence and env var handling remain outside this connector. Hosts may read `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, or `CLAUDE_CODE_OAUTH_TOKEN`; this connector only consumes explicit FCP configure parameters and intentionally avoids importing CLI profiles or provider-routing environment variables.
- `claude setup-token` produces a one-year OAuth token for Claude Code CLI scripts. Rotation is operator/host work: rerun `claude setup-token` or update the host credential provider, then reconfigure this connector with a redacted/host-mediated value.
- Home zone: `z:work`.
...
- Current upstream auth contracts used for this boundary:
  - Claude API authentication: `x-api-key` for API keys or `Authorization: Bearer <token>` for WIF access tokens.
  - Claude Code authentication: `ANTHROPIC_AUTH_TOKEN` is for bearer-token gateways/proxies, `ANTHROPIC_API_KEY` is sent as `X-Api-Key`, and `CLAUDE_CODE_OAUTH_TOKEN` is generated by `claude setup-token` for CLI automation.
...
- If `auth_token` or `bearer_token` is rejected because it looks like an Anthropic API key, move the value to `api_key`.
- If a Claude Code OAuth/setup-token credential expires, rerun `claude setup-token` or rotate the host-owned credential provider and reconfigure; `anthropic.auth.refresh_oauth` reports status only and never mints or persists tokens.
