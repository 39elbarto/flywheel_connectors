...
async fn anthropic_provider_contract_is_advertised() {
    let redaction_marker = "anthropic-provider-contract-marker";
        ...
        &ProviderContract::new("anthropic", "Anthropic")
            ...
            .with_operation(
                ProviderOperationContract::from_input_schema(
                    "anthropic.message",
                    "claude",
                    input_schema(&introspection, "anthropic.message"),
                )
            ...
            .with_operation(
                ProviderOperationContract::from_input_schema(
                    "anthropic.chat",
                    "claude",
                    input_schema(&introspection, "anthropic.chat"),
                )
                .require_default_model(),
            )
            .with_operation(
                ProviderOperationContract::from_input_schema(
                    ...
                    "claude",
                    input_schema(&introspection, "anthropic.message.stream"),
                )
                .require_default_model(),
            )
