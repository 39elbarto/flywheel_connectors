...
const CONNECTOR_ID: &str = "apple-notes";
const PROVIDER: &str = "macOS Apple Notes";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "APPLE_NOTES_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "APPLE_NOTES_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "APPLE_NOTES_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str = "cargo test -p fcp-apple-notes --test live_verification -- --nocapture";

#[test]
fn apple_notes_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the macOS Notes lab host/account.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe Apple Notes device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the lab host and verified Notes access.",
        )
        .with_account_setup(
            "Run on a dedicated macOS lab host with Apple Notes enabled for a synthetic test account.",
        )
        .with_budget(0.01)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "apple_notes_device_lab_verification",
    ...
    println!("APPLE_NOTES_DEVICE_LAB_JSONL {serialized}");
}
