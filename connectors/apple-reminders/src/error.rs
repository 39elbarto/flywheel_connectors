...
mod tests {
    ...
    fn unsupported_maps_to_unavailable() {
        ...
        assert!(matches!(err.to_fcp_error(), FcpError::ConnectorUnavailable { .. }));
    }
    ...
    fn error_display_config() {
        assert_eq!(AppleRemindersError::Config("m".into()).to_string(), "configuration error: m");
    }

    #[test]
    fn error_display_unsupported() {
        assert_eq!(AppleRemindersError::UnsupportedPlatform("L".into()).to_string(), "unsupported platform: L");
    }

    #[test]
    fn error_display_process() {
        assert_eq!(AppleRemindersError::Process("p".into()).to_string(), "process error: p");
    }

    #[test]
    fn error_display_parse() {
        assert_eq!(AppleRemindersError::Parse("q".into()).to_string(), "parse error: q");
    }

    #[test]
    fn connector_error_mapping_from_timeout() {
        use fcp_sdk::migration::ConnectorErrorMapping;
        let err = AppleRemindersError::from_async_error(fcp_async_core::AsyncError::Timeout { timeout_ms: 5000 });
