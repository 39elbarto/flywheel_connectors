//! Local loopback acceptance coverage for the `Asana` connector.
...
const OP_WORKSPACES_LIST: &str = "asana.workspaces.list";
const ACCEPTANCE_SUITE_CLASS: &str = "local_non_mock";
const ACCESS_TOKEN: &str = "1/local_acceptance_pat";
...
async fn loopback_workspaces_list_uses_asana_request_boundary() {
    ...
    connector
        .handle_handshake(json!({ "session_id": "asana-local-non-mock" }))
        .await
        .expect("handshake connector");

    let result = connector
        .handle_invoke(json!({
            "operation_id": OP_WORKSPACES_LIST,
            "input": {}
        }))
        .await
        .expect("list workspaces through loopback fixture");
    let observation = fixture.join();

    assert_eq!(ACCEPTANCE_SUITE_CLASS, "local_non_mock");
    assert_eq!(observation.request_line, "GET /workspaces HTTP/1.1");
    assert!(
        observation
            .headers
            .iter()
    ...
    assert!(
        observation.headers.iter().any(|line| {
            line.eq_ignore_ascii_case("user-agent: fcp-asana/0.1.0 (FCP connector)")
        })
    );
    assert_eq!(result["data"][0]["gid"], "workspace_123");
    assert_eq!(result["data"][0]["name"], "Acceptance Engineering");
    ...
    let artifact = json!({
        ...
        "command": "cargo test -p fcp-asana --test local_non_mock -- --nocapture",
        ...
        "auth_gate": {
            "mode": "synthetic_personal_access_token",
