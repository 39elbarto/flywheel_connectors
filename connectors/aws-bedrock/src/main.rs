//! FCP AWS Bedrock connector entrypoint.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP AWS Bedrock Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut BedrockConnector, message: &str) -> serde_json::Value {
