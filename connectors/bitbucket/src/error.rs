//! `Bitbucket`-specific error types.
...
/// Result alias for `Bitbucket` operations.
...
pub enum BitbucketError {
    ...
    #[error("Bitbucket API error ({status_code}): {message}")]
...
impl ConnectorErrorMapping for BitbucketError {
...
mod tests {
    ...
    fn unauthorized_not_retryable() {
        assert!(!BitbucketError::Unauthorized.is_retryable());
    }

    #[test]
    fn forbidden_not_retryable() {
        assert!(!BitbucketError::Forbidden.is_retryable());
    }

    #[test]
    fn not_found_not_retryable() {
        assert!(
            !BitbucketError::NotFound {
                resource: "repo".into()
            }
            .is_retryable()
        );
    ...
    fn retry_after_none_for_unauthorized() {
        assert_eq!(BitbucketError::Unauthorized.retry_after(), None);
    }

    #[test]
    fn retry_after_none_for_forbidden() {
        assert_eq!(BitbucketError::Forbidden.retry_after(), None);
    }
    ...
    fn not_found_to_fcp_error() {
        ...
        .to_fcp_error()
        {
            FcpError::External {
                ...
                assert!(message.contains("repo_abc"));
                assert!(!retryable);
            }
            other => panic!("expected External, got {other:?}"),
        }
    ...
    fn json_error_to_fcp_internal() {
        ...
        match BitbucketError::Json(bad.unwrap_err()).to_fcp_error() {
    ...
    fn error_display_unauthorized() {
        assert_eq!(
            BitbucketError::Unauthorized.to_string(),
    ...
    fn error_display_forbidden() {
        assert_eq!(
            BitbucketError::Forbidden.to_string(),
    ...
    fn rate_limited_retry_after_zero() {
        let err = BitbucketError::RateLimited { retry_after_ms: 0 };
