//! FCP `Bitbucket` Connector - Main entrypoint
//!
//! A `Bitbucket` Cloud connector implementing the Flywheel Connector Protocol.
//! Provides access to repositories, pull requests, branches, commits, and pipelines.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Bitbucket Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut BitbucketConnector, message: &str) -> serde_json::Value {
