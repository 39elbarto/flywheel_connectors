//! FCP `Bitwarden` Connector - Main entrypoint
//!
//! A `Bitwarden` password and secret management connector implementing the
//! Flywheel Connector Protocol. Provides access to vault items, collections,
//! and folders.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Bitwarden Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut BitwardenConnector, message: &str) -> serde_json::Value {
