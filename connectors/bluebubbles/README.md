# BlueBubbles Connector V3 Contract

> **Status**: runtime contract documented; wrapper/shared-runtime/manifest drift documented
...
This document fixes the operator-facing contract for `fcp.bluebubbles`. The connector exposes the BlueBubbles bridge surface currently shipped in this checkout: message send, local media send, chat and message reads, target resolution, selected Private API actions, attachment download, webhook registration, host-forwarded webhook ingress, normalized event streaming, local health, and server-info probing.

The connector is intentionally a bounded BlueBubbles bridge package over the shared `fcp_imessage::BlueBubblesConnector` runtime. It is not a standalone iMessage protocol implementation, hosted BlueBubbles server, Apple ID login flow, BlueBubbles server installer, remote proxy manager, group administration client, phone-number registration tool, durable message archive, or general macOS automation surface.
...
- Package, library, and binary name are `fcp-bluebubbles`.
- Manifest ID is `fcp.bluebubbles`.
- `fcp_bluebubbles::new_connector()` wraps the shared `BlueBubblesConnector` with connector ID `fcp.bluebubbles` and the BlueBubbles manifest.
- The shared implementation lives under `connectors/imessage`.
- Runtime operation IDs and capability IDs intentionally remain `imessage.*`.
...
- The BlueBubbles package is branded as `fcp.bluebubbles`, but its runtime operation IDs and capability IDs are still `imessage.*`.
- The BlueBubbles wrapper and iMessage package share one implementation. Divergent behavior between them should be treated as a bug unless it is purely connector ID or manifest text.
...
A follow-up parity bead should decide whether the BlueBubbles wrapper should expose BlueBubbles-prefixed operation IDs or keep the current shared `imessage.*` API, enforce manifest host/port policy in runtime configuration, bind capability tokens to conversation and message resources, make simulation validate schemas and local policy, clarify `attachment_dir`, and decide whether any additional upstream Private API or webhook event kinds belong in scope.

## First-Slice Scope

The current BlueBubbles README slice documents the existing runtime surface:

- Dedicated `fcp.bluebubbles` wrapper around the shared `BlueBubblesConnector`
...
- Runtime/manifest/upstream drift around operation IDs, localhost enforcement, broader upstream API coverage, simulation, shutdown, and missing resource binding
...
- Live BlueBubbles payloads can contain private messages, phone numbers, emails, group names, attachment metadata and bytes, read status, tapbacks, local contact names, and server details. Treat live input and output as private-zone sensitive unless the host supplies a stricter zone policy.
...
- `connectors/bluebubbles/src/lib.rs` defines `CONNECTOR_ID`, wraps the shared runtime, and proves the dedicated connector ID.
- `connectors/bluebubbles/src/main.rs` defines the stdio JSON-RPC entrypoint.
- `connectors/bluebubbles/manifest.toml` defines the BlueBubbles-branded manifest, private-zone policy, network constraints, operations, and event topics.
- `connectors/bluebubbles/tests/conformance_contract.rs` checks manifest/runtime catalog alignment.
- `connectors/bluebubbles/tests/integration.rs` covers wrapper lifecycle, bound tokens, loopback provider calls, webhook ingress, redaction, policy rejection, error mapping, and media-root denial.
...
```bash
git diff --check -- connectors/bluebubbles/README.md
LC_ALL=C rg -n '[^ -~]' connectors/bluebubbles/README.md
rg -n '\bmaster\b' connectors/bluebubbles/README.md
ubs connectors/bluebubbles/README.md
```
