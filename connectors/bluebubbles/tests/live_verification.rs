...
const CONNECTOR_ID: &str = "bluebubbles";
const PROVIDER: &str = "BlueBubbles bridge";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "BLUEBUBBLES_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "BLUEBUBBLES_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "BLUEBUBBLES_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str = "cargo test -p fcp-bluebubbles --test live_verification -- --nocapture";

#[test]
fn bluebubbles_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the BlueBubbles bridge lab instance.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe BlueBubbles device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the BlueBubbles lab bridge.",
        )
        .with_account_setup(
            "Use a dedicated BlueBubbles server paired with a synthetic Apple account and no personal chats.",
        )
        .with_budget(0.01)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "bluebubbles_device_lab_verification",
    ...
    println!("BLUEBUBBLES_DEVICE_LAB_JSONL {serialized}");
}
