//! FCP `Box` Connector - Main entrypoint
//!
//! A `Box` cloud storage connector implementing the Flywheel Connector Protocol.
//! Provides access to files, folders, and sharing collaborations.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Box Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut BoxConnector, message: &str) -> serde_json::Value {
