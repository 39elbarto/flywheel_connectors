//! Local loopback acceptance coverage for the `Calendly` connector.
...
const BEAD_ID: &str = "flywheel_connectors-bky21.3.6.58";
const CONNECTOR_ID: &str = "fcp.calendly";
const ACCESS_TOKEN: &str = "local-calendly-access-token";
const CANCEL_REASON_SENTINEL: &str = "CALENDLY_CANCEL_REASON_SHOULD_NOT_APPEAR_IN_EVIDENCE";
...
const USER_RESPONSE_BODY: &str = r#"{
  "resource": {
    "uri": "https://api.calendly.com/users/user-local",
    "name": "Local User",
    "email": "local@example.com",
    "scheduling_url": "https://calendly.com/local-user",
    "timezone": "UTC",
    "current_organization": "https://api.calendly.com/organizations/org-local"
  }
}"#;

const SCHEDULING_LINK_RESPONSE_BODY: &str = r#"{
  "resource": {
    "booking_url": "https://calendly.com/s/local-link",
    "owner": "https://api.calendly.com/event_types/event-type-local",
...
impl LoopbackFixture {
    fn start(responses: Vec<ResponseSpec>) -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind Calendly listener");
...
async fn local_non_mock_identity_and_mutations_use_loopback_boundary() {
    let fixture = LoopbackFixture::start(vec![
        ResponseSpec::json(200, USER_RESPONSE_BODY),
        ResponseSpec::json(201, SCHEDULING_LINK_RESPONSE_BODY),
        ResponseSpec::json(202, r#"{"status":"accepted"}"#),
    ]);
    ...
    let user = invoke(&connector, &signing_key, OP_USER_GET, json!({}))
        .await
        .expect("user.get should succeed");
    let link = invoke(
        &connector,
        &signing_key,
        OP_SCHEDULING_LINKS_CREATE,
        json!({
            "owner_uri": "https://api.calendly.com/event_types/event-type-local",
            "owner_type": "EventType",
            "max_event_count": 1
        }),
    )
    .await
    .expect("scheduling link create should succeed");
    let cancelled = invoke(
        &connector,
        &signing_key,
        OP_EVENTS_CANCEL,
        json!({
            "event_uuid": "event-local",
            "reason": CANCEL_REASON_SENTINEL
        }),
    )
    .await
    .expect("events.cancel should succeed");
    ...
    assert_eq!(
        ...
        "POST /scheduling_links HTTP/1.1"
    );
    assert_eq!(
        observations[2].request_line,
        "POST /scheduled_events/event-local/cancellation HTTP/1.1"
    );
    ...
    let link_body: Value = serde_json::from_str(&observations[1].body).expect("link body is JSON");
    assert_eq!(
        link_body["owner"],
        "https://api.calendly.com/event_types/event-type-local"
    );
    assert_eq!(link_body["owner_type"], "EventType");
    assert_eq!(link_body["max_event_count"], 1);
    let cancel_body: Value =
        serde_json::from_str(&observations[2].body).expect("cancel body is JSON");
    assert_eq!(cancel_body["reason"], CANCEL_REASON_SENTINEL);

    assert_eq!(
        user["resource"]["uri"],
        "https://api.calendly.com/users/user-local"
    );
    assert_eq!(
        link["resource"]["booking_url"],
        "https://calendly.com/s/local-link"
    );
    assert_eq!(cancelled["status"], "cancelled");

    let logs = vec![
        evidence_log(OP_USER_GET, Some(&observations[0]), "passed"),
        evidence_log(OP_SCHEDULING_LINKS_CREATE, Some(&observations[1]), "passed"),
        evidence_log(OP_EVENTS_CANCEL, Some(&observations[2]), "passed"),
    ];
...
fn evidence_schema_carries_connector_and_tracker_identity() {
    let log = evidence_log(OP_USER_GET, None, "passed");
    ...
    assert_eq!(OperationId::from_static(OP_USER_GET).as_str(), OP_USER_GET);
    assert_eq!(
        RequestId::new("calendly-local").to_string(),
        "calendly-local"
    );
...
async fn configured_connector(
    ...
    connector
        ...
        .await
        .expect("configure Calendly connector");
    connector
        ...
        .await
        .expect("handshake Calendly connector");
    connector
}
...
fn capability_for_operation(operation: &'static str) -> &'static str {
    match operation {
        ...
        OP_HEALTH | OP_USER_GET => CAP_USER_READ,
...
async fn invoke(
    ...
    let response = connector
        .invoke(InvokeRequest {
            ...
            id: RequestId::new(format!("calendly-local-{operation}")),
...
fn evidence_log(
    ...
    EvidenceLog {
        ...
        redaction: "bearer_user_uri_event_uri_invitee_and_provider_body_not_logged",
    }
}
...
fn route_label(request: &RequestObservation) -> &'static str {
    match (request.method(), request.target()) {
        ...
        ("POST", "/scheduled_events/event-local/cancellation") => "events.cancel",
        _ => "unrecognized",
    }
}
