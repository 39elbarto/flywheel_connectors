//! Local loopback acceptance coverage for the `ClickUp` connector.
...
const API_TOKEN: &str = "pk_local_acceptance_token";
...
async fn loopback_spaces_list_uses_clickup_request_boundary() {
    ...
    connector
        .handle_handshake(json!({ "session_id": "clickup-local-non-mock" }))
        .await
        .expect("handshake connector");

    let result = connector
        .handle_invoke(json!({
            "operation_id": OP_SPACES_LIST,
            "input": {
                "team_id": "team_123"
            }
        }))
        .await
        .expect("list spaces through loopback fixture");
    let observation = fixture.join();

    assert_eq!(ACCEPTANCE_SUITE_CLASS, "local_non_mock");
    assert_eq!(
        observation.request_line,
        "GET /team/team_123/space HTTP/1.1"
    );
    assert!(
        observation
            .headers
    ...
    assert!(
        observation
            .headers
            .iter()
            .any(|line| line.eq_ignore_ascii_case(&format!("authorization: {API_TOKEN}")))
    );
    assert!(observation.headers.iter().any(|line| {
        line.eq_ignore_ascii_case("user-agent: fcp-clickup/0.1.0 (FCP connector)")
    }));
    assert_eq!(result["spaces"][0]["id"], "space_123");
    assert_eq!(result["spaces"][0]["name"], "Acceptance Engineering");
    ...
    let artifact = json!({
        ...
        "command": "cargo test -p fcp-clickup --test local_non_mock -- --nocapture",
