...
fn run_fcp_loop() -> Result<()> {
    ...
    let mut connector = ComfyUiConnector::new();
...
async fn handle_message(connector: &mut ComfyUiConnector, message: &str) -> serde_json::Value {
...
fn with_optional_id(
    ...
    if let (Some(object), Some(id)) = (response.as_object_mut(), id) {
