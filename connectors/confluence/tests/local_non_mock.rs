//! Local loopback acceptance coverage for the `Confluence` connector.
...
use fcp_confluence::connector::{ConfluenceConnector, operations_info};
...
const BEAD_ID: &str = "flywheel_connectors-bky21.3.6.60";
const CONNECTOR_ID: &str = "fcp.confluence";
const EMAIL: &str = "local@example.com";
const API_TOKEN: &str = "local-confluence-api-token";
const BODY_SENTINEL: &str = "CONFLUENCE_BODY_SHOULD_NOT_APPEAR_IN_EVIDENCE";
...
const SPACES_RESPONSE_BODY: &str = r#"{
  "results": [
    {
      "id": "space-1",
      "key": "ENG",
  ...
  "size": 1,
  "_links": {
    "next": "/rest/api/space?start=2",
    "base": "/wiki"
  }
}"#;

const PAGES_RESPONSE_BODY: &str = r#"{
  "results": [
    {
      "id": "page-1",
      "title": "Runbook",
...
const CREATED_PAGE_RESPONSE_BODY: &str = r#"{
  "id": "page-created",
  ...
  "_links": {
    "webui": "/spaces/ENG/pages/page-created"
  }
}"#;
...
impl LoopbackFixture {
    fn start(responses: Vec<ResponseSpec>) -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind Confluence listener");
...
async fn local_non_mock_spaces_pages_create_and_search_use_loopback_boundary() {
    let fixture = LoopbackFixture::start(vec![
        ResponseSpec::json(200, SPACES_RESPONSE_BODY),
        ResponseSpec::json(200, PAGES_RESPONSE_BODY),
        ResponseSpec::json(201, CREATED_PAGE_RESPONSE_BODY),
    ...
    let spaces = invoke(
        ...
        &signing_key,
        OP_SPACES_LIST,
        json!({"start": 0, "limit": 2}),
    )
    .await
    .expect("spaces.list should succeed");
    let pages = invoke(
        &connector,
        &signing_key,
        OP_PAGES_LIST,
        json!({"space_key": "ENG", "start": 2, "limit": 3}),
    )
    .await
    .expect("pages.list should succeed");
    let created = invoke(
        &connector,
        &signing_key,
        OP_PAGES_CREATE,
    ...
    .await
    .expect("pages.create should succeed");
    let search = invoke(
        &connector,
        &signing_key,
        OP_SEARCH,
        json!({
            "cql": "space = ENG and text ~ \"runbook\"",
            "start": 5,
            "limit": 10
        }),
    )
    ...
    assert_eq!(
        ...
        "GET /rest/api/space?start=0&limit=2 HTTP/1.1"
    );
    assert_eq!(observations[1].method(), "GET");
    assert!(
        observations[1]
            .target()
            .starts_with("/rest/api/space/ENG/content/page?")
    );
    assert!(observations[1].target().contains("start=2"));
    assert!(observations[1].target().contains("limit=3"));
    assert!(observations[1].target().contains("expand=version%2Cspace"));
    assert_eq!(
        observations[2].request_line,
        "POST /rest/api/content HTTP/1.1"
    );
    assert_eq!(observations[3].method(), "GET");
    assert!(observations[3].target().starts_with("/rest/api/search?"));
    assert!(observations[3].target().contains("cql="));
    assert!(observations[3].target().contains("start=5"));
    assert!(observations[3].target().contains("limit=10"));

    let expected_auth = expected_auth_header();
    ...
    assert_eq!(create_body["type"], "page");
    assert_eq!(create_body["title"], "Created by FCP");
    assert_eq!(create_body["space"]["key"], "ENG");
    assert_eq!(create_body["body"]["storage"]["value"], BODY_SENTINEL);
    assert_eq!(create_body["body"]["storage"]["representation"], "storage");
    assert_eq!(create_body["ancestors"][0]["id"], "page-parent");

    assert_eq!(spaces["results"][0]["key"], "ENG");
    assert_eq!(pages["results"][0]["id"], "page-1");
    assert_eq!(created["id"], "page-created");
    assert_eq!(search["results"][0]["title"], "Runbook");

    let logs = vec![
        evidence_log(OP_SPACES_LIST, Some(&observations[0]), "passed"),
        evidence_log(OP_PAGES_LIST, Some(&observations[1]), "passed"),
        evidence_log(OP_PAGES_CREATE, Some(&observations[2]), "passed"),
        evidence_log(OP_SEARCH, Some(&observations[3]), "passed"),
    ];
    assert_redacted(&logs);
}

#[fcp_async_core::runtime::test]
async fn local_non_mock_health_rate_limit_maps_retry_after_without_secret_logging() {
    ...
    assert_eq!(
        observations[0].request_line,
        "GET /rest/api/space?limit=1 HTTP/1.1"
    );
...
fn evidence_schema_carries_connector_and_tracker_identity() {
    let log = evidence_log(OP_SPACES_LIST, None, "passed");
    ...
    assert_eq!(
        OperationId::from_static(OP_SPACES_LIST).as_str(),
        OP_SPACES_LIST
    );
    assert_eq!(
        RequestId::new("confluence-local").to_string(),
        "confluence-local"
    );
    assert_eq!(ZoneId::work().as_str(), "z:work");
    assert_eq!(operations_info().len(), 9);
}
...
async fn configured_connector(
    ...
    connector
        ...
        .await
        .expect("configure Confluence connector");
    connector
        ...
        .await
        .expect("handshake Confluence connector");
    connector
}

fn capability_for_operation(operation: &'static str) -> &'static str {
    match operation {
        OP_HEALTH | OP_SPACES_LIST => CAP_SPACES_READ,
        OP_PAGES_LIST | OP_SEARCH => CAP_PAGES_READ,
        OP_PAGES_CREATE => CAP_PAGES_WRITE,
...
fn capability_for(
    ...
    let cose = CapabilityTokenBuilder::new()
        ...
        .zone_id("z:work")
        .principal("user:confluence-local")
...
async fn invoke(
    ...
    let response = connector
        .invoke(InvokeRequest {
            ...
            id: RequestId::new(format!("confluence-local-{operation}")),
...
fn evidence_log(
    ...
    EvidenceLog {
        ...
        redaction: "basic_auth_email_api_token_page_body_provider_body_not_logged",
    }
}

fn route_label(request: &RequestObservation) -> &'static str {
    match (request.method(), request.target()) {
        ("GET", target) if target.starts_with("/rest/api/space?limit=1") => "health",
        ("GET", target) if target.starts_with("/rest/api/space?") => "spaces.list",
        ("GET", target) if target.starts_with("/rest/api/space/ENG/content/page?") => "pages.list",
        ("POST", "/rest/api/content") => "pages.create",
        ("GET", target) if target.starts_with("/rest/api/search?") => "search",
        _ => "unrecognized",
    }
}
