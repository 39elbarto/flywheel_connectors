...
const OPERATION: &str = "deepgram.self_check_projects";
...
fn emit_live_jsonl(status: &str, reason: &str, connector_status: &str) {
    println!(
        "DEEPGRAM_LIVE_JSONL {}",
        json!({
            "event": "deepgram_live_read_smoke",
            "fixture_mode": "live",
            ...
            "resource_class": "authenticated_project_read_probe",
            "connector_status": connector_status,
            "call_ceiling": 1,
            "rate_limit_guidance": "Performs one authenticated GET /v1/projects probe through handle_self_check.",
...
async fn deepgram_live_read_project_probe_or_structured_skip_jsonl() {
    ...
    connector
        ...
        .await
        .expect("configure Deepgram API key");
    connector
        .handle_handshake(json!({ "session_id": "deepgram-live-read" }))
        .await
        .expect("handshake Deepgram live connector");
    ...
    match connector.handle_self_check().await {
        Ok(value) => {
            let status = value
                ...
                .unwrap_or("unknown");
            assert_eq!(status, "ok", "Deepgram live self-check should pass");
            emit_live_jsonl("passed", "", status);
        }
        Err(error) => {
            emit_live_jsonl("failed", &error.to_string(), "error");
            panic!("Deepgram live read smoke failed: {error}");
        }
    }
}
