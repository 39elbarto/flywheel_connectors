//! FCP `DocuSign` Connector - Main entrypoint
//!
//! A `DocuSign` eSignature connector implementing the Flywheel Connector Protocol.
//! Provides access to envelopes, templates, recipients, and document download.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP DocuSign Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut DocuSignConnector, message: &str) -> serde_json::Value {
