//! FCP `DuckDB` `MotherDuck` Connector - Main entrypoint
//!
//! A `DuckDB` `MotherDuck` Cloud API connector implementing the Flywheel Connector Protocol.
//! Provides access to databases, tables, queries, schemas, and shares.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP DuckDB MotherDuck Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut DuckDbConnector, message: &str) -> serde_json::Value {
