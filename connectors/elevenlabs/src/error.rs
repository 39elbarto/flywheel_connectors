...
pub enum ElevenLabsError {
    ...
    /// API error response
    #[error("ElevenLabs API error ({status_code}): {message}")]
...
impl ConnectorErrorMapping for ElevenLabsError {
...
mod tests {
    ...
    fn http_timeout_maps_to_upstream_timeout() {
        let err = ElevenLabsError::Api {
            status_code: 408,
            message: "deadline exceeded after 5000ms".into(),
        };
    ...
    fn rate_limited_maps_correctly() {
        let err = ElevenLabsError::RateLimited {
            retry_after_ms: 5000,
        };
    ...
    fn not_configured_maps_correctly() {
        let err = ElevenLabsError::NotConfigured;
    ...
    fn json_error_maps_to_internal() {
        ...
        let err = ElevenLabsError::Json(json_err);
    ...
    fn from_async_timeout() {
        let err = ElevenLabsError::from_async_error(AsyncError::Timeout { timeout_ms: 5000 });
        assert!(matches!(
            err,
            ElevenLabsError::Api {
                status_code: 408,
    ...
    fn display_formats() {
        ...
        assert_eq!(err.to_string(), "ElevenLabs API error (404): Not Found");
    }
