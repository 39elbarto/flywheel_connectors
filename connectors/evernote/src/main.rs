//! FCP `Evernote` Connector - Main entrypoint
//!
//! An `Evernote` notes connector implementing the Flywheel Connector Protocol.
//! Provides access to notebooks, notes, and tags.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Evernote Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut EvernoteConnector, message: &str) -> serde_json::Value {
