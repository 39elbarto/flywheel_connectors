...
const OP_SEARCH: &str = "exa.search";
const CAP_SEARCH: &str = "exa.search";

struct ExaSuiteAdapter {
    connector: ExaConnector,
    id: ConnectorId,
}

impl ExaSuiteAdapter {
    fn new() -> Self {
        Self {
            connector: ExaConnector::new(),
            id: ConnectorId::from_static("fcp.exa"),
        }
    }
}

fcp_core::impl_fcp_sealed!(ExaSuiteAdapter);

#[fcp_core::async_trait]
impl FcpConnector for ExaSuiteAdapter {
    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        self.connector
            .handle_handshake(json!({ "session_id": "exa-connector-suite" }))
            .await?;
        ...
        Ok(HandshakeResponse {
            ...
            manifest_hash: "sha256:exa-connector-suite".into(),
            nonce: req.nonce,
            event_caps: None,
            auth_caps: None,
    ...
    async fn health(&self) -> HealthSnapshot {
        match self.connector.handle_health().await {
            Ok(payload) => match payload.get("status").and_then(serde_json::Value::as_str) {
                ...
                Some(other) => HealthSnapshot::degraded(format!("exa_status:{other}")),
                None => HealthSnapshot::error("exa_status:missing"),
            },
    ...
    fn introspect(&self) -> Introspection {
        Introspection {
            operations: vec![OperationInfo {
                ...
                summary: "Run an Exa search".into(),
                description: None,
                input_schema: json!({
                    "type": "object",
                    "required": ["query"],
                ...
                ai_hints: AgentHint {
                    when_to_use: "Use for read-only web search over Exa.".into(),
...
fn search_invoke_with_input(id: &'static str, input: serde_json::Value) -> InvokeRequest {
    InvokeRequest {
        r#type: "invoke".into(),
        id: RequestId::new(id),
        connector_id: ConnectorId::from_static("fcp.exa"),
...
async fn connector_suite_search_happy_path_uses_mock_server() {
    ...
    Mock::given(method("POST"))
        .and(path("/search"))
        .and(header("x-api-key", "exa_test_key"))
        .and(header("x-exa-integration", "fcp"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "results": [
                {"id": "r1", "title": "FCP", "url": "https://example.test/fcp"}
            ]
        })))
        .expect(1)
        .mount(&server)
    ...
    let mut connector = ExaSuiteAdapter::new();
    let mut runner = E2eRunner::new("fcp-exa");
    let report = runner
        .run_connector_suite(
            &mut connector,
            suite(&server, "exa_search_connector_suite_happy_path", false),
        )
        .await
...
#[fcp_async_core::runtime::test]
async fn connector_suite_search_suffix_base_url_uses_single_search_path_and_clamps() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/search"))
        .and(header("x-api-key", "exa_test_key"))
...
async fn connector_suite_search_error_path_is_expected() {
    ...
    let report = runner
        .run_connector_suite(
            &mut connector,
            suite(&server, "exa_search_connector_suite_error_path", true),
        )
        .await
        .expect("connector suite run");
