...
pub enum FeishuError {
    ...
    /// Feishu API returned an error response.
    #[error("Feishu API error ({code}): {message}")]
    Api { code: i32, message: String },

    /// HTTP-level error from Feishu.
    #[error("Feishu HTTP error ({status}): {message}")]
    HttpStatus { status: u16, message: String },

    /// Rate limited by Feishu API.
...
impl FeishuError {
    ...
    pub fn to_fcp_error(&self) -> FcpError {
        match self {
            ...
            Self::HttpStatus { status, message } => FcpError::External {
                service: "feishu".into(),
...
impl ConnectorErrorMapping for FeishuError {
...
pub type FeishuResult<T> = Result<T, FeishuError>;
...
mod tests {
    ...
    fn unauthorized_not_retryable() {
        let err = FeishuError::Unauthorized("bad token".into());
        assert!(!err.is_retryable());
    }

    #[test]
    fn api_throttle_error_is_retryable() {
        let err = FeishuError::Api {
            code: 99991400,
            message: "request throttled".into(),
        };
        assert!(err.is_retryable());
    }

    #[test]
    fn api_other_error_not_retryable() {
        let err = FeishuError::Api {
            code: 99991663,
    ...
    #[test]
    fn http_status_400_not_retryable() {
        let err = FeishuError::HttpStatus {
            status: 400,
    ...
    fn invalid_input_maps_to_invalid_request() {
        let err = FeishuError::InvalidInput("bad param".into());
        let fcp_err = ConnectorErrorMapping::to_fcp_error(&err);
    ...
    fn config_error_not_retryable() {
        let err = FeishuError::Config("missing credentials".into());
        assert!(!err.is_retryable());
    }

    #[test]
    fn from_async_timeout() {
        let err = FeishuError::from_async_error(AsyncError::Timeout { timeout_ms: 5000 });
        assert!(matches!(err, FeishuError::Async(_)));
        assert!(err.is_retryable());
    }

    #[test]
    fn from_async_cancelled() {
        let err = FeishuError::from_async_error(AsyncError::Cancelled);
        assert!(matches!(err, FeishuError::Async(_)));
    }

    #[test]
    fn from_async_channel_closed() {
        let err = FeishuError::from_async_error(AsyncError::ChannelClosed);
        assert!(matches!(err, FeishuError::Async(_)));
    }
    ...
    fn error_display_format() {
        ...
        assert!(display.contains("Feishu API error"));
        assert!(display.contains("99991663"));
    }

    #[test]
    fn json_error_not_retryable() {
        let err = FeishuError::Json(serde_json::from_str::<String>("invalid").unwrap_err());
        assert!(!err.is_retryable());
    }
    ...
    #[test]
    fn http_status_maps_to_external() {
        let err = FeishuError::HttpStatus {
            status: 503,
        ...
        let fcp_err = ConnectorErrorMapping::to_fcp_error(&err);
        match fcp_err {
            FcpError::External { status_code, .. } => assert_eq!(status_code, Some(503)),
