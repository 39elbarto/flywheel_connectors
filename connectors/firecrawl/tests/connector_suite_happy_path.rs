...
impl FirecrawlSuiteAdapter {
    fn new() -> Self {
        Self {
            connector: FirecrawlConnector::new(),
            id: ConnectorId::from_static(CONNECTOR_ID),
        }
    }
}

fcp_core::impl_fcp_sealed!(FirecrawlSuiteAdapter);

#[fcp_core::async_trait]
impl FcpConnector for FirecrawlSuiteAdapter {
    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        self.connector.handle_handshake(json!({})).await?;
        Ok(HandshakeResponse {
            ...
            manifest_hash: "sha256:firecrawl-connector-suite".to_string(),
            nonce: req.nonce,
            event_caps: None,
            auth_caps: None,
    ...
    async fn health(&self) -> HealthSnapshot {
        match self.connector.handle_health().await {
            Ok(payload) => match payload.get("status").and_then(serde_json::Value::as_str) {
                Some("ready") => HealthSnapshot::ready(),
                Some(other) => HealthSnapshot::degraded(format!("firecrawl_status:{other}")),
                None => HealthSnapshot::error("firecrawl_status:missing".to_string()),
            },
...
async fn connector_suite_happy_path_scrapes_page() {
    ...
    let invoke = InvokeRequest {
        ...
        id: RequestId::new("firecrawl-connector-suite"),
        connector_id: ConnectorId::from_static(CONNECTOR_ID),
        operation: OperationId::from_static(OP_SCRAPE),
        zone_id: ZoneId::work(),
        input: json!({
            "url": "https://example.com",
            "formats": ["markdown"]
