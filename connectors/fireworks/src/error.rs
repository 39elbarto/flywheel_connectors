    ...
    match error {
        ...
        OpenAiError::Network(NetworkError::Cancelled { message }) => {
            FcpError::ConnectorUnavailable {
                code: 5000,
                message: format!("Fireworks request cancelled: {message}"),
            }
        }
        ...
        OpenAiError::Provider {
            ...
            message: format!("Fireworks provider returned HTTP {status}"),
