...
impl ChatInvokeInput {
    pub fn into_request(self, default_model: &str) -> FcpResult<ChatCompletionsRequest> {
        validate_fireworks_chat_input(&self)?;
...
impl LegacyCompletionsInput {
    pub fn into_request(self, default_model: &str) -> FcpResult<CompletionsRequest> {
        ...
        let mut provider_extensions = self.provider_extensions;
        validate_optional_context_length_behavior(
            self.context_length_exceeded_behavior.as_deref(),
        )?;
        insert_optional_string(
            &mut provider_extensions,
            "context_length_exceeded_behavior",
...
pub fn chat_request_from_value(
    ...
    let input: ChatInvokeInput =
        serde_json::from_value(value).map_err(|err| FcpError::InvalidRequest {
            code: 1003,
            message: format!("Invalid Fireworks chat input: {err}"),
        })?;
...
pub fn embeddings_request_from_value(
    ...
    let input: EmbeddingsInvokeInput =
        serde_json::from_value(value).map_err(|err| FcpError::InvalidRequest {
            code: 1003,
            message: format!("Invalid Fireworks embeddings input: {err}"),
        })?;
    input.into_request(if default_model.trim().is_empty() {
...
pub fn legacy_request_from_value(
    ...
    let input: LegacyCompletionsInput =
        serde_json::from_value(value).map_err(|err| FcpError::InvalidRequest {
            code: 1003,
            message: format!("Invalid Fireworks legacy completions input: {err}"),
        })?;
    input.into_request(if default_model.trim().is_empty() {
        DEFAULT_MODEL
    } else {
...
pub fn validate_fireworks_model_id(field: &str, model: &str) -> FcpResult<String> {
    ...
    match parts.as_slice() {
        ["accounts", account, "models", model_name]
            if !account.is_empty() && !model_name.is_empty() =>
        {
            Ok(trimmed.to_string())
        }
        [namespace, model_name] if !namespace.is_empty() && !model_name.is_empty() => {
            Ok(trimmed.to_string())
        }
        _ => invalid(&format!(
            "{field} must be accounts/<account>/models/<model> or namespace/model"
        )),
    }
}

fn validate_fireworks_chat_input(input: &ChatInvokeInput) -> FcpResult<()> {
    ...
    if input.top_logprobs.is_some_and(|value| value > 5) {
        return invalid("top_logprobs must be between 0 and 5 when supplied");
    }
    validate_optional_reasoning_effort(input.reasoning_effort.as_deref())?;
    validate_optional_context_length_behavior(input.context_length_exceeded_behavior.as_deref())?;
...
fn validate_optional_reasoning_effort(reasoning_effort: Option<&str>) -> FcpResult<()> {
    if reasoning_effort
        .is_some_and(|value| !matches!(value, "none" | "low" | "medium" | "high" | "max"))
    {
        return invalid("reasoning_effort must be none, low, medium, high, or max when supplied");
    }
    Ok(())
}

fn validate_optional_context_length_behavior(value: Option<&str>) -> FcpResult<()> {
    if value.is_some_and(|value| !matches!(value, "truncate" | "error")) {
        return invalid("context_length_exceeded_behavior must be truncate or error when supplied");
    }
    Ok(())
}
...
fn validate_optional_model(field: &str, model: Option<&str>) -> FcpResult<()> {
    if let Some(model) = model {
        validate_fireworks_model_id(field, model)?;
    }
    Ok(())
}
...
fn validate_or_default_model(model: Option<String>, default_model: &str) -> FcpResult<String> {
    model.map_or_else(
        || validate_fireworks_model_id("default_model", default_model),
        |model| validate_fireworks_model_id("model", &model),
    )
}
