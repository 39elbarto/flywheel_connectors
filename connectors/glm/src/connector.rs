...
use crate::client::{
    DEFAULT_BASE_URL, DEFAULT_EMBEDDING_MODEL, DEFAULT_MODEL, GlmAuth, GlmClient, GlmJwtAuth,
    GlmProvider, normalize_glm_base_url, split_bigmodel_api_key, validate_auth_material,
};
use crate::error::openai_error_to_fcp;
use crate::types::{chat_request_from_value, embeddings_request_from_value};

pub const CONNECTOR_ID: &str = "fcp.glm";
...
impl GlmConfig {
    fn from_params(params: &Value) -> FcpResult<Self> {
        ...
        let jwt_combined_credential = optional_auth(params, &config_field(&["jwt", "api", "key"]))?;
        let jwt_key_id = optional_auth(params, &config_field(&["api", "key", "id"]))?;
        let jwt_signing_material = optional_auth(
            params,
            &config_field(&["api", "key", "signing", "material"]),
        )?;
        let credential_id = optional_auth(params, "credential_id")?;
        let jwt_ttl = Duration::from_secs(optional_positive_u64(params, "jwt_ttl_seconds", 60)?);

        let auth = build_auth(
            direct_bearer,
            jwt_combined_credential,
            jwt_key_id,
            jwt_signing_material,
            credential_id,
            jwt_ttl,
        )?;
        let base_url = normalize_glm_base_url(params.get("base_url").and_then(Value::as_str))
        ...
        let default_model = params
            ...
            .to_string();
        let default_embedding_model = params
    ...
    fn build_client(&self) -> GlmClient {
        GlmClient::new(
            GlmProvider::new(self.base_url.clone(), self.auth.clone()),
...
pub struct GlmConnector {
    ...
    config: Option<GlmConfig>,
    client: Option<Arc<GlmClient>>,
...
impl GlmConnector {
    ...
    pub async fn handle_configure(&mut self, params: Value) -> FcpResult<Value> {
        let config = GlmConfig::from_params(&params)?;
        ...
        info!(auth = %auth_mode, base_url = %base_url, "GLM connector configured");
        Ok(json!({
            ...
            "checks": [
                {
                    ...
                    "critical": true,
                    "message": if self.config.is_some() { Value::Null } else { json!("Call configure with exactly one GLM bearer, combined JWT, split JWT, or host credential reference.") }
                },
                {
                    "name": "auth_redaction",
                ...
                },
                {
                    ...
                    "critical": true,
                    "message": "base_url is constrained to open.bigmodel.cn/api/paas/v4, with loopback allowed for tests"
                },
                {
                    "name": "handshake",
    ...
    async fn invoke_operation(&self, operation: &str, input: Value) -> FcpResult<Value> {
        ...
        let client = self.client.as_ref().ok_or_else(|| FcpError::Internal {
            message: "GLM client not initialized".into(),
        })?;
    ...
    pub async fn handle_simulate(&self, params: Value) -> FcpResult<Value> {
        let operation = params
            ...
            .unwrap_or("");
        Ok(json!({
            "allowed": matches!(operation, OP_CHAT | OP_CHAT_STREAM | OP_EMBEDDINGS | OP_MODELS | OP_HEALTH),
            "reason": if matches!(operation, OP_CHAT | OP_CHAT_STREAM | OP_EMBEDDINGS | OP_MODELS | OP_HEALTH) {
                "Supported operation."
            } else {
                "Unknown operation."
            }
...
impl Default for GlmConnector {
    fn default() -> Self {
        Self::new()
    }
}

fcp_core::impl_fcp_sealed!(GlmConnector);

#[fcp_core::async_trait]
impl FcpConnector for GlmConnector {
    ...
    async fn health(&self) -> HealthSnapshot {
        if self.config.is_some() && self.session_id.is_some() {
            ...
            HealthSnapshot::degraded("glm_handshake_pending")
        } else {
            HealthSnapshot::error("glm_not_configured")
        }
    }
    ...
    async fn self_check(&self) -> FcpResult<SelfCheckReport> {
        if self.config.is_none() {
            return Ok(SelfCheckReport::degraded(
                "not_configured",
                "GLM connector is not configured",
            ));
        }
        if self
            .config
    ...
    async fn simulate(&self, req: SimulateRequest) -> FcpResult<SimulateResponse> {
        if matches!(
            ...
            Ok(SimulateResponse::denied(
                req.id,
                "operation is not supported by GLM",
                "FCP-3010",
            ))
        }
    }
...
fn operations_info() -> Vec<OperationInfo> {
    vec![
        OperationInfo {
            ...
            summary: "Create a GLM chat completion".into(),
            description: Some("Uses BigModel POST /chat/completions.".into()),
            ...
            ai_hints: AgentHint {
                when_to_use: "Use direct GLM when a first-party Zhipu model is required.".into(),
                common_mistakes: vec![
                    "Do not log prompts, completions, API keys, JWTs, or JWT signatures.".into(),
                    "Use GLM's documented JWT credential shape when short-lived auth is required.".into(),
                ],
                examples: vec![r#"{"model":"glm-5.1","messages":[{"role":"user","content":"Hello"}]}"#.into()],
                related: vec![
        ...
        OperationInfo {
            ...
            summary: "Create a GLM streaming chat completion".into(),
            description: Some("Uses BigModel SSE streaming and returns chunk metadata plus assembled content.".into()),
            ...
            idempotency: IdempotencyClass::None,
            ai_hints: AgentHint {
                when_to_use: "Use when the caller can consume incremental GLM stream chunks.".into(),
        ...
        OperationInfo {
            ...
            summary: "Create GLM embeddings".into(),
            description: Some("Uses BigModel POST /embeddings with embedding-3 by default.".into()),
            ...
            ai_hints: AgentHint {
                when_to_use: "Use for first-party GLM embedding vectors.".into(),
                common_mistakes: vec!["Do not log source text used for embeddings.".into()],
                examples: vec![r#"{"model":"embedding-3","input":"hello"}"#.into()],
        ...
        OperationInfo {
            ...
            summary: "List documented GLM models".into(),
            description: Some("Returns a conservative documented static catalog because current public docs do not advertise /models.".into()),
            ...
            idempotency: IdempotencyClass::Strict,
            ai_hints: AgentHint {
                when_to_use: "Use before choosing a GLM model id.".into(),
        ...
        OperationInfo {
            ...
            summary: "Probe GLM connector health".into(),
            description: Some("Reports connector readiness and documented model catalog count without sending prompts.".into()),
            ...
            idempotency: IdempotencyClass::Strict,
            ai_hints: AgentHint {
                when_to_use: "Use for a bounded readiness check before dispatching user-visible GLM operations.".into(),
                common_mistakes: Vec::new(),
                examples: vec!["{}".into()],
                related: vec![CapabilityId::from_static(CAP_MODELS)],
            },
    ...
    match operation {
        ...
        OP_EMBEDDINGS => {
            let model = input
                ...
                .and_then(Value::as_str)
                .unwrap_or(DEFAULT_EMBEDDING_MODEL);
            vec![format!("glm:embedding-model:{model}")]
        }
        OP_MODELS | OP_HEALTH => vec!["glm:models".into()],
        _ => Vec::new(),
    }
}
...
fn build_auth(
    ...
    jwt_combined_credential: Option<String>,
    jwt_key_id: Option<String>,
    jwt_signing_material: Option<String>,
    credential_id: Option<String>,
    jwt_ttl: Duration,
) -> FcpResult<GlmAuth> {
    let jwt_parts = match (jwt_key_id, jwt_signing_material) {
        (Some(id), Some(signing_material)) => Some((id, signing_material)),
        (None, None) => None,
        _ => {
            return Err(FcpError::InvalidRequest {
                code: 1003,
                message: "Provide both split JWT fields for GLM JWT auth".into(),
            });
        }
    };
    let jwt_combined = jwt_combined_credential
        .map(|value| split_bigmodel_api_key(&value))
        .transpose()
        .map_err(invalid_config)?;
    let modes = [
        direct_bearer.is_some(),
        jwt_parts.is_some(),
        jwt_combined.is_some(),
    ...
    if modes != 1 {
        return Err(FcpError::InvalidRequest {
            code: 1003,
            message: "Provide exactly one GLM auth mode".into(),
        });
    }
    if let Some(key) = direct_bearer {
        Ok(GlmAuth::ApiKey(key))
    } else if let Some((id, signing_material)) = jwt_parts.or(jwt_combined) {
        Ok(GlmAuth::Jwt(GlmJwtAuth::new(id, signing_material, jwt_ttl)))
    } else if let Some(id) = credential_id {
        Ok(GlmAuth::CredentialId(id))
    } else {
        unreachable!("exactly one auth mode already verified")
    }
}
...
mod tests {
    ...
    fn handshake_manifest_hash_tracks_bundled_manifest() {
        ...
        let actual = GlmConnector::manifest_hash();

        assert_eq!(actual, expected);
        assert_ne!(actual, "sha256:glm-connector-v1");
    }
}
