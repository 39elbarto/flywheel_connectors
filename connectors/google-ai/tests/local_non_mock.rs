//! Local loopback acceptance coverage for the Google AI connector.

#![allow(
    clippy::future_not_send,
    clippy::missing_errors_doc,
    clippy::missing_panics_doc,
    clippy::must_use_candidate,
    clippy::too_many_lines,
    clippy::unreadable_literal,
    clippy::unused_async
)]

use std::{
    io::{self, Read, Write},
    net::{TcpListener, TcpStream},
    thread::{self, JoinHandle},
    time::Duration,
};

use chrono::{Duration as ChronoDuration, Utc};
use fcp_crypto::{cose::CapabilityTokenBuilder, ed25519::Ed25519SigningKey};
use fcp_google_ai::connector::GoogleAiConnector;
use fcp_prelude::{
    CapabilityConstraints, CapabilityId, CapabilityToken, FcpError, HandshakeRequest, InstanceId,
    ZoneId,
};
use serde_json::{Value, json};

const LOCAL_API_KEY: &str = "local-google-ai-key";
const OP_GENERATE_CONTENT: &str = "google-ai.generate_content";
const OP_LIST_MODELS: &str = "google-ai.list_models";
const OP_GET_USAGE: &str = "google-ai.get_usage";
const MODELS_CAPABILITY: &str = "google-ai.models";
const GENERATE_CAPABILITY: &str = "google-ai.generate";
const USAGE_CAPABILITY: &str = "google-ai.usage";

const LIST_MODELS_RESPONSE: &str = r#"{
  "models": [
    {
      "name": "models/gemini-2.0-flash",
      "displayName": "Gemini 2.0 Flash",
      "supportedGenerationMethods": ["generateContent", "countTokens"],
      "inputTokenLimit": 1048576,
      "outputTokenLimit": 8192
    }
  ],
  "nextPageToken": "models-page-2"
}"#;

const GENERATE_CONTENT_RESPONSE: &str = r#"{
  "candidates": [
    {
      "content": {
        "role": "model",
        "parts": [{ "text": "Local Gemini answer" }]
      },
      "finishReason": "STOP",
      "index": 0
    }
  ],
  "usageMetadata": {
    "promptTokenCount": 3,
    "candidatesTokenCount": 4,
    "totalTokenCount": 7
  },
  "modelVersion": "gemini-2.0-flash-001"
}"#;

#[derive(Debug)]
struct FixtureObservation {
    request_line: String,
    headers: String,
    body: String,
}

impl FixtureObservation {
    fn content_type_json_seen(&self) -> bool {
        header_value_contains(&self.headers, "content-type", "application/json")
    }

    fn credential_header_absent(&self) -> bool {
        !header_name_seen(&self.headers, "authorization")
            && !header_name_seen(&self.headers, "x-fcp-credential-id")
    }

    fn user_agent_seen(&self) -> bool {
        header_value_contains(&self.headers, "user-agent", "fcp-google-ai/0.1.0")
    }
}

struct LoopbackFixture {
    base_url: String,
    handle: Option<JoinHandle<FixtureObservation>>,
}

impl LoopbackFixture {
    fn start(response_status: &'static str, response_body: &'static str) -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind loopback listener");
        let address = listener.local_addr().expect("read listener address");
        let handle = thread::spawn(move || {
            let (stream, _) = listener.accept().expect("accept connector request");
            handle_request(stream, response_status, response_body)
        });

        Self {
            base_url: format!("http://{address}/v1beta"),
            handle: Some(handle),
        }
    }

    fn base_url(&self) -> &str {
        &self.base_url
    }

    fn join(mut self) -> FixtureObservation {
        self.handle
            .take()
            .expect("fixture thread present")
            .join()
            .expect("fixture thread completed")
    }
}

fn handle_request(
    mut stream: TcpStream,
    response_status: &str,
    response_body: &str,
) -> FixtureObservation {
    stream
        .set_read_timeout(Some(Duration::from_secs(5)))
        .expect("set read timeout");

    let (headers, body) = read_http_request(&mut stream);
    let request_line = headers.lines().next().unwrap_or_default().to_string();

    write!(
        stream,
        "HTTP/1.1 {response_status}\r\ncontent-type: application/json\r\ncontent-length: {}\r\nconnection: close\r\n\r\n{response_body}",
        response_body.len()
    )
    .expect("write connector response");

    FixtureObservation {
        request_line,
        headers,
        body,
    }
}

fn read_http_request(stream: &mut TcpStream) -> (String, String) {
    let mut request = Vec::new();
    let mut buffer = [0_u8; 1024];
    let header_end = loop {
        let bytes_read = stream.read(&mut buffer).expect("read connector request");
        assert!(bytes_read > 0, "connector should send request headers");
        request.extend_from_slice(&buffer[..bytes_read]);
        if let Some(position) = request.windows(4).position(|window| window == b"\r\n\r\n") {
            break position + 4;
        }
        assert!(request.len() < 8192, "request headers should stay bounded");
    };

    let header_text = String::from_utf8_lossy(&request[..header_end]).to_string();
    let content_length = content_length(&header_text);
    while request.len() < header_end + content_length {
        let bytes_read = stream.read(&mut buffer).expect("read connector body");
        assert!(bytes_read > 0, "connector body ended before content-length");
        request.extend_from_slice(&buffer[..bytes_read]);
        assert!(request.len() < 65536, "request body should stay bounded");
    }

    let body =
        String::from_utf8_lossy(&request[header_end..header_end + content_length]).to_string();
    (header_text, body)
}

fn content_length(headers: &str) -> usize {
    headers
        .lines()
        .find_map(|line| {
            let (name, value) = line.split_once(':')?;
            if name.eq_ignore_ascii_case("content-length") {
                Some(value.trim().parse::<usize>().expect("valid content-length"))
            } else {
                None
            }
        })
        .unwrap_or(0)
}

fn header_name_seen(headers: &str, expected_name: &str) -> bool {
    headers.lines().any(|line| {
        line.split_once(':')
            .is_some_and(|(name, _)| name.eq_ignore_ascii_case(expected_name))
    })
}

fn header_value_contains(headers: &str, expected_name: &str, expected_value: &str) -> bool {
    headers.lines().any(|line| {
        let Some((name, value)) = line.split_once(':') else {
            return false;
        };
        name.eq_ignore_ascii_case(expected_name)
            && value
                .to_ascii_lowercase()
                .contains(&expected_value.to_ascii_lowercase())
    })
}

fn request_target(request_line: &str) -> &str {
    request_line.split_whitespace().nth(1).unwrap_or_default()
}

fn request_path(request_line: &str) -> &str {
    let target = request_target(request_line);
    target.split_once('?').map_or(target, |(path, _)| path)
}

fn request_line_contains_query_pair(request_line: &str, name: &str, value: &str) -> bool {
    request_target(request_line)
        .split_once('?')
        .map(|(_, query)| query)
        .is_some_and(|query| {
            query
                .split('&')
                .any(|pair| pair == format!("{name}={value}"))
        })
}

fn redacted_request_line(request_line: &str) -> String {
    request_line.replace(LOCAL_API_KEY, "<redacted>")
}

fn handshake_req(host_public_key: [u8; 32], instance_id: &InstanceId) -> HandshakeRequest {
    HandshakeRequest {
        protocol_version: "2.0".to_string(),
        zone: ZoneId::work(),
        zone_dir: None,
        host_public_key,
        nonce: [44_u8; 32],
        capabilities_requested: vec![
            CapabilityId::from_static(MODELS_CAPABILITY),
            CapabilityId::from_static(GENERATE_CAPABILITY),
            CapabilityId::from_static(USAGE_CAPABILITY),
        ],
        host: None,
        transport_caps: None,
        requested_instance_id: Some(instance_id.clone()),
    }
}

fn capability_token(
    signing_key: &Ed25519SigningKey,
    instance_id: &InstanceId,
    capability: &str,
    operation: &str,
) -> CapabilityToken {
    let constraints = CapabilityConstraints {
        resource_allow: vec!["*".into()],
        ..Default::default()
    };
    let mut cbor = Vec::new();
    ciborium::into_writer(&constraints, &mut cbor).expect("serialize constraints");

    let now = Utc::now();
    let raw = CapabilityTokenBuilder::new()
        .capability_id(capability)
        .zone_id("z:work")
        .target_instance(instance_id.as_str())
        .principal("user:local-google-ai")
        .operations(&[operation])
        .issuer("node:local-google-ai")
        .validity(now, now + ChronoDuration::hours(1))
        .try_constraints_cbor(&cbor)
        .expect("constraints cbor should validate")
        .sign(signing_key)
        .expect("capability token should sign");
    CapabilityToken::from_raw(raw)
}

async fn setup_connector(base_url: &str) -> (GoogleAiConnector, Ed25519SigningKey, InstanceId) {
    let mut connector = GoogleAiConnector::new();
    let signing_key = Ed25519SigningKey::generate();
    let instance_id = InstanceId::new();

    connector
        .handle_configure(json!({
            "api_key": LOCAL_API_KEY,
            "base_url": base_url
        }))
        .await
        .expect("configure connector");
    connector
        .handle_handshake(
            serde_json::to_value(handshake_req(
                signing_key.verifying_key().to_bytes(),
                &instance_id,
            ))
            .expect("serialize handshake request"),
        )
        .await
        .expect("handshake connector");

    (connector, signing_key, instance_id)
}

fn generate_content_input() -> Value {
    json!({
        "model": "gemini-2.0-flash",
        "contents": [
            {
                "role": "user",
                "parts": [{ "text": "Say hello from the local fixture" }]
            }
        ]
    })
}

#[fcp_async_core::runtime::test]
async fn local_non_mock_list_models_uses_google_ai_request_boundary() {
    let fixture = LoopbackFixture::start("200 OK", LIST_MODELS_RESPONSE);
    let (connector, signing_key, instance_id) = setup_connector(fixture.base_url()).await;

    let health = connector.handle_health().await.expect("health response");
    assert_eq!(health["status"], "healthy");
    assert_eq!(health["auth"], "api_key:redacted");

    let doctor = connector.handle_doctor().await.expect("doctor response");
    assert_eq!(doctor["status"], "healthy");

    let introspection = connector
        .handle_introspect()
        .await
        .expect("introspection response");
    let operations = introspection["operations"].as_array().expect("operations");
    assert!(operations.iter().any(|operation| {
        operation["id"] == OP_LIST_MODELS && operation["capability"] == MODELS_CAPABILITY
    }));

    let result = connector
        .handle_invoke(json!({
            "operation": OP_LIST_MODELS,
            "input": {
                "page_size": 2
            },
            "capability_token": capability_token(
                &signing_key,
                &instance_id,
                MODELS_CAPABILITY,
                OP_LIST_MODELS,
            )
        }))
        .await
        .expect("list models through connector");
    let observation = fixture.join();

    assert_eq!(request_path(&observation.request_line), "/v1beta/models");
    assert!(request_line_contains_query_pair(
        &observation.request_line,
        "key",
        LOCAL_API_KEY,
    ));
    assert!(request_line_contains_query_pair(
        &observation.request_line,
        "pageSize",
        "2",
    ));
    assert!(observation.credential_header_absent());
    assert!(!observation.content_type_json_seen());
    assert!(observation.user_agent_seen());
    assert!(observation.body.is_empty());
    assert_eq!(result["models"][0]["name"], "models/gemini-2.0-flash");
    assert_eq!(result["nextPageToken"], "models-page-2");
    assert!(!result.to_string().contains(LOCAL_API_KEY));

    let artifact = json!({
        "connector": "google-ai",
        "suite_class": "local_non_mock",
        "acceptance_suite_class": "local_non_mock",
        "bead_id": "flywheel_connectors-bky21.3.6.35",
        "fixture_mode": "loopback_http",
        "provider_class": "local_sufficient",
        "operation": OP_LIST_MODELS,
        "method": "GET",
        "request_line": redacted_request_line(&observation.request_line),
        "auth_gate": {
            "mode": "api_key_query",
            "api_key_query_seen": request_line_contains_query_pair(
                &observation.request_line,
                "key",
                LOCAL_API_KEY,
            ),
            "credential_headers_absent": observation.credential_header_absent(),
            "instance_bound_token_verified": true
        },
        "headers": {
            "user_agent_seen": observation.user_agent_seen()
        },
        "diagnostics": {
            "health_status": health["status"],
            "doctor_status": doctor["status"]
        },
        "cleanup": "fixture_thread_joined",
        "result": "passed"
    });
    println!("{artifact}");
}

#[fcp_async_core::runtime::test]
async fn local_non_mock_self_check_lists_one_model_without_secret_leakage() {
    let fixture = LoopbackFixture::start("200 OK", LIST_MODELS_RESPONSE);
    let (connector, _, _) = setup_connector(fixture.base_url()).await;

    let report = connector
        .handle_self_check()
        .await
        .expect("self-check response");
    let observation = fixture.join();

    assert_eq!(request_path(&observation.request_line), "/v1beta/models");
    assert!(request_line_contains_query_pair(
        &observation.request_line,
        "key",
        LOCAL_API_KEY,
    ));
    assert!(request_line_contains_query_pair(
        &observation.request_line,
        "pageSize",
        "1",
    ));
    assert!(observation.credential_header_absent());
    assert!(observation.user_agent_seen());
    assert_eq!(report["status"], "ok");
    assert!(!report.to_string().contains(LOCAL_API_KEY));

    let artifact = json!({
        "connector": "google-ai",
        "suite_class": "local_non_mock",
        "acceptance_suite_class": "local_non_mock",
        "bead_id": "flywheel_connectors-bky21.3.6.35",
        "fixture_mode": "loopback_http",
        "provider_class": "local_sufficient",
        "operation": "self_check",
        "method": "GET",
        "request_line": redacted_request_line(&observation.request_line),
        "api_key_query_seen": request_line_contains_query_pair(
            &observation.request_line,
            "key",
            LOCAL_API_KEY,
        ),
        "secret_leaked": false,
        "cleanup": "fixture_thread_joined",
        "result": "passed"
    });
    println!("{artifact}");
}

#[fcp_async_core::runtime::test]
async fn local_non_mock_generate_content_posts_body_and_records_usage() {
    let fixture = LoopbackFixture::start("200 OK", GENERATE_CONTENT_RESPONSE);
    let (connector, signing_key, instance_id) = setup_connector(fixture.base_url()).await;

    let result = connector
        .handle_invoke(json!({
            "operation": OP_GENERATE_CONTENT,
            "input": generate_content_input(),
            "capability_token": capability_token(
                &signing_key,
                &instance_id,
                GENERATE_CAPABILITY,
                OP_GENERATE_CONTENT,
            )
        }))
        .await
        .expect("generate content through connector");
    let observation = fixture.join();
    let body: Value = serde_json::from_str(&observation.body).expect("request body json");

    assert_eq!(
        request_path(&observation.request_line),
        "/v1beta/models/gemini-2.0-flash:generateContent"
    );
    assert!(request_line_contains_query_pair(
        &observation.request_line,
        "key",
        LOCAL_API_KEY,
    ));
    assert!(observation.credential_header_absent());
    assert!(observation.content_type_json_seen());
    assert!(observation.user_agent_seen());
    assert_eq!(
        body["contents"][0]["parts"][0]["text"],
        "Say hello from the local fixture"
    );
    assert_eq!(
        result["candidates"][0]["content"]["parts"][0]["text"],
        "Local Gemini answer"
    );
    assert_eq!(result["usageMetadata"]["promptTokenCount"], 3);
    assert_eq!(result["usageMetadata"]["candidatesTokenCount"], 4);
    assert_eq!(result["provenance"]["source"], "google-ai");
    assert_eq!(result["provenance"]["model"], "gemini-2.0-flash");
    assert!(!result.to_string().contains(LOCAL_API_KEY));

    let usage = connector
        .handle_invoke(json!({
            "operation": OP_GET_USAGE,
            "input": {},
            "capability_token": capability_token(
                &signing_key,
                &instance_id,
                USAGE_CAPABILITY,
                OP_GET_USAGE,
            )
        }))
        .await
        .expect("get usage through connector");
    assert_eq!(usage["total_input_tokens"], 3);
    assert_eq!(usage["total_output_tokens"], 4);
    assert_eq!(usage["requests_total"], 1);
    assert_eq!(usage["requests_error"], 0);

    let artifact = json!({
        "connector": "google-ai",
        "suite_class": "local_non_mock",
        "acceptance_suite_class": "local_non_mock",
        "bead_id": "flywheel_connectors-bky21.3.6.35",
        "fixture_mode": "loopback_http",
        "provider_class": "local_sufficient",
        "operation": OP_GENERATE_CONTENT,
        "method": "POST",
        "request_line": redacted_request_line(&observation.request_line),
        "auth_gate": {
            "mode": "api_key_query",
            "api_key_query_seen": request_line_contains_query_pair(
                &observation.request_line,
                "key",
                LOCAL_API_KEY,
            ),
            "credential_headers_absent": observation.credential_header_absent(),
            "instance_bound_token_verified": true
        },
        "body_shape": {
            "content_count": body["contents"].as_array().map_or(0, Vec::len),
            "first_part_text_seen": body["contents"][0]["parts"][0]["text"]
                == "Say hello from the local fixture"
        },
        "usage": usage,
        "cleanup": "fixture_thread_joined",
        "result": "passed"
    });
    println!("{artifact}");
}

#[fcp_async_core::runtime::test]
async fn local_non_mock_wrong_capability_fails_before_loopback_egress() {
    let listener = TcpListener::bind("127.0.0.1:0").expect("bind loopback listener");
    listener
        .set_nonblocking(true)
        .expect("set nonblocking listener");
    let base_url = format!(
        "http://{}/v1beta",
        listener.local_addr().expect("read listener address")
    );
    let (connector, signing_key, instance_id) = setup_connector(&base_url).await;

    let error = connector
        .handle_invoke(json!({
            "operation": OP_GENERATE_CONTENT,
            "input": generate_content_input(),
            "capability_token": capability_token(
                &signing_key,
                &instance_id,
                MODELS_CAPABILITY,
                OP_GENERATE_CONTENT,
            )
        }))
        .await
        .expect_err("models capability should not authorize content generation");

    assert!(matches!(
        error,
        FcpError::CapabilityDenied { .. } | FcpError::OperationNotGranted { .. }
    ));
    let accept_error = listener
        .accept()
        .expect_err("capability denial should happen before loopback egress");
    assert_eq!(accept_error.kind(), io::ErrorKind::WouldBlock);

    let artifact = json!({
        "connector": "google-ai",
        "suite_class": "local_non_mock",
        "acceptance_suite_class": "local_non_mock",
        "bead_id": "flywheel_connectors-bky21.3.6.35",
        "fixture_mode": "loopback_http",
        "provider_class": "local_sufficient",
        "operation": OP_GENERATE_CONTENT,
        "denial": "wrong_capability",
        "loopback_egress_attempted": false,
        "result": "passed"
    });
    println!("{artifact}");
}
