//! Local loopback acceptance coverage for the Google Slides connector.
...
const READ_CAPABILITY: &str = "slides.read";
const WRITE_CAPABILITY: &str = "slides.write";
const DOCUMENT_ID: &str = "doc_test_123";
const EXPECTED_PATH: &str = "/v1/presentations/doc_test_123?";

const SUCCESS_RESPONSE: &str = r#"{
  "presentationId": "doc_test_123",
...
fn handle_request(
    ...
    let user_agent_seen = header_value_contains(&headers, "user-agent", "fcp-google-slides/0.1.0");
...
async fn setup_connector(base_url: &str) -> (SlidesConnector, Ed25519SigningKey, InstanceId) {
...
fn get_input() -> Value {
    json!({ "presentation_id": DOCUMENT_ID })
}

#[fcp_async_core::runtime::test]
async fn local_non_mock_get_presentation_uses_slides_request_boundary() {
    ...
    let result = connector
        ...
        .await
        .expect("get presentation through connector");
    ...
    assert_eq!(
        result["presentation"]["metadata"]["presentation_id"],
        DOCUMENT_ID
    );
    assert_eq!(
        result["presentation"]["metadata"]["title"],
        "Connector Suite Notes"
    );
    assert_eq!(result["presentation"]["metadata"]["revision_id"], "rev-1");
    assert_eq!(result["presentation"]["text"], "Hello from Slides");
    assert_eq!(result["presentation"]["text_complete"], true);
    ...
    let artifact = json!({
        ...
        "operation": OP_GET_DOCUMENT,
        "method": "GET",
        "endpoint_shape": "/v1/presentations/{presentation_id}",
        "request_target_verified": true,
        "presentation_id_redacted": true,
        "auth_gate": {
            "mode": "bearer",
...
async fn local_non_mock_wrong_capability_fails_before_loopback_egress() {
    ...
    let error = connector
        ...
        .await
        .expect_err("write capability should not authorize presentation reads");
