//! FCP `Home Assistant` Connector - Main entrypoint
//!
//! A `Home Assistant` smart home connector implementing the Flywheel Connector Protocol.
//! Provides access to entity states, service calls, automations, scenes, and history.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Home Assistant Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
fn run_fcp_loop() -> Result<()> {
    ...
    let mut connector = HomeAssistantConnector::new();
...
async fn handle_message(
    connector: &mut HomeAssistantConnector,
    message: &str,
) -> serde_json::Value {
