//! Local loopback acceptance coverage for the `Home Assistant` REST boundary.
...
const TOKEN: &str = "ha_local_token";
const AUTH_HEADER: &str = "Bearer ha_local_token";
...
async fn local_non_mock_list_states_uses_loopback_api_boundary() {
    let fixture = LoopbackFixture::start();
    let client = HomeAssistantClient::new(
        HomeAssistantAuth::BearerToken(TOKEN.into()),
        Some(fixture.base_url()),
    )
    .expect("construct Home Assistant client");

    let response = client
        .list_states()
        .await
        .expect("list states through loopback fixture");
    client.shutdown();
    let observation = fixture.join();

    assert_eq!(ACCEPTANCE_SUITE_CLASS, "local_non_mock");
    assert_eq!(observation.request_line, "GET /api/states HTTP/1.1");
    assert!(has_header(
        &observation.headers,
        "authorization",
        AUTH_HEADER
    ...
    assert!(has_header(
        &observation.headers,
        "user-agent",
        "fcp-homeassistant/0.1.0 (FCP connector)"
    ));
    assert_eq!(response[0]["entity_id"], "sensor.local_temperature");
    assert_eq!(response[0]["state"], "72");
    ...
    let artifact = json!({
        ...
        "command": "cargo test -p fcp-homeassistant --test local_non_mock -- --nocapture",
