...
const CONNECTOR_ID: &str = "hue";
const PROVIDER: &str = "Philips Hue bridge";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "HUE_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "HUE_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "HUE_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str = "cargo test -p fcp-hue --test live_verification -- --nocapture";

#[test]
fn hue_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the Hue bridge lab network.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe Hue device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the Hue lab bridge.",
        )
        .with_account_setup(
            "Use a dedicated Hue bridge or test room; avoid personal rooms and record only hashed device identifiers.",
        )
        .with_budget(0.01)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "hue_device_lab_verification",
    ...
    println!("HUE_DEVICE_LAB_JSONL {serialized}");
}
