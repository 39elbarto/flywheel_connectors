//! FCP `iMessage` (`BlueBubbles`) Connector - Main entrypoint.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP iMessage (BlueBubbles) Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut BlueBubblesConnector, message: &str) -> serde_json::Value {
