//! Local loopback acceptance coverage for the `Intercom` connector.
...
const BEAD_ID: &str = "flywheel_connectors-bky21.4.6.4.3";
const CONNECTOR_ID: &str = "fcp.intercom";
const ACCESS_TOKEN: &str = "local-intercom-access-token";
...
const SENSITIVE_NAME: &str = "Acceptance Contact";
const STARTING_AFTER: &str = "cursor-safe";
...
const LIST_RESPONSE_BODY: &str = r#"{
  ...
  "pages": {"next": {"starting_after": "cursor-provider"}}
}"#;
...
const RATE_LIMIT_BODY: &str = r#"{"message":"provider body"}"#;
...
impl LoopbackFixture {
    fn start(responses: Vec<ResponseSpec>) -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind Intercom listener");
...
async fn local_non_mock_contacts_list_and_create_use_loopback_boundary() {
    ...
    assert_eq!(
        ...
        "/contacts?per_page=2&starting_after=cursor-safe"
    );
    assert_eq!(observations[1].method(), "POST");
    assert_eq!(observations[1].target(), "/contacts");
    ...
    assert_eq!(body["email"], SENSITIVE_EMAIL);
    assert_eq!(body["name"], SENSITIVE_NAME);
    assert_eq!(contacts["data"].as_array().map_or(0, Vec::len), 1);
    assert_eq!(created["id"], CONTACT_ID);
...
async fn local_non_mock_rate_limit_maps_retry_after_without_secret_logging() {
    ...
    assert_eq!(observations[0].method(), "GET");
    assert_eq!(observations[0].target(), "/contacts");
    assert_auth_headers(&observations);
    match err {
        FcpError::External {
            service,
...
async fn evidence_schema_carries_connector_and_tracker_identity() {
    ...
    assert_eq!(
        RequestId::new("intercom-local").to_string(),
        "intercom-local"
    );
    assert_eq!(ZoneId::work().as_str(), "z:work");

    let introspection = IntercomConnector::new()
        .handle_introspect()
        .await
...
async fn configured_connector(base_url: &str) -> IntercomConnector {
    ...
    connector
        ...
        .await
        .expect("configure Intercom connector");
    connector
        .handle_handshake(json!({"session_id": "intercom-local"}))
        .await
        .expect("handshake Intercom connector");
    connector
}
...
fn assert_auth_headers(observations: &[RequestObservation]) {
    for observation in observations {
        assert_eq!(
            ...
            Some("Bearer local-intercom-access-token")
        );
    }
}
...
fn capability_for_operation(operation: &'static str) -> &'static str {
    match operation {
        OP_CONTACTS_LIST => "intercom.contacts.read",
        OP_CONTACTS_CREATE => "intercom.contacts.write",
...
fn route_label(request: &RequestObservation) -> &'static str {
    match (request.method(), request.target()) {
        ("GET", target) if target.starts_with("/contacts") => "contacts_list",
