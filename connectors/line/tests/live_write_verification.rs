...
const CONNECTOR: &str = "line";
const PROVIDER: &str = "LINE Messaging API";
const GATE_ENV: &str = "FCP_LIVE_WRITE";
const WRITE_OPERATION: &str = "line.rich_menu.create";
const CLEANUP_OPERATION: &str = "line.rich_menu.delete";
const TARGET_ENV: &str = "LINE_LIVE_TEST_CHANNEL_ID";
const AUTH_FAILURE_MAPPING: &str = "LINE 401/403 maps to unauthorized before rich-menu mutation";
...
fn manifest() -> EnvironmentManifest {
    EnvironmentManifest::live_write(CONNECTOR, PROVIDER)
        .with_env_secret(
            ...
            "LINE channel access token for a disposable test channel",
        )
        .with_env_var(
            TARGET_ENV,
            "Dedicated LINE test channel where synthetic rich menus can be created and deleted",
        )
        .with_account_setup(
            "Use a disposable LINE Messaging API channel. The suite creates one prefixed rich menu and must delete the same rich menu before passing.",
        )
        .with_budget(0.25)
        ...
        .with_metadata(
            ...
            json!({
                "list_operation": "line.rich_menu.list",
...
fn live_write_cleanup_verification_escalates_stale_resources() {
    ...
    let stale_resource = format!("{prefix}-rich-menu-orphan-00000000-20000101");
