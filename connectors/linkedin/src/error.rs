//! Error types for the LinkedIn connector.
...
/// Result alias for LinkedIn operations.
pub type LinkedInResult<T> = Result<T, LinkedInError>;
...
pub enum LinkedInError {
    ...
    #[error("LinkedIn API error ({status_code}): {message}")]
...
impl ConnectorErrorMapping for LinkedInError {
...
mod tests {
    ...
    fn unauthorized_not_retryable() {
        assert!(!LinkedInError::Unauthorized.is_retryable());
    }

    #[test]
    fn forbidden_not_retryable() {
        assert!(!LinkedInError::Forbidden.is_retryable());
    }

    #[test]
    fn not_found_not_retryable() {
        assert!(
            !LinkedInError::NotFound {
    ...
    fn retry_after_none_for_unauthorized() {
        assert_eq!(LinkedInError::Unauthorized.retry_after(), None);
    }

    #[test]
    fn retry_after_none_for_forbidden() {
        assert_eq!(LinkedInError::Forbidden.retry_after(), None);
    }
    ...
    fn not_found_to_fcp_error() {
        match (LinkedInError::NotFound {
            resource: "post_abc".into(),
        })
        .to_fcp_error()
        {
            FcpError::External {
                ...
                assert!(message.contains("post_abc"));
                assert!(!retryable);
            }
            other => panic!("expected External, got {other:?}"),
        }
    ...
    fn json_error_to_fcp_internal() {
        ...
        match LinkedInError::Json(bad.unwrap_err()).to_fcp_error() {
    ...
    fn error_display_unauthorized() {
        assert_eq!(
            LinkedInError::Unauthorized.to_string(),
    ...
    fn error_display_forbidden() {
        assert_eq!(
            LinkedInError::Forbidden.to_string(),
    ...
    fn error_display_api() {
        assert_eq!(
            ...
            .to_string(),
            "LinkedIn API error (500): Internal"
        );
    }
