//! FCP LinkedIn Connector - Main entrypoint
//!
//! A LinkedIn social networking connector implementing the Flywheel Connector Protocol.
//! Provides access to profiles, posts, connections, companies, and analytics.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP LinkedIn Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
fn run_fcp_loop() -> Result<()> {
    ...
    let mut connector = LinkedInConnector::new();
...
async fn handle_message(connector: &mut LinkedInConnector, message: &str) -> serde_json::Value {
