//! Local loopback acceptance coverage for the `LinkedIn` REST boundary.
...
const ACCESS_TOKEN: &str = "linkedin_local_token";
const AUTH_HEADER: &str = "Bearer linkedin_local_token";
const RESTLI_HEADER: &str = "2.0.0";
const RESPONSE_BODY: &str = r#"{
  "id": "local-member",
  "localizedFirstName": "Local",
...
async fn local_non_mock_get_profile_uses_loopback_api_boundary() {
    let fixture = LoopbackFixture::start();
    let client = LinkedInClient::new(
        LinkedInAuth::AccessToken(ACCESS_TOKEN.into()),
        Some(fixture.base_url()),
    )
    .expect("construct LinkedIn client");

    let response = client
        .get_profile()
        .await
        .expect("get profile through loopback fixture");
    client.shutdown();
    let observation = fixture.join();

    assert_eq!(ACCEPTANCE_SUITE_CLASS, "local_non_mock");
    assert_eq!(observation.request_line, "GET /v2/me HTTP/1.1");
    assert!(has_header(
        &observation.headers,
        "authorization",
        AUTH_HEADER
    ...
    assert!(has_header(
        &observation.headers,
        "user-agent",
        "fcp-linkedin/0.1.0 (FCP connector)"
    ));
    assert_eq!(response["id"], "local-member");
    assert_eq!(response["localizedFirstName"], "Local");
    ...
    let artifact = json!({
        ...
        "command": "cargo test -p fcp-linkedin --test local_non_mock -- --nocapture",
        ...
        "auth_gate": {
            ...
            "restli_protocol_header_verified": true
        },
