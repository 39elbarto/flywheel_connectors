...
pub const DEFAULT_EMBEDDING_MODEL: &str = "local-embedding-model";
pub const USER_AGENT: &str = "fcp-lm-studio/0.1.0";
...
pub struct LmStudioUrlPolicy {
    pub tailnet_only: bool,
    pub allowed_hosts: Vec<String>,
}

impl LmStudioUrlPolicy {
...
impl LmStudioProvider {
    pub fn new(base_url: impl Into<String>, auth: LmStudioAuth) -> Self {
        Self {
...
impl OpenAiCompatProvider for LmStudioProvider {
    ...
    fn auth_header(&self, req: &mut HttpRequest) {
        match &self.auth {
            LmStudioAuth::None => {}
            LmStudioAuth::ApiKey(key) => req.bearer_auth(key),
            LmStudioAuth::CredentialId(id) => req.upsert_header("x-fcp-credential-id", id),
        }
    }
...
pub struct LmStudioClient {
    inner: OpenAiCompatClient<LmStudioProvider>,
...
impl LmStudioClient {
    ...
    pub const fn provider(&self) -> &LmStudioProvider {
        &self.provider
    }
...
pub fn normalize_lm_studio_base_url(
    raw: Option<&str>,
    policy: &LmStudioUrlPolicy,
...
pub fn classify_lm_studio_base_url(base_url: &str) -> &'static str {
