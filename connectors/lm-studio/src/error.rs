    ...
    match error {
        ...
        OpenAiError::Network(NetworkError::Cancelled { message }) => {
            FcpError::ConnectorUnavailable {
                code: 5000,
                message: format!("LM Studio request cancelled: {message}"),
            }
        }
        ...
        OpenAiError::Provider {
            ...
            message: format!("LM Studio provider returned HTTP {status}"),
