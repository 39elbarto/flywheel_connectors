...
use fcp_lm_studio::connector::{CONNECTOR_ID, test_handshake_request, test_invoke_request};
use fcp_lm_studio::types::{
...
async fn configured_connector(
    ...
    extra_config: Value,
) -> (fcp_lm_studio::LmStudioConnector, Ed25519SigningKey) {
    let mut connector = fcp_lm_studio::LmStudioConnector::new();
...
async fn invoke(
    connector: &fcp_lm_studio::LmStudioConnector,
    signing_key: &Ed25519SigningKey,
...
fn provider_construction_optional_auth_base_url_and_model_policy() {
    let provider = LmStudioProvider::new(DEFAULT_BASE_URL, LmStudioAuth::None);
    let mut request = HttpRequest::default();
    provider.auth_header(&mut request);
    assert!(request.headers.is_empty());
    assert_eq!(provider.provider_name(), "lm_studio");

    let api_key_provider = LmStudioProvider::new(
        DEFAULT_BASE_URL,
        LmStudioAuth::ApiKey("lm-studio-key".into()),
    );
    ...
    assert_eq!(
        ...
        Some("Bearer lm-studio-key")
    );

    let credential_provider = LmStudioProvider::new(
        DEFAULT_BASE_URL,
        LmStudioAuth::CredentialId("cred:lm-studio".into()),
    );
    ...
    let default_policy = LmStudioUrlPolicy::default();
    assert_eq!(
        normalize_lm_studio_base_url(None, &default_policy).unwrap(),
        DEFAULT_BASE_URL
    );
    assert!(
        normalize_lm_studio_base_url(Some("https://api.openai.com/v1"), &default_policy).is_err()
    );
    assert!(
        normalize_lm_studio_base_url(Some("http://localhost:1234/api"), &default_policy).is_err()
    );

    let allowed = LmStudioUrlPolicy::new(false, vec!["lm-studio.tailnet.ts.net".into()]);
    assert_eq!(
        normalize_lm_studio_base_url(Some("https://lm-studio.tailnet.ts.net/v1"), &allowed)
            .unwrap(),
        "https://lm-studio.tailnet.ts.net/v1"
    );
    let tailnet_only = LmStudioUrlPolicy::new(true, vec!["lm-studio.tailnet.ts.net".into()]);
    assert!(normalize_lm_studio_base_url(None, &tailnet_only).is_err());
    assert_eq!(
        classify_lm_studio_base_url("https://lm-studio.tailnet.ts.net/v1"),
        "tailnet_dns"
    );
    assert!(validate_lm_studio_model_id("model", DEFAULT_MODEL).is_ok());
    assert!(validate_lm_studio_model_id("model", "lmstudio-community/qwen2.5-7b-instruct").is_ok());
    assert!(validate_lm_studio_model_id("model", "model with spaces").is_err());
}

#[test]
fn request_builders_validate_chat_embeddings_and_lm_studio_extensions() {
    let chat = chat_request_from_value(
        json!({
            "messages": [{"role": "user", "content": "private prompt"}],
            "provider_extensions": {"fixture_case": "json"}
        }),
        DEFAULT_MODEL,
    )
    .expect("chat request should parse");
    assert_eq!(chat.model, DEFAULT_MODEL);
    assert_eq!(chat.provider_extensions["fixture_case"], "json");
...
async fn chat_stream_embeddings_models_health_and_redaction_work_through_connector() {
    ...
    Mock::given(method("POST"))
        ...
        .await;
    let sse = concat!(
        "data: {\"id\":\"chunk-1\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"local-model\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"hel\"},\"finish_reason\":null}]}\n\n",
        "data: {\"id\":\"chunk-2\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"local-model\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"lo\"},\"finish_reason\":\"stop\"}]}\n\n",
        "data: [DONE]\n\n"
    );
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
...
async fn auth_required_tailnet_policy_rate_limit_cancellation_and_shutdown_are_safe() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .and(header("authorization", "Bearer lm-studio-proxy-key"))
    ...
    let (mut connector, signing_key) = configured_connector(
        &server,
        &[CAP_CHAT],
        json!({"api_key": "lm-studio-proxy-key", "wait_on_rate_limit_ms": 1000}),
    )
    .await;
...
#[fcp_async_core::runtime::test]
async fn lm_studio_loopback_e2e_jsonl_matrix() {
    let git_revision =
        std::env::var("LM_STUDIO_E2E_GIT_REVISION").unwrap_or_else(|_| "unknown".to_string());
    let server = MockServer::start().await;
    mount_models(&server, DEFAULT_MODEL, 1).await;
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        ...
        .await;
    let sse = concat!(
        "data: {\"id\":\"chunk-a\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"local-model\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"ok\"},\"finish_reason\":\"stop\"}]}\n\n",
        "data: [DONE]\n\n"
    );
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
...
#[fcp_async_core::runtime::test]
async fn lm_studio_local_smoke_or_structured_skip_jsonl() {
    let git_revision =
        std::env::var("LM_STUDIO_E2E_GIT_REVISION").unwrap_or_else(|_| "unknown".to_string());
    let base_url =
        std::env::var("LM_STUDIO_E2E_BASE_URL").unwrap_or_else(|_| DEFAULT_BASE_URL.into());
    let model = std::env::var("LM_STUDIO_E2E_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());

    if base_url == DEFAULT_BASE_URL && !localhost_lm_studio_listens() {
        emit_jsonl(
            &git_revision,
            "local_smoke",
            "local",
            ...
            json!({
                "base_url_class": "loopback",
                "skip_reason": "LM Studio is not listening on 127.0.0.1:1234",
    ...
    let Ok(configure) = configure else {
        emit_jsonl(
            ...
            json!({
                ...
                "skip_reason": "LM_STUDIO_E2E_BASE_URL failed connector base_url policy",
                "cleanup_result": "not_started_by_test"
            }),
        );
        return;
            ...
            if model_present {
                ...
                emit_jsonl(
                    ...
                    json!({
                        "base_url_class": configured_base_url_class,
                        "model_id_hash": blake3::hash(model.as_bytes()).to_hex().to_string(),
                        "skip_reason": "configured model is not loaded; connector does not auto-load models",
                        "cleanup_result": "shutdown_after_probe"
                    }),
                );
            }
...
fn connector_id_matches_manifest_contract() {
    assert_eq!(CONNECTOR_ID, "fcp.lm_studio");
}
...
fn emit_jsonl(
    ...
    record.insert("event".into(), json!("lm_studio_fixture_operation"));
    ...
    record.insert(
        "command_line".into(),
        json!("cargo test -p fcp-lm-studio --test integration lm_studio_loopback_e2e_jsonl_matrix -- --nocapture"),
    );
    ...
    println!("LM_STUDIO_E2E_JSONL {}", Value::Object(record));
}

fn localhost_lm_studio_listens() -> bool {
    let addr: SocketAddr = "127.0.0.1:1234".parse().expect("static socket addr parses");
