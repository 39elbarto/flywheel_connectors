//! Local loopback acceptance coverage for the FCP `LM Studio` connector.
...
use fcp_lm_studio::{DEFAULT_EMBEDDING_MODEL, DEFAULT_MODEL, LmStudioConnector};
use fcp_prelude::{CapabilityConstraints, CapabilityToken, FcpError, InstanceId};
use serde_json::{Value, json};

const CONNECTOR: &str = "lm-studio";
const PACKAGE: &str = "fcp-lm-studio";
const BEAD_ID: &str = "flywheel_connectors-37fca";
const ACCEPTANCE_SUITE_CLASS: &str = "local_non_mock";
const API_SECRET: &str = "lm-studio-local-non-mock-secret";
  ...
  "choices": [
    {
      ...
      "message": {
        "role": "assistant",
        "content": "loopback LM Studio response"
      },
      "finish_reason": "stop"
    }
  ],
...
async fn local_non_mock_chat_embeddings_models_and_health_use_openai_compat_loopback() {
    ...
    let chat = invoke(
        ...
        json!({
            ...
            "provider_extensions": {"fixture_case": "local_non_mock_chat"}
        }),
    )
    .await
    .expect("chat completions should invoke the production client path");
    assert_eq!(chat["content"], "loopback LM Studio response");
    ...
    assert_eq!(
        chat_body["fixture_case"], "local_non_mock_chat",
        "provider extensions should be flattened into the OpenAI-compatible request body"
    );
...
struct ConfiguredLmStudio {
    connector: LmStudioConnector,
    signing_key: Ed25519SigningKey,
}

async fn setup_connector(base_url: &str, capabilities: &[&'static str]) -> ConfiguredLmStudio {
    let signing_key = Ed25519SigningKey::generate();
    let verifying_key = signing_key.verifying_key();
    let mut connector = LmStudioConnector::new();
    connector
        ...
        .await
        .expect("configure LM Studio connector");
    connector
        ...
        .await
        .expect("handshake LM Studio connector");
...
fn signed_capability(
    ...
    let cose = CapabilityTokenBuilder::new()
        ...
        .zone_id("z:owner")
        .principal("user:lm-studio-local-non-mock")
        .operations(&[operation])
        .issuer("node:lm-studio-local-non-mock")
...
fn assert_request(captured: &CapturedRequest, request_line: &str) {
    ...
    assert!(
        ...
        "request should carry configured LM Studio bearer auth; head={}",
        captured.head
    );
    assert!(
    ...
    assert!(
        header_seen(&captured.head, "user-agent", "fcp-lm-studio/0.1.0"),
        "request should carry the LM Studio user agent; head={}",
        captured.head
    );
}
...
fn print_artifact(case_name: &str, details: &Value) {
    let artifact = json!({
        ...
        "bead_id": BEAD_ID,
        "case": case_name,
        "command": "cargo test -p fcp-lm-studio --test local_non_mock -- --nocapture",
