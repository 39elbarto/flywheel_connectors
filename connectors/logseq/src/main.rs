//! FCP `Logseq` Connector - Main entrypoint
//!
//! A `Logseq` PKM connector implementing the Flywheel Connector Protocol.
//! Provides access to pages and blocks in a Logseq graph.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Logseq Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut LogseqConnector, message: &str) -> serde_json::Value {
