...
- `e2ee.verified_decryption_requested=true` is accepted as an explicit readiness request. The connector now has trust-gated decrypted text and encrypted-media projection boundaries that can emit `matrix.message.decrypted` only from a verified crypto result after account identity, stable device ID, own-device trust, sender-device trust, cross-signing, room/session/event binding, room-key backup, recovery state, redaction, replay/duplicate, media encryption metadata, MXC URI, content hash, MIME, size limit, and resource-handle policy checks pass. Current live sync paths still pass no plaintext/media candidate until the audited Matrix crypto decrypt result is wired in, so they surface precise metadata denials instead of silently downgrading to unsafe decrypted, media, or ciphertext delivery.
...
- Use the E2EE decrypted-projection harness for JSONL evidence of verified plaintext projection, verified encrypted media handle-only resource exposure, redacted media denial, wrong room/event binding, media hash mismatch, backup/recovery mismatch, unavailable crypto structured skip, retry/final-failure state, operator readiness guidance, manual/supervised parity for text and media, event topics, resource cleanup, redaction-safe artifact paths, and shutdown:
...
- decrypted E2EE event or encrypted-media delivery from raw homeserver JSON, before account/device/sender-device/cross-signing/backup/recovery/redaction/replay trust is mechanically verified, or before media binding/hash/size/resource-policy checks pass
