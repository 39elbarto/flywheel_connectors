//! FCP `Metabase` Connector - Main entrypoint
//!
//! A `Metabase` BI connector implementing the Flywheel Connector Protocol.
//! Provides access to dashboards, questions, and query results.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Metabase Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut MetabaseConnector, message: &str) -> serde_json::Value {
