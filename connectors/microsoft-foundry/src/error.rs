...
#[derive(Debug, Error)]
pub enum MicrosoftFoundryError {
    #[error(transparent)]
    OpenAi(#[from] OpenAiError),
}

impl MicrosoftFoundryError {
...
pub fn openai_error_to_fcp(error: &OpenAiError) -> FcpError {
    match error {
        ...
        OpenAiError::PermissionDenied { message } => FcpError::CapabilityDenied {
            capability: "microsoft_foundry".into(),
        ...
        OpenAiError::ServiceUnavailable {
            ...
            service: "microsoft_foundry".into(),
            message: safe_provider_message(message),
        ...
        OpenAiError::Network(NetworkError::Cancelled { message }) => {
            FcpError::ConnectorUnavailable {
                code: 5000,
                message: format!("Microsoft Foundry request cancelled: {message}"),
            }
        }
        ...
        OpenAiError::Provider {
            ...
            message: format!("Microsoft Foundry provider returned HTTP {status}"),
