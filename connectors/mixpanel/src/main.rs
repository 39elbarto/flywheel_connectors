//! FCP Mixpanel Connector - Main entrypoint
//!
//! A Mixpanel product analytics connector implementing the Flywheel Connector Protocol.
//! Provides access to events, funnels, and insights.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Mixpanel Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
fn run_fcp_loop() -> Result<()> {
    ...
    let mut connector = MixpanelConnector::new();
...
async fn handle_message(connector: &mut MixpanelConnector, message: &str) -> serde_json::Value {
