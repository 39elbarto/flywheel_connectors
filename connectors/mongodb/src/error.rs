...
pub enum MongoDbError {
    ...
    /// `MongoDB` Atlas Data API returned an error
    #[error("MongoDB API error ({status_code}): {message}")]
...
impl ConnectorErrorMapping for MongoDbError {
...
mod tests {
    ...
    fn unauthorized_not_retryable() {
        assert!(!MongoDbError::Unauthorized.is_retryable());
    }

    #[test]
    fn forbidden_not_retryable() {
        assert!(!MongoDbError::Forbidden.is_retryable());
    }

    #[test]
    fn not_found_not_retryable() {
        assert!(
            !MongoDbError::NotFound {
    ...
    fn retry_after_for_rate_limited() {
        let err = MongoDbError::RateLimited {
    ...
    fn retry_after_none_for_unauthorized() {
        assert_eq!(MongoDbError::Unauthorized.retry_after(), None);
    }

    #[test]
    fn retry_after_none_for_forbidden() {
        assert_eq!(MongoDbError::Forbidden.retry_after(), None);
    }
    ...
    fn unauthorized_to_fcp_error() {
        match MongoDbError::Unauthorized.to_fcp_error() {
            FcpError::External {
                service,
                status_code,
                retryable,
    ...
    fn forbidden_to_fcp_error() {
        match MongoDbError::Forbidden.to_fcp_error() {
            FcpError::External {
                service,
                status_code,
                retryable,
    ...
    fn not_found_to_fcp_error() {
        match (MongoDbError::NotFound {
            resource: "collection_abc".into(),
        })
        .to_fcp_error()
        {
            FcpError::External {
                ...
                assert!(message.contains("collection_abc"));
                assert!(!retryable);
            }
            other => panic!("expected External, got {other:?}"),
        }
    ...
    fn rate_limited_to_fcp_error() {
        match (MongoDbError::RateLimited {
    ...
    fn json_error_to_fcp_internal() {
        ...
        match MongoDbError::Json(bad.unwrap_err()).to_fcp_error() {
    ...
    fn error_display_unauthorized() {
        assert_eq!(
            MongoDbError::Unauthorized.to_string(),
    ...
    fn error_display_forbidden() {
        assert_eq!(
            MongoDbError::Forbidden.to_string(),
    ...
    fn error_display_api() {
        assert_eq!(
            ...
            .to_string(),
            "MongoDB API error (500): Internal"
        );
    }
    ...
    fn retry_after_none_for_json_error() {
        ...
        assert_eq!(MongoDbError::Json(bad.unwrap_err()).retry_after(), None);
    }
    ...
    fn rate_limited_fcp_error_service_name() {
        ...
        .to_fcp_error()
        {
            FcpError::External { service, .. } => assert_eq!(service, "mongodb"),
            other => panic!("expected External, got {other:?}"),
        }
    }
    ...
    fn json_not_retryable() {
        let bad: Result<serde_json::Value, _> = serde_json::from_str("{bad");
        assert!(!MongoDbError::Json(bad.unwrap_err()).is_retryable());
    }

    #[test]
    fn error_debug_format_api() {
        let err = MongoDbError::Api {
            status_code: 503,
    ...
    fn error_debug_format_unauthorized() {
        let dbg = format!("{:?}", MongoDbError::Unauthorized);
        assert!(dbg.contains("Unauthorized"));
    }

    #[test]
    fn error_debug_format_forbidden() {
        let dbg = format!("{:?}", MongoDbError::Forbidden);
    ...
    fn error_debug_format_not_found() {
        let err = MongoDbError::NotFound {
            resource: "test_collection".into(),
        };
        let dbg = format!("{err:?}");
        assert!(dbg.contains("NotFound"));
        assert!(dbg.contains("test_collection"));
    }

    #[test]
    fn retry_after_zero_ms() {
        let err = MongoDbError::RateLimited { retry_after_ms: 0 };
    ...
    #[test]
    fn not_found_to_fcp_error_service_is_mongodb() {
        match (MongoDbError::NotFound {
            resource: "x".into(),
        })
        .to_fcp_error()
    ...
    fn error_display_json() {
        ...
        let err = MongoDbError::Json(bad.unwrap_err());
