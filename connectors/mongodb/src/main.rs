//! FCP `MongoDB` Connector - Main entrypoint
//!
//! A `MongoDB` Atlas Data API connector implementing the Flywheel Connector Protocol.
//! Provides access to `MongoDB` databases, collections, and documents via the Atlas Data API.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP MongoDB Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut MongoDbConnector, message: &str) -> serde_json::Value {
