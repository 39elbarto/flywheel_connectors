...
pub const USER_AGENT: &str = "fcp-moonshot/0.1.0";
...
impl MoonshotProvider {
    pub fn new(base_url: impl Into<String>, auth: MoonshotAuth) -> Self {
        Self {
    ...
    #[must_use]
    pub const fn auth(&self) -> &MoonshotAuth {
        &self.auth
    }
}

impl OpenAiCompatProvider for MoonshotProvider {
    ...
    fn auth_header(&self, req: &mut HttpRequest) {
        match &self.auth {
            MoonshotAuth::ApiKey(key) => req.bearer_auth(key),
            MoonshotAuth::CredentialId(id) => req.upsert_header("x-fcp-credential-id", id),
        }
    }
...
pub struct MoonshotClient {
    inner: OpenAiCompatClient<MoonshotProvider>,
}
...
impl MoonshotClient {
    ...
    #[must_use]
    pub const fn provider(&self) -> &MoonshotProvider {
        self.inner.provider()
    }
...
pub fn normalize_moonshot_base_url(raw: Option<&str>) -> Result<String, String> {
    ...
    if !allowed_host || !valid_scheme || path != "/v1" {
        return Err(format!(
            "base_url must be https://api.moonshot.ai/v1 or https://api.moonshot.cn/v1 (localhost/127.0.0.1/::1 allowed over http/https for tests): {value}"
        ));
    }
