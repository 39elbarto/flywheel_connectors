...
This document fixes the operator-facing contract for `fcp.n8n`. The connector exposes bounded workflow, project, tag, execution, credential-metadata, and n8n 2.19+ folder reads, plus guarded draft creation/update and typed publish/unpublish lifecycle writes. Lifecycle requests validate exact target, UUID idempotency, full state precondition, and current-chat approval, then use only the exact official MCP `publish_workflow`/`unpublish_workflow` tools with an independent REST GET readback; direct REST lifecycle remains an explicit connector-only route and no legacy endpoint is guessed.
...
- Deterministic lifecycle failures proven to occur before the official MCP side effect are exposed only as bounded categories such as `official_mcp_policy_failed` or `official_mcp_capability_failed`. Invocation, timeout, teardown, malformed/ambiguous response, and all other cases that could conceal a provider-side effect remain `unknown_outcome` and are never retried.
