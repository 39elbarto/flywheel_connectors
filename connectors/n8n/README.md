...
> **Current host evidence (2026-09-12, read-only)**: `/usr/local/lib/fwc-n8n/current` points to `release-20260912-cddd9f168-rc14` (git revision `cddd9f1685b99393b81aa054c2e578e9d5e36b58`), and the installed binary reports `{"bundleAvailable":true}`. The fixed tree contains the release-bound provenance and signed provision receipt. Their presence does **not** by itself prove live provider lifecycle acceptance: the latest RC20 and RC21 gates remain redaction-safe bounded NO-GO before a complete EEC and Hetzner publish/readback/unpublish/final-readback cycle.
>
> **Source candidate (`flywheel_connectors-nqm81.30`)**: Targets the installed local package `2.82.1`, with an isolated, network-disabled catalog check and exact input-schema pins. Runtime and assembler use neutral client names: n8n-mcp rewrites schemas for names containing `n8n` or `langchain`. Reviewed Hetzner lifecycle pins and strict response decoding are updated; old lifecycle pins are admitted only for signed predecessor validation and rollback, never for a new staged candidate. Source `main` is newer than the installed revision; these changes are not installed or live-accepted yet. No global package change or release switch is claimed.
