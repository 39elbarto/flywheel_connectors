...
> **Current installed release/read-only smoke boundary (2026-09-18)**: `/usr/local/lib/fwc-n8n/current` resolves to the verified installed release `release-20260917-74b9b97c0-rc2` (git revision `74b9b97c0e63cf5c4336becd4a9c393ab4c82e14`), and `/usr/local/bin/fwc-n8n status` reports `{"bundleAvailable":true}`. The fixed zero-idle systemd broker is installed and listening on `/run/fwc/fwc-n8n-secret-broker.sock`; the documented read-only smoke boundary is `crates/fcp-host/tests/n8n_owned_static_smoke.rs`. The installed local MCP policy remains pinned to `n8n-mcp` `2.84.4`, protocol `2024-11-05`, and network-disabled execution. These checks establish installed bundle/broker readiness only; they do not by themselves establish or authorize lifecycle writes.
> **Current source/main static evidence (2026-09-18; not live evidence)**: The current `main` tree is `74b9b97c0e63cf5c4336becd4a9c393ab4c82e14`. The lifecycle boundary retains the one-official-MCP-attempt/one-independent-REST-readback/no-retry rule, including terminal `unknown_outcome` handling. Focused verification passed `21` `official_mcp_lifecycle` tests, the targeted `fcp-host` filters exited successfully, and `cargo fmt --check` was clean. These checks complement the retained live acceptance above and do not extend it to archive/restore, execution, credential mutation, or permanent deletion.
