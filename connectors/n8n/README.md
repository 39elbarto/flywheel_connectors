...
> **Newer source/main static evidence (2026-09-15; not installed-release evidence)**: Source commits `a3e0b557d` and `2ed780f42` are merged as `61dc1729c`. The lifecycle boundary sets `LIFECYCLE_READBACK_RESERVE=15,200ms`; for request deadline `D`, the sole side-effecting Official MCP attempt receives the absolute write deadline `D-R`, followed by at most one independent REST readback on the original `D`. Known pre-provider failures return without a readback, and no automatic provider retry occurs. Archive provider advisory, malformed, and unknown outcomes are reconciled by one readback; the readback is authoritative for the verified state. Source/static checks only recorded `172 passed, 1 ignored`, with `cargo check`, `cargo fmt --check`, and `git diff --check` clean. These checks do not update the installed release or prove real process-boundary/provider behavior: the installed release has not received this source patch, and mock/injected bridge tests do not prove that behavior, so live acceptance remains **NO-GO**.
