...
- The crate also builds the operator wrapper `fwc-n8n`. Its current commands are
  ...
  The bounded typed lifecycle/archive, draft-create, and disposable-delete
...
- `n8n.workflows.unarchive` accepts only an exact archived workflow target with the full current state precondition and external signed approval. It performs exactly one typed REST `POST /api/v1/workflows/{workflowId}/unarchive` with no request body, using the existing host credential/egress path and upstream `workflow:delete` scope, then one independent REST `GET /workflows/{id}` readback requiring `isArchived=false` while preserving workflow identity, draft/published summaries, version, active flag, and active version. It never publishes, activates, archives, deletes, calls MCP, uses a generic REST fallback, or restores a version; timeout, disconnect, conflict, malformed/ambiguous response, or readback uncertainty is unknown and never retried.
