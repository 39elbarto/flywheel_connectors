...
> **Historical host snapshot (2026-09-07, read-only)**: `/usr/local/lib/fwc-n8n/current` pointed to `release-20260904-9fccc3c46` (git revision `9fccc3c46b3736444e24865b6c3c6c66fefa57da`). That snapshot and its local `n8n-mcp` package claim are retained as historical evidence only; they do **not** describe the current installed release or prove cryptographic receipt verification or live provider acceptance.
> **Historical host evidence (2026-09-12, read-only)**: `/usr/local/lib/fwc-n8n/current` pointed to `release-20260912-cddd9f168-rc14` (git revision `cddd9f1685b99393b81aa054c2e578e9d5e36b58`), and the installed binary reported `{"bundleAvailable":true}`. This remains historical evidence only.
> **Historical host evidence (2026-09-14, read-only)**: `/usr/local/lib/fwc-n8n/current` pointed to `release-20260914-2855e4545-rc1` (git revision `2855e4545efbec41ec5c662c7314fcc8561729eb`), and the installed binary reported `{"bundleAvailable":true}`. The fixed tree contained the release-bound provenance and signed Ed25519 provision receipt; the local MCP policy pinned `n8n-mcp` `2.84.4`, protocol `2024-11-05`, and network-disabled execution.
> **Historical bounded EEC evidence (2026-09-14)**: Through the installed `/usr/local/bin/fwc-n8n` wrapper, the target workflow baseline was read, one owner-approved `n8n.mcp_access.reconcile` apply restored `availableInMCP=false`, the ledger committed an `applied` receipt, and one independent GET confirmed the exact version, graph, inactive, unpublished, and unarchived state. This proves the bounded MCP-access path for that RC only; publish/unpublish, execution, and the complete two-server EEC-then-Hetzner lifecycle remain unaccepted.
>
> **Historical candidate (`flywheel_connectors-nqm81.30`, 2026-09-14)**: The installed local package was pinned to `2.84.4`, with an isolated, network-disabled catalog check and exact input-schema pins. Runtime and assembler used neutral client names: n8n-mcp rewrote schemas for names containing `n8n` or `langchain`. Reviewed Hetzner lifecycle pins and strict response decoding were updated; old lifecycle pins were admitted only for signed predecessor validation and rollback, never for a new staged candidate. That candidate was installed and had bounded EEC MCP-access evidence, but no full two-server lifecycle acceptance was claimed. No global package change was made.
> **Historical installed release/read-only smoke boundary (2026-09-18)**: `/usr/local/lib/fwc-n8n/current` then resolved to the verified installed release `release-20260917-74b9b97c0-rc2` (git revision `74b9b97c0e63cf5c4336becd4a9c393ab4c82e14`), and `/usr/local/bin/fwc-n8n status` reported `{"bundleAvailable":true}`. The fixed zero-idle systemd broker was installed and listening on `/run/fwc/fwc-n8n-secret-broker.sock`; the documented read-only smoke boundary is `crates/fcp-host/tests/n8n_owned_static_smoke.rs`. The installed local MCP policy was pinned to `n8n-mcp` `2.84.4`, protocol `2024-11-05`, and network-disabled execution. These historical checks established installed bundle/broker readiness only; they did not by themselves establish or authorize lifecycle writes.
> **Historical source/main static evidence (2026-09-18; not live evidence)**: The `main` tree at that time was `74b9b97c0e63cf5c4336becd4a9c393ab4c82e14`. The lifecycle boundary retained the one-official-MCP-attempt/one-independent-REST-readback/no-retry rule, including terminal `unknown_outcome` handling. Focused verification passed `21` `official_mcp_lifecycle` tests, the targeted `fcp-host` filters exited successfully, and `cargo fmt --check` was clean. These checks complemented the retained publish/unpublish acceptance and did not extend it to archive/restore, execution, credential mutation, or permanent deletion.
> **Historical bounded Hetzner lifecycle evidence (2026-09-17; PASS)**: On the installed release `release-20260916-5dfb5745-owner-rc2`, a fresh disposable webhook workflow `Cb7jfMXeDxyHytnY` (version `2dbce392-dcbd-4b8f-b2bf-da39e3c2b71c`) passed the exact inactive/unpublished/unarchived baseline, one owner-approved official-MCP `publish_workflow` wrapper invocation, and an independent REST GET proving `active=true`, matching active version, and matching draft/published graph. After that proof, one owner-approved `unpublish_workflow` wrapper invocation and an independent REST GET proved `active=false`, `activeVersionId=null`, `isArchived=false`, `published=null`, and the unchanged draft graph. The retained request metadata binds the publish transition to approval ref `nqm81.10-hetzner-publish-89fb4d0b-5c9a-43ce-92d3-62e7861ba38c`, wrapper correlation `09e16ee0-8500-4b80-97d0-ca9d9cec5dee`, and independent GET correlation `4e7fbd86-1f5c-4f59-a22e-e96b90ef50ca`; it binds the unpublish transition to approval ref `nqm81.10-hetzner-unpublish-de91b7a4-6c32-4f85-9a17-2e5d8c3b704f`, wrapper correlation `0a5fd2c9-8e71-4b34-a6d3-1f7c90e528bd`, and independent GET correlation `7c1e9a52-4d68-4fb0-b327-8a6c1d5e904f`. The publish safe projection has a reporting defect (`status=verified` together with `response_class=error`), and its GET represented `isArchived=null`; this is not a clean envelope and `null` is not interpreted as `false`. Exact provider-attempt counts must come from retained claim/receipt records, not from these projections or exit codes. Redaction-safe evidence is retained under `/srv/dev-ssd/fcp/nqm81.30/live-hetzner-create-20260917/` and `/srv/dev-ssd/fcp/nqm81.30/live-hetzner-publish-20260917/`; raw provider bodies, credentials, seeds, and tokens were not persisted. The remote Hetzner runtime reports `2.37.10` while the release policy remains pinned to `2.34.6`; this observed version drift was not changed because it is not a lifecycle-equality gate. At the time of this packet it proved only the Hetzner half; the later EEC-first pass is recorded in the current acceptance block above.
...
This document fixes the operator-facing contract for `fcp.n8n`. The connector exposes bounded workflow, project, tag, execution, credential-metadata, and n8n 2.19+ folder reads, plus guarded draft creation/update, typed official-MCP publish/unpublish/archive writes, a typed REST unarchive path, and a narrowly scoped disposable-workflow REST delete. Execution reads include the unchanged metadata-only `n8n.executions.get` contract and the separately named `n8n.executions.diagnostics` projection for bounded node errors. The `n8n.workflows.execute` seam validates its bounded manual/production contract and is admitted only by the immutable owner-provisioned EEC/Hetzner `execute_workflow` schema bindings; no live execution acceptance is claimed. Each enabled write validates exact target, UUID idempotency, full state precondition, and current-chat approval, then uses only its exact owner-approved provider path with an independent typed readback; the exact typed REST unarchive route is separately guarded, while other direct REST lifecycle/archive/execution routes remain fail-closed and no legacy endpoint is guessed. Disposable delete additionally requires a host-issued receipt proving the same workflow was created through the bounded draft path.
...
- `n8n.workflows.unarchive` (typed REST `POST /api/v1/workflows/{workflowId}/unarchive` with no body and upstream `workflow:delete` scope; archived baseline only, one guarded provider attempt plus independent readback preserving workflow identity, draft graph/published summary/active state while allowing n8n to rotate the draft version identifier)
- `n8n.workflows.delete_disposable` (typed REST `DELETE /workflows/{id}` only for a host-receipted disposable draft; one guarded provider attempt plus independent `GET` requiring 404)
...
- **Future/unimplemented:** `n8n.runtime.status`,
  ...
  unimplemented execution paths. Version restore, versions, credential
  mutation, permanent/general deletion, and provider activation remain
  fail-closed.
...
- The crate also builds the operator wrapper `fwc-n8n`. Its current commands are
  ...
  The bounded typed lifecycle/archive/unarchive, draft-create, and disposable-delete
  owner-confirmation seam in `fcp-host` computes a redaction-safe plan digest
  and exposes only a fail-closed issuer
  until a trusted host adapter is installed. It is not a second ledger and is
  not an independent provider gate. Approved plans must reuse the existing
  `ApprovalToken` canonicalization/verification contract and the existing host
  run-once path, which remains authoritative for full
  resource/workflow/input/precondition/idempotency binding, one-use claim,
  provider-attempt receipts, `unknown` recovery, and no automatic retry. A
  future trusted host/KeePass adapter must wire confirmation into that path
  before lifecycle writes are enabled.
  Current/release/systemd and fallback MCP profiles are unchanged.
  ...
  scope.
- Official MCP publish/unpublish is live-accepted only for the bounded
  `flywheel_connectors-nqm81.10` disposable cycles recorded above. The earlier
  2026-08-21 EEC attempt that returned `unknown_outcome` remains historical
  evidence: its independent readback stayed inactive, no retry was made, and
  the disposable workflow was later removed after MCP access was disabled and
  DELETE/404 readback verification.
...
- `n8n.workflows.lifecycle` accepts only `publish` and `unpublish`, requires an exact workflow target, UUID idempotency key, an explicit `versionId` when the caller pins a version, the `activeVersionId`/`active`/`isArchived`/`stateDigest` precondition, and one matching interactive approval. The host builds only the exact official MCP tool arguments after fresh `tools/list` policy verification; one side-effecting MCP call is followed by exactly one independent REST `GET /workflows/{id}` readback, including when the provider attempt has an ambiguous or unknown outcome. The provider response and envelope are bounded advisory data only: opaque, version-specific, or provider-rejection fields neither declare success nor override or block the readback when the outer bridge response permits reconciliation. Publish proves active state, an explicit version or an unambiguous readback-selected version, published graph consistency, draft preservation, and `isArchived=false`; unpublish proves inactive/null active version, archive preservation, and draft preservation from that readback. Exact independent readback is the sole lifecycle success proof; readback uncertainty or mismatch remains non-success and is never retried automatically. Known failures proven before the provider side effect remain bounded preflight failures. Other direct REST lifecycle routes remain fail-closed unless their exact route is separately proven.
- `n8n.workflows.archive` accepts only an exact inactive, unarchived workflow target with the full lifecycle precondition and external signed approval. It calls only the owner-policy-bound official MCP `archive_workflow` tool once, then performs an independent REST `GET /workflows/{id}` readback requiring `isArchived=true`, inactive/null active version, and unchanged draft/published summaries. The provider's required name field is consumed but never returned. Active or already-archived baselines, timeout, disconnect, conflict, malformed/ambiguous response, or readback uncertainty fail closed.
- `n8n.workflows.unarchive` accepts only an exact archived workflow target with the full current state precondition and external signed approval. It performs exactly one typed REST `POST /api/v1/workflows/{workflowId}/unarchive` with no request body, using the existing host credential/egress path and upstream `workflow:delete` scope, then one independent REST `GET /workflows/{id}` readback requiring `isArchived=false` while preserving workflow identity, the draft graph digest, the published summary, the active flag, and the active version. n8n may issue a fresh draft version identifier during unarchive; the resulting version and derived state digest are therefore not required to equal the archived baseline. The fwc-n8n envelope deadline is carried into fcp-host, whose trusted per-invocation launch snapshot derives the fixed `FCP_N8N_UNARCHIVE_REQUEST_TIMEOUT_MS` value for the owned connector `ProcessSpec`; managed configuration and operation input cannot override or supply it. This reserves a reconciliation tail across the baseline GET, one POST, and final GET, so a stalled POST cannot consume the GET opportunity. Any post-attempt timeout, disconnect, HTTP error, malformed/advisory response, provider ID mismatch, or normalization error still receives exactly one GET; a matching readback succeeds even when the provider result is contradictory, while mismatch/readback failure is unknown and never retried. It never publishes, activates, archives, deletes, calls MCP, uses a generic REST fallback, or restores a version. Bounded live acceptance passed on EEC and Hetzner in `flywheel_connectors-nqm81.23` (2026-09-21), with archived-to-unarchived readback preserving the draft graph and inactive/unpublished/null-active-version state. `restore_workflow_version` is separate, and this result does not repair or accept MCP archive.
- The offline `fcp-host` approval seam covers typed lifecycle/archive/unarchive, `n8n.workflows.create_draft`, and `n8n.workflows.delete_disposable` operations and exact `eec`/`hetzner` targets. Direct REST approvals bind the exact server, canonical resource URI, complete input, precondition, UUID idempotency key, and short expiry through the existing `fcp.n8n` request hash; disposable deletion additionally retains the host-issued creation receipt precondition. It computes only a redaction-safe plan digest and has no private-key, ledger, provider-call, or secret-get path. The existing host run-once ledger remains the sole durable replay/unknown boundary; a future trusted host/KeePass adapter must implement the issuer and wire it into that path.
- The official-MCP provider response is a bounded advisory signal, not a lifecycle state authority. Its typed fields may be absent, opaque, version-specific, or contradictory; provider rejection markers are retained only as fixed redaction-safe diagnostics when the readback cannot prove the transition. The independent REST readback alone must prove the requested lifecycle state, preserve the baseline draft/archive invariants, and select a version only when an omitted publish version is unambiguous; provider fields never override it. Every possible provider attempt, including an ambiguous or `unknown_outcome` attempt, gets exactly one reconciliation readback. Known pre-provider failures remain deterministic, while readback uncertainty or mismatch is non-success and is never retried.
- Deterministic lifecycle failures proven to occur before the official MCP side effect are exposed only as bounded categories such as `official_mcp_policy_failed` or `official_mcp_capability_failed` and return without reconciliation. Invocation, timeout, teardown, malformed/ambiguous response, and all other cases that could conceal a provider-side effect receive exactly one independent readback; an exact readback may prove success, otherwise the result remains `unknown_outcome` and is never retried. When the supervised child provides one of its fixed redaction-safe invoke classes, the error envelope may additionally contain only that allowlisted `diagnostic` label; it never contains provider text, payloads, headers, credentials, or changes the retry decision.
...
| `n8n.workflows.unarchive` | baseline `GET`, one no-body `POST /api/v1/workflows/{workflowId}/unarchive`, then independent `GET` | `n8n.workflows.lifecycle` | `Risky` | `High` | `BestEffort` | id, archived full precondition, exact approval reference, UUID idempotency key; upstream `workflow:delete` scope |
| `n8n.workflows.delete_disposable` | baseline `GET`, one `DELETE /workflows/{id}`, then independent `GET` requiring 404 | `n8n.workflows.write` | `Risky` | `High` | `BestEffort` | id, host-issued `creationReceipt`, inactive/unarchived full precondition, exact approval reference, UUID idempotency key |
...
- version restore, versions, test/prepare-test execution, credential mutation, and permanent/general deletion; raw execution data/result retrieval and execution management remain out of scope (diagnostics returns only its bounded normalized error projection)
