...
> **Current installed release/read-only smoke boundary (2026-09-15)**: `/usr/local/lib/fwc-n8n/current` resolves to the verified installed release `release-20260915-15e6d63e3-invoke-unknown` (git revision `15e6d63e3316b1764442fee022a23e8a4b19cb34`), and `/usr/local/bin/fwc-n8n status` reports `{"bundleAvailable":true}`. The fixed zero-idle systemd broker is installed and listening on `/run/fwc/fwc-n8n-secret-broker.sock`; the documented read-only smoke boundary is `crates/fcp-host/tests/n8n_owned_static_smoke.rs`. The installed local MCP policy remains pinned to `n8n-mcp` `2.84.4`, protocol `2024-11-05`, and network-disabled execution. These checks establish installed bundle/broker readiness only; no lifecycle writes are claimed, and publish/unpublish, execution, and the complete EEC-then-Hetzner lifecycle remain unaccepted/NO-GO.
> **Newer source/main static evidence (2026-09-15; not installed-release evidence)**: Source commits `a3e0b557d` and `2ed780f42` are merged as `61dc1729c`; archive error normalization is in `cf1f6b8de`, and malformed/invalid readback regression coverage is in `57a445745` (both included in current `main`). The lifecycle boundary sets `LIFECYCLE_READBACK_RESERVE=15,200ms`; for request deadline `D`, the sole side-effecting Official MCP archive attempt receives the absolute write deadline `D-R`, followed by at most one independent REST readback on the original `D`. After a possible archive attempt, readback transport, decode, invalid-state, and mismatch errors are terminal `unknown_outcome`; the provider diagnostic/correlation is retained first, with readback diagnostic/correlation as fallback. Exactly one Official MCP attempt and at most one REST readback remain, with no provider retry. Focused archive coverage recorded `9 passed`; the full `fwc-n8n` static suite recorded `176 passed, 1 ignored`, with `cargo check`, `cargo fmt --check`, and `git diff --check` clean. These checks do not update the installed release or prove real process-boundary/provider behavior: installed `release-20260915-15e6d63e3-invoke-unknown` does not contain these source patches, and mock/injected bridge tests do not prove that behavior, so live acceptance remains **NO-GO**.
