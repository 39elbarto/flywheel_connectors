...
- `n8n.workflows.unarchive` accepts only an exact archived workflow target with the full current state precondition and external signed approval. It performs exactly one typed REST `POST /api/v1/workflows/{workflowId}/unarchive` with no request body, using the existing host credential/egress path and upstream `workflow:delete` scope, then one independent REST `GET /workflows/{id}` readback requiring `isArchived=false` while preserving workflow identity, draft/published summaries, version, active flag, and active version. The host/run-once seam propagates a bounded per-request deadline budget across this three-request sequence and reserves a reconciliation tail, so a stalled POST cannot consume the GET opportunity. Any post-attempt timeout, disconnect, HTTP error, malformed/advisory response, provider ID mismatch, or normalization error still receives exactly one GET; mismatch/readback failure is unknown and never retried. It never publishes, activates, archives, deletes, calls MCP, uses a generic REST fallback, or restores a version.
