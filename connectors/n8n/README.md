...
> **Current installed release/read-only smoke boundary (2026-09-16)**: `/usr/local/lib/fwc-n8n/current` resolves to the verified installed release `release-20260916-5dfb5745-owner-rc2` (git revision `5dfb574531982e353d6dfcbcf2e51f7882cd4099`), and `/usr/local/bin/fwc-n8n status` reports `{"bundleAvailable":true}`. The fixed zero-idle systemd broker is installed and listening on `/run/fwc/fwc-n8n-secret-broker.sock`; the documented read-only smoke boundary is `crates/fcp-host/tests/n8n_owned_static_smoke.rs`. The installed local MCP policy remains pinned to `n8n-mcp` `2.84.4`, protocol `2024-11-05`, and network-disabled execution. These checks establish installed bundle/broker readiness; the complete EEC-then-Hetzner lifecycle remains unaccepted/NO-GO even though the bounded Hetzner lifecycle half is now proven below.
> **Newer source/main static evidence (2026-09-15; not installed-release evidence)**: Source commits `a3e0b557d` and `2ed780f42` are merged as `61dc1729c`; archive error normalization is in `cf1f6b8de`, and malformed/invalid readback regression coverage is in `57a445745` (both included in the current `main` tree). The lifecycle boundary sets `LIFECYCLE_READBACK_RESERVE=15,200ms`; for request deadline `D`, the sole side-effecting Official MCP archive attempt receives the absolute write deadline `D-R`, followed by at most one independent REST readback on the original `D`. After a possible archive attempt, readback transport, decode, invalid-state, and mismatch errors are terminal `unknown_outcome`; the provider diagnostic/correlation is retained first, with readback diagnostic/correlation as fallback. Exactly one Official MCP attempt and at most one REST readback remain, with no provider retry. Focused archive coverage recorded `9 passed`; the full `fwc-n8n` static suite recorded `176 passed, 1 ignored`, with `cargo check`, `cargo fmt --check`, and `git diff --check` clean. These checks do not update the installed release or prove real process-boundary/provider behavior. The installed release above is the code ancestor of the current `main` tree, while the complete EEC-then-Hetzner live acceptance remains **NO-GO**.
> **Latest bounded Hetzner lifecycle evidence (2026-09-17; partial GO)**: On the installed release `release-20260916-5dfb5745-owner-rc2`, a fresh disposable webhook workflow `Cb7jfMXeDxyHytnY` (version `2dbce392-dcbd-4b8f-b2bf-da39e3c2b71c`) passed the exact inactive/unpublished/unarchived baseline, one owner-approved official-MCP `publish_workflow` wrapper invocation, and an independent REST GET proving `active=true`, matching active version, and matching draft/published graph. After that proof, one owner-approved `unpublish_workflow` wrapper invocation and an independent REST GET proved `active=false`, `activeVersionId=null`, `isArchived=false`, `published=null`, and the unchanged draft graph. The retained request metadata binds the publish transition to approval ref `nqm81.10-hetzner-publish-89fb4d0b-5c9a-43ce-92d3-62e7861ba38c`, wrapper correlation `09e16ee0-8500-4b80-97d0-ca9d9cec5dee`, and independent GET correlation `4e7fbd86-1f5c-4f59-a22e-e96b90ef50ca`; it binds the unpublish transition to approval ref `nqm81.10-hetzner-unpublish-de91b7a4-6c32-4f85-9a17-2e5d8c3b704f`, wrapper correlation `0a5fd2c9-8e71-4b34-a6d3-1f7c90e528bd`, and independent GET correlation `7c1e9a52-4d68-4fb0-b327-8a6c1d5e904f`. The publish safe projection has a reporting defect (`status=verified` together with `response_class=error`), and its GET represented `isArchived=null`; this is not a clean envelope and `null` is not interpreted as `false`. Exact provider-attempt counts must come from retained claim/receipt records, not from these projections or exit codes. Redaction-safe evidence is retained under `/srv/dev-ssd/fcp/nqm81.30/live-hetzner-create-20260917/` and `/srv/dev-ssd/fcp/nqm81.30/live-hetzner-publish-20260917/`; raw provider bodies, credentials, seeds, and tokens were not persisted. The remote Hetzner runtime reports `2.37.10` while the release policy remains pinned to `2.34.6`; this observed version drift was not changed because it is not a lifecycle-equality gate. This proves only the Hetzner half; the required EEC-first cycle remains **NO-GO**.
...
- The offline `fcp-host` approval seam is limited to the typed lifecycle/archive, `n8n.workflows.create_draft`, and `n8n.workflows.delete_disposable` operations and exact `eec`/`hetzner` targets. Direct REST approvals bind the exact server, canonical resource URI, complete input, precondition, UUID idempotency key, and short expiry through the existing `fcp.n8n` request hash; disposable deletion additionally retains the host-issued creation receipt precondition. It computes only a redaction-safe plan digest and has no private-key, ledger, provider-call, or secret-get path. The existing host run-once ledger remains the sole durable replay/unknown boundary; a future trusted host/KeePass adapter must implement the issuer and wire it into that path.
