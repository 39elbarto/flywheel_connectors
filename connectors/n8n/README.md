...
> **Current host evidence (2026-09-14, read-only)**: `/usr/local/lib/fwc-n8n/current` points to `release-20260914-2855e4545-rc1` (git revision `2855e4545efbec41ec5c662c7314fcc8561729eb`), and the installed binary reports `{"bundleAvailable":true}`. The fixed tree contains the release-bound provenance and signed Ed25519 provision receipt; the local MCP policy pins `n8n-mcp` `2.84.4`, protocol `2024-11-05`, and network-disabled execution.
> **Current bounded EEC evidence (2026-09-14)**: Through the installed `/usr/local/bin/fwc-n8n` wrapper, the target workflow baseline was read, one owner-approved `n8n.mcp_access.reconcile` apply restored `availableInMCP=false`, the ledger committed an `applied` receipt, and one independent GET confirmed the exact version, graph, inactive, unpublished, and unarchived state. This proves the bounded MCP-access path for the current RC only; publish/unpublish, execution, and the complete two-server EEC-then-Hetzner lifecycle remain unaccepted.
>
> **Current candidate (`flywheel_connectors-nqm81.30`)**: The installed local package is pinned to `2.84.4`, with an isolated, network-disabled catalog check and exact input-schema pins. Runtime and assembler use neutral client names: n8n-mcp rewrites schemas for names containing `n8n` or `langchain`. Reviewed Hetzner lifecycle pins and strict response decoding are updated; old lifecycle pins are admitted only for signed predecessor validation and rollback, never for a new staged candidate. The candidate is installed and has bounded EEC MCP-access evidence, but no full current-release two-server lifecycle acceptance is claimed. No global package change was made.
