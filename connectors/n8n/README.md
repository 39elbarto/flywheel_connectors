...
- The crate also builds the operator wrapper `fwc-n8n`. Its current commands are
  ...
  The bounded typed lifecycle/archive owner-confirmation seam in `fcp-host`
  computes a redaction-safe plan digest and exposes only a fail-closed issuer
...
- The offline `fcp-host` approval seam is limited to these three typed lifecycle/archive operations and exact `eec`/`hetzner` targets. It computes only a redaction-safe plan digest and has no private-key, ledger, provider-call, or secret-get path. The existing host run-once ledger remains the sole durable replay/unknown boundary; a future trusted host/KeePass adapter must implement the issuer and wire it into that path.
