...
- **Manifest operations:** the 19 operations listed above are declared by
  `connectors/n8n/manifest.toml`; activation's typed REST provider path is
  implemented in source but remains live-unaccepted.
...
- **Future/unimplemented:** `n8n.runtime.status`,
  ...
  mutation, and permanent/general deletion remain fail-closed.
...
- Runtime capability surface:
  ...
  - `n8n.workflows.write` gates the activation approval boundary; a valid EEC/Hetzner request then uses only the guarded typed REST publish/unpublish path and its independent readback.
...
- `n8n.workflows.activate` is a guarded typed REST operation limited to EEC and Hetzner. It performs one baseline `GET`, exactly one `POST /workflows/{id}/publish` or `/unpublish`, and one independent `GET`; active=true proves active plus matching published version/graph, active=false proves inactive with null activeVersionId and null published, and both preserve workflow identity, draft graph, and archive state. The host fixes the REST-API credential purpose, applies the 1 MiB response policy, and derives a bounded timeout with reconciliation tail; no automatic retry is allowed. Source and offline tests are implemented, but live acceptance remains unclaimed.
...
| `n8n.workflows.activate` | baseline `GET`, exactly one `POST /workflows/{id}/publish` or `/unpublish`, independent `GET` | `n8n.workflows.write` | `Risky` | `High` | `BestEffort` | `id`, `active`, publish `versionId?`, full lifecycle precondition, UUID idempotency key, and one matching approval |
...
- live activation acceptance and release promotion; the guarded typed REST source path is implemented and remains unaccepted until the bounded EEC-then-Hetzner procedure passes
...
- all read operations through deterministic HTTP fixtures, including hostile-field redaction, bounded query encoding, cursor validation/preservation, strict workflow publication-state invariants, graph/state digest separation, and null required-key assertions, plus activation route, timeout, stale-precondition, malformed-response, readback-mismatch, and no-retry coverage
