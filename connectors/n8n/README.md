...
- `n8n.workflows.unarchive` (typed REST `POST /api/v1/workflows/{workflowId}/unarchive` with no body and upstream `workflow:delete` scope; archived baseline only, one guarded provider attempt plus independent readback preserving workflow identity, draft/published/version/active state)
