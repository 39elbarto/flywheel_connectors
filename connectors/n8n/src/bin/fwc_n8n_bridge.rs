...
fn process_spec(
    ...
    let owner_admission = match envelope.operation {
        ...
        super::HostRunOnceOperation::WorkflowsUnarchive => {
            Some(WORKFLOWS_UNARCHIVE_OWNER_ADMISSION)
        }
        super::HostRunOnceOperation::WorkflowsLifecycle => {
...
mod tests {
    ...
    fn production_spec_has_only_fixed_host_controls() {
        ...
        .expect("workflow archive write spec");
        assert_eq!(
            archive.fixed_args,
            vec![OsString::from("n8n-official-mcp-run-once-supervised")]
        );
        ...
        assert_eq!(archive_admission["mode"], "owner-approved-single-host");
