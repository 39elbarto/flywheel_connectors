...
impl ProvisionRequest {
    ...
    pub fn validate(self) -> Result<InstallPlan, ProvisionError> {
        ...
        let previous_release =
            validate_current_pointer(&self.current_path, &self.releases_root, self.expected_owner)?;
        let release_path = self.releases_root.join(&self.release_id);
        reject_existing_path(&release_path)?;
        validate_stage_tree(&self)?;
        Ok(InstallPlan {
...
/// A verified plan for a privileged installer.  There is intentionally no
...
fn validate_stage_tree(request: &ProvisionRequest) -> Result<(), ProvisionError> {
    #[cfg(not(unix))]
    {
        let _ = request;
    ...
    impl Fixture {
        ...
        fn provision_receipt_for(&self, root: &Path, release_id: &str) -> Vec<u8> {
            ...
            serde_json::to_vec(&serde_json::json!({
                ...
                "bindings": [
                    {"server":"eec","archive_input_schema_digest":"sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","archive_output_schema_digest":"sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"},
                    {"server":"hetzner","archive_input_schema_digest":"sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","archive_output_schema_digest":"sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd"}
                ],
                "artifacts": artifacts
            }))
        ...
        fn set_previous_git_revision(&self) {
            ...
            let mut provision: Value = serde_json::from_slice(
                &fs::read(previous.join(PROVISION_RECEIPT_FILE)).expect("previous provision"),
            )
            .expect("previous provision json");
            provision["git_revision"] = Value::String(revision.to_owned());
            let digest = hash_file(&previous.join(PROVENANCE_FILE)).expect("provenance digest");
            if let Some(artifact) = provision["artifacts"].as_array_mut().and_then(|artifacts| {
                artifacts.iter_mut().find(|artifact| {
                    artifact.get("path").and_then(Value::as_str) == Some(PROVENANCE_FILE)
                })
            }) {
            ...
            fs::write(
                previous.join(PROVISION_RECEIPT_FILE),
                serde_json::to_vec(&provision).expect("previous provision bytes"),
            )
        ...
        fn request(&self) -> ProvisionRequest {
            ...
            ], self.owner, self.current.clone(), self.releases.clone())
        }
    }
