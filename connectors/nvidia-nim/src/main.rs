...
use fcp_nvidia_nim::NvidiaNimConnector;
...
async fn handle_message(connector: &mut NvidiaNimConnector, message: &str) -> serde_json::Value {
    ...
    match result {
        Ok(value) => {
            ...
            response.insert("jsonrpc".into(), serde_json::json!("2.0"));
        ...
        Err(error) => {
            let mut response = serde_json::Map::new();
            response.insert("jsonrpc".into(), serde_json::json!("2.0"));
            response.insert("error".into(), serde_json::to_value(error.to_response()).unwrap_or_else(|serialization_error| {
                serde_json::json!({
                    "code": "FCP-9002",
                    "message": format!("Failed to serialize error response: {serialization_error}")
                })
            }));
            if let Some(id) = id {
                response.insert("id".into(), id);
