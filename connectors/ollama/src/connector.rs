...
use crate::client::{
    DEFAULT_EMBEDDING_MODEL, DEFAULT_MODEL, OllamaAuth, OllamaClient, OllamaProvider,
    OllamaUrlPolicy, classify_ollama_base_url, normalize_ollama_base_url, validate_auth_material,
};
...
pub const CONNECTOR_ID: &str = "fcp.ollama";
...
impl OllamaConfig {
    fn from_params(params: &Value) -> FcpResult<Self> {
        ...
        let auth = match (inline_auth, credential_id) {
            (Some(key), None) => OllamaAuth::ApiKey(key),
            (None, Some(id)) => OllamaAuth::CredentialId(id),
            (Some(_), Some(_)) => {
        ...
        let tailnet_only = params
            ...
            .unwrap_or(false);
        let url_policy = OllamaUrlPolicy::new(tailnet_only, allowed_hosts.clone());
        let base_url =
            normalize_ollama_base_url(params.get("base_url").and_then(Value::as_str), &url_policy)
                .map_err(invalid_config)?;
        let base_url_class = classify_ollama_base_url(&base_url);
    ...
    fn build_client(&self) -> OllamaClient {
        OllamaClient::new(
            OllamaProvider::new(self.base_url.clone(), self.auth.clone()),
...
pub struct OllamaConnector {
    ...
    client: Option<Arc<OllamaClient>>,
...
impl OllamaConnector {
    ...
    pub async fn handle_configure(&mut self, params: Value) -> FcpResult<Value> {
        let config = OllamaConfig::from_params(&params)?;
        ...
        Ok(json!({
            ...
            "checks": [
                ...
                },
                {
                    ...
                    "passed": true,
                    "critical": true,
                    "message": "connector uses only /v1 OpenAI-compatible endpoints and never auto-pulls models"
                },
                {
                    "name": "handshake",
    ...
    async fn invoke_operation(&self, operation: &str, input: Value) -> FcpResult<Value> {
        ...
        let client = self.client.as_ref().ok_or_else(|| FcpError::Internal {
            message: "Ollama client not initialized".into(),
        })?;
...
fcp_core::impl_fcp_sealed!(OllamaConnector);

#[fcp_core::async_trait]
impl FcpConnector for OllamaConnector {
    ...
    async fn health(&self) -> HealthSnapshot {
        if self.config.is_some() && self.session_id.is_some() {
            ...
            HealthSnapshot::degraded("ollama_handshake_pending")
        } else {
            HealthSnapshot::error("ollama_not_configured")
        }
    }
    ...
    async fn self_check(&self) -> FcpResult<SelfCheckReport> {
        if self.config.is_none() {
            return Ok(SelfCheckReport::degraded(
                "not_configured",
                "Ollama connector is not configured",
            ));
        }
        if self
            .config
    ...
    async fn simulate(&self, req: SimulateRequest) -> FcpResult<SimulateResponse> {
        if is_supported_operation(req.operation.as_str()) {
            ...
            Ok(SimulateResponse::denied(
                req.id,
                "operation is not supported by Ollama",
                "FCP-3010",
            ))
        }
    }
...
fn operations_info() -> Vec<OperationInfo> {
    vec![
        OperationInfo {
            ...
            summary: "Create an Ollama chat completion".into(),
            description: Some("Uses the Ollama OpenAI-compatible POST /v1/chat/completions endpoint.".into()),
            ...
            ai_hints: AgentHint {
                when_to_use: "Use when local or tailnet-hosted inference is required and the model is already present in Ollama.".into(),
                common_mistakes: vec![
                    "Do not log prompts, completions, tool-call arguments, or API keys.".into(),
                    "Do not expect this connector to pull missing models.".into(),
                    "Configure allowed_hosts for non-loopback tailnet/private hosts before use.".into(),
                ],
                examples: vec![r#"{"model":"llama3.2","messages":[{"role":"user","content":"Hello"}]}"#.into()],
        ...
        OperationInfo {
            ...
            summary: "Create an Ollama streaming chat completion".into(),
            description: Some("Uses Ollama's OpenAI-compatible SSE chat stream and returns redaction-safe chunk metadata plus assembled text.".into()),
        ...
        OperationInfo {
            ...
            summary: "Create Ollama text embeddings".into(),
            description: Some("Uses Ollama's OpenAI-compatible POST /v1/embeddings endpoint; models must already be installed.".into()),
            ...
            ai_hints: AgentHint {
                when_to_use: "Use for local embeddings with a pre-staged embedding model such as nomic-embed-text.".into(),
                common_mistakes: vec!["Do not log input text or embedding vectors.".into()],
                examples: vec![r#"{"model":"nomic-embed-text","input":"hello"}"#.into()],
                related: vec![CapabilityId::from_static(CAP_MODELS)],
            },
            rate_limit: None,
            requires_approval: Some(ApprovalMode::None),
        ...
        OperationInfo {
            ...
            summary: "List Ollama models".into(),
            description: Some("Reads and caches GET /v1/models without using native /api/tags.".into()),
            ...
            ai_hints: AgentHint {
                ...
                common_mistakes: vec!["Do not infer that an absent model will be auto-pulled.".into()],
                examples: vec!["{}".into()],
        ...
        OperationInfo {
            ...
            summary: "Probe Ollama health".into(),
            ...
            idempotency: IdempotencyClass::Strict,
            ai_hints: AgentHint {
                when_to_use: "Use to confirm the configured local or tailnet Ollama endpoint is reachable.".into(),
...
fn resource_uris_for_operation(operation: &str, input: &Value) -> Vec<String> {
    match operation {
        ...
        OP_EMBEDDINGS => {
            ...
            vec![format!("ollama:embedding-model:{model}")]
        }
        OP_MODELS | OP_HEALTH => vec!["ollama:models".into()],
        _ => Vec::new(),
    }
}
...
fn optional_model(params: &Value, field: &str, default: &str) -> FcpResult<String> {
    ...
    validate_ollama_model_id(field, raw)
}
...
mod tests {
    ...
    fn handshake_manifest_hash_tracks_bundled_manifest() {
        ...
        let actual = OllamaConnector::manifest_hash();

        assert_eq!(actual, expected);
        assert_ne!(actual, "sha256:ollama-connector-v1");
    }
}
