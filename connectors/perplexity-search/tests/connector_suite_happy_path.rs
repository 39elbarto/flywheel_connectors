...
async fn configured_connector(
    ...
    connector
        .configure(config)
        .await
        .expect("Perplexity test connector should configure");
    connector
        ...
        .await
        .expect("Perplexity test connector should handshake");
    (connector, signing_key, requested_instance_id)
}

async fn invoke_result(connector: &PerplexitySearchConnector, request: InvokeRequest) -> Value {
    connector
        .invoke(request)
        .await
        .expect("Perplexity provider contract should invoke")
        .result
        .expect("invoke result should be present")
}
...
#[fcp_async_core::runtime::test]
async fn direct_search_invoke_maps_provider_contract_output() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
        .and(header("authorization", "Bearer pplx-test-key"))
    ...
    let (connector, signing_key, instance_id) = configured_connector(
        json!({
            "api_key": "pplx-test-key",
            "base_url": server.uri()
        }),
    ...
    let output = invoke_result(
        &connector,
        search_invoke(
            "perplexity-direct-search",
            search_capability(&signing_key, &instance_id),
        ),
    )
    .await;
...
#[fcp_async_core::runtime::test]
async fn native_search_invoke_maps_provider_contract_output() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/search"))
        .and(header("authorization", "Bearer pplx-test-key"))
    ...
    let output = invoke_result(
        &connector,
        native_search_invoke(
            "perplexity-native-direct-search",
            native_search_capability(&signing_key, &instance_id),
        ),
    )
    .await;
...
#[fcp_async_core::runtime::test]
async fn search_errors_map_provider_status_contracts() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
...
#[fcp_async_core::runtime::test]
async fn search_rejects_malformed_provider_response() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
    ...
    assert!(matches!(
        err,
        FcpError::Internal { message } if message.contains("JSON parse error")
    ));
}

#[fcp_async_core::runtime::test]
async fn search_timeout_remains_retryable_external_error() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
        .respond_with(
...
#[fcp_async_core::runtime::test]
async fn search_with_system_prompt_maps_provider_answer() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
    ...
    let output = invoke_result(
        &connector,
        invoke_for_operation(
            "perplexity-search-system-prompt",
            OP_SEARCH,
            json!({
                "query": "What is Rust?",
