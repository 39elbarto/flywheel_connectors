//! FCP `PostHog` Connector - Main entrypoint
//!
//! A `PostHog` product analytics connector implementing the Flywheel Connector Protocol.
//! Provides access to events, insights, and feature flags.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP PostHog Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut PostHogConnector, message: &str) -> serde_json::Value {
