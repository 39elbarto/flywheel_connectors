//! Local loopback acceptance coverage for the `PostHog` HTTP boundary.
...
const API_KEY: &str = "phx_local_posthog_key";
const AUTH_HEADER: &str = "Bearer phx_local_posthog_key";
const PROJECT_ID: &str = "project_local";
const RESPONSE_BODY: &str = r#"{
  "results": [
    {
      "id": 42,
      "name": "Local Acceptance Insight",
      "short_id": "local-insight"
    }
  ]
}"#;
...
async fn local_non_mock_list_insights_uses_loopback_api_boundary() {
    let fixture = LoopbackFixture::start();
    let client = PostHogClient::new(
        PostHogAuth::ApiKey(API_KEY.into()),
        PROJECT_ID,
        Some(fixture.base_url()),
    )
    .expect("construct PostHog client");

    let response = client
        .list_insights()
        .await
        .expect("list insights through loopback fixture");
    client.shutdown();
    ...
    assert_eq!(
        ...
        "GET /api/projects/project_local/insights HTTP/1.1"
    );
    assert!(has_header(
        &observation.headers,
        "authorization",
    ...
    assert!(has_header(
        &observation.headers,
        "user-agent",
        "fcp-posthog/0.1.0 (FCP connector)"
    ));
    assert_eq!(response["results"][0]["id"], 42);
    assert_eq!(response["results"][0]["name"], "Local Acceptance Insight");
