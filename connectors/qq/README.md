...
- Token-refresh boundary evidence: `RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p fcp-qq --test local_non_mock local_non_mock_health_refreshes_expired_access_token_once -- --nocapture` and `RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p fcp-qq --test local_non_mock local_non_mock_channel_send_refreshes_expired_access_token_once -- --nocapture`. These loopback lanes drive `qq.health` and `qq.messages.send_channel` through the production connector, prove one 401/403-triggered token cache invalidation and refresh before the retried gateway probe or outbound send, and emit only structured booleans/counts in the printed artifacts.
