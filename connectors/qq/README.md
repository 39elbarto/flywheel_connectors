...
- The verifier runs the connector-boundary `qq_gateway_projection_logs_policy_replay_and_shutdown` e2e lane through `rch`, extracts `QQ_GATEWAY_PROJECTION_JSONL` records, checks policy, reply, media, replay, heartbeat, terminal reconnect failure, and shutdown coverage, and writes a replay bundle under `artifacts/e2e/qq-gateway-projection/<run-id>/`.
