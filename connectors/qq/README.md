...
- Token-refresh boundary evidence: `RCH_REQUIRE_REMOTE=1 rch exec -- cargo test -p fcp-qq --test local_non_mock local_non_mock_health_refreshes_expired_access_token_once -- --nocapture`. This loopback lane drives `qq.health` through the production connector, proves one 401/403-triggered token cache invalidation and refresh before the retried gateway probe, and emits only structured booleans/counts in the printed artifact.
