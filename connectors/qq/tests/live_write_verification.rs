...
const CONNECTOR: &str = "qq";
const PROVIDER: &str = "QQ Bot";
const GATE_ENV: &str = "FCP_LIVE_WRITE";
const WRITE_OPERATION: &str = "qq.messages.send_channel";
const CLEANUP_OPERATION: &str = "test_channel_retention_prune";
const TARGET_ENV: &str = "QQ_LIVE_CHANNEL_ID";
const AUTH_FAILURE_MAPPING: &str = "QQ 401/403 maps to unauthorized before message mutation";
...
fn manifest() -> EnvironmentManifest {
    EnvironmentManifest::live_write(CONNECTOR, PROVIDER)
        .with_env_secret(
            "app_id",
            "QQ_APP_ID",
            "QQ bot app id for a dedicated test account",
        )
        .with_env_secret(
            "bot_token",
            "QQ_BOT_TOKEN",
            "QQ bot token for a dedicated test account",
        )
        .with_env_var(
            TARGET_ENV,
            "Dedicated QQ channel that accepts prefixed synthetic messages",
        )
        .with_account_setup(
            "Use a disposable QQ bot account and one dedicated test channel. Every message must include the synthetic prefix and stay within the call ceiling.",
        )
        .with_budget(0.25)
