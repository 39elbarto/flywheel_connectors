...
use fcp_qwen::client::{DEFAULT_EMBEDDING_MODEL, DEFAULT_MODEL, DEFAULT_VISION_MODEL};
use fcp_qwen::connector::{CONNECTOR_ID, test_handshake_request};
...
async fn qwen_live_smoke_or_structured_skip_jsonl() {
    ...
    let chat_model =
        std::env::var("QWEN_LIVE_CHAT_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
    let vision_model = std::env::var("QWEN_LIVE_VISION_MODEL")
        .unwrap_or_else(|_| DEFAULT_VISION_MODEL.to_string());
    let embedding_model = std::env::var("QWEN_LIVE_EMBEDDING_MODEL")
    ...
    let vision = invoke(
        ...
        json!({
            "messages": [{
                "role": "user",
                "content": [
                    {"type": "text", "text": "Describe this image in one phrase."},
    ...
    .await
    .expect("live Qwen-VL image_url chat should succeed");
    emit_jsonl(
        &git_revision,
        "qwen_vl",
        "live",
        "passed",
        json!({
            "model_id_hash": blake3::hash(vision_model.as_bytes()).to_hex().to_string(),
            "completion_bytes": vision["content"].as_str().unwrap_or_default().len(),
...
fn emit_jsonl(
    ...
    record.insert("event".into(), json!("qwen_live_operation"));
    ...
    record.insert(
        "command_line".into(),
        json!("cargo test -p fcp-qwen --test live_verification qwen_live_smoke_or_structured_skip_jsonl -- --nocapture"),
    );
