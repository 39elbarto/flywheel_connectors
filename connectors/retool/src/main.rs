//! FCP `Retool` Connector - Main entrypoint
//!
//! A `Retool` connector implementing the Flywheel Connector Protocol.
//! Provides access to workflows.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Retool Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut RetoolConnector, message: &str) -> serde_json::Value {
