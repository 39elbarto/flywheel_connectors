//! FCP `Roam Research` Connector - Main entrypoint
//!
//! A `Roam Research` PKM connector implementing the Flywheel Connector Protocol.
//! Provides access to pages and blocks in a Roam graph.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Roam Research Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut RoamConnector, message: &str) -> serde_json::Value {
