...
mod tests {
    ...
    fn rate_limited_is_retryable() {
        assert!(SegmentError::RateLimited { retry_after_ms: 5000 }.is_retryable());
    }

    #[test]
    fn api_500_is_retryable() {
        assert!(SegmentError::Api { status_code: 500, message: "err".into() }.is_retryable());
    }

    #[test]
    fn api_503_is_retryable() {
        assert!(
            SegmentError::Api { status_code: 503, message: "unavailable".into() }.is_retryable()
        );
    }

    #[test]
    fn api_429_is_retryable() {
        assert!(SegmentError::Api { status_code: 429, message: "too many".into() }.is_retryable());
    }
    ...
    fn not_found_not_retryable() {
        assert!(!SegmentError::NotFound { resource: "source".into() }.is_retryable());
    }

    #[test]
    fn api_400_not_retryable() {
        assert!(
            !SegmentError::Api { status_code: 400, message: "bad request".into() }.is_retryable()
        );
    }

    #[test]
    fn retry_after_for_rate_limited() {
        let err = SegmentError::RateLimited { retry_after_ms: 30_000 };
    ...
    fn retry_after_none_for_api_error() {
        assert_eq!(
            SegmentError::Api { status_code: 500, message: "err".into() }.retry_after(),
            None
        );
    }
    ...
    fn retry_after_none_for_not_found() {
        assert_eq!(SegmentError::NotFound { resource: "x".into() }.retry_after(), None);
    }
    ...
    fn not_found_to_fcp_error() {
        match (SegmentError::NotFound { resource: "source_abc".into() }).to_fcp_error() {
    ...
    fn rate_limited_to_fcp_error() {
        match (SegmentError::RateLimited { retry_after_ms: 60_000 }).to_fcp_error() {
    ...
    fn api_error_to_fcp_error() {
        match (SegmentError::Api { status_code: 503, message: "unavailable".into() }).to_fcp_error()
        {
    ...
    fn api_error_non_retryable_to_fcp_error() {
        match (SegmentError::Api { status_code: 400, message: "bad".into() }).to_fcp_error() {
    ...
    fn error_display_not_found() {
        assert_eq!(
            SegmentError::NotFound { resource: "source".into() }.to_string(),
            "Not found: source"
        );
    }
    ...
    fn error_display_rate_limited() {
        assert_eq!(
            SegmentError::RateLimited { retry_after_ms: 2000 }.to_string(),
    ...
    fn error_display_api() {
        assert_eq!(
            SegmentError::Api { status_code: 500, message: "Internal".into() }.to_string(),
