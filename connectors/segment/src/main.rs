//! FCP Segment Connector - Main entrypoint
//!
//! A Segment CDP connector implementing the Flywheel Connector Protocol.
//! Provides access to sources, destinations, and event tracking.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Segment Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut SegmentConnector, message: &str) -> serde_json::Value {
