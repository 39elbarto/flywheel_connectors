//! FCP `SendGrid` Connector - Main entrypoint
//!
//! A `SendGrid` email API connector implementing the Flywheel Connector Protocol.
//! Provides access to mail sending, contacts, lists, templates, and statistics.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP SendGrid Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut SendGridConnector, message: &str) -> serde_json::Value {
