...
const CONNECTOR_ID: &str = "signal";
const PROVIDER: &str = "signal-cli daemon";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "SIGNAL_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "SIGNAL_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "SIGNAL_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str = "cargo test -p fcp-signal --test live_verification -- --nocapture";

#[test]
fn signal_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the signal-cli lab daemon/account.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe Signal device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the signal-cli lab daemon.",
        )
        .with_account_setup(
            "Use an isolated signal-cli daemon with a synthetic account and no personal recipients.",
        )
        .with_budget(0.01)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "signal_device_lab_verification",
    ...
    println!("SIGNAL_DEVICE_LAB_JSONL {serialized}");
}
