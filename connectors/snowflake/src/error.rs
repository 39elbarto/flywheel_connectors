...
/// Result alias for `Snowflake` operations.
...
pub enum SnowflakeError {
    ...
    /// `Snowflake` API returned an error
    #[error("Snowflake API error ({status_code}): {message}")]
    ...
    #[error("Authentication failed: invalid access token or account identifier")]
    Unauthorized,
...
mod tests {
    ...
    fn unauthorized_not_retryable() {
        assert!(!SnowflakeError::Unauthorized.is_retryable());
    }

    #[test]
    fn forbidden_not_retryable() {
        assert!(!SnowflakeError::Forbidden.is_retryable());
    }
    ...
    fn retry_after_none_for_unauthorized() {
        assert_eq!(SnowflakeError::Unauthorized.retry_after(), None);
    }

    #[test]
    fn retry_after_none_for_forbidden() {
        assert_eq!(SnowflakeError::Forbidden.retry_after(), None);
    }
    ...
    fn not_found_to_fcp_error() {
        match (SnowflakeError::NotFound {
            resource: "ANALYTICS_DB".into(),
        })
        .to_fcp_error()
        {
            FcpError::External {
                ...
                assert!(message.contains("ANALYTICS_DB"));
                assert!(!retryable);
            }
            other => panic!("expected External, got {other:?}"),
        }
    ...
    fn rate_limited_to_fcp_error() {
        match (SnowflakeError::RateLimited {
    ...
    fn json_error_to_fcp_internal() {
        ...
        match SnowflakeError::Json(bad.unwrap_err()).to_fcp_error() {
    ...
    fn error_display_unauthorized() {
        assert_eq!(
            SnowflakeError::Unauthorized.to_string(),
            "Authentication failed: invalid access token or account identifier"
        );
    }
    ...
    fn error_display_forbidden() {
        assert_eq!(
            SnowflakeError::Forbidden.to_string(),
    ...
    fn error_display_api() {
        assert_eq!(
            ...
            .to_string(),
            "Snowflake API error (500): Internal"
        );
    }
    ...
    fn not_found_to_fcp_error_service() {
        ...
        .to_fcp_error()
        {
            FcpError::External { service, .. } => assert_eq!(service, "snowflake"),
            other => panic!("expected External, got {other:?}"),
        }
    }
