//! FCP `Snowflake` Connector - Main entrypoint
//!
//! A `Snowflake` data warehouse connector implementing the Flywheel Connector Protocol.
//! Provides access to SQL queries, databases, warehouses, and tables.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Snowflake Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut SnowflakeConnector, message: &str) -> serde_json::Value {
