...
const CONNECTOR_ID: &str = "sonos";
const PROVIDER: &str = "Sonos local network";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "SONOS_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "SONOS_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "SONOS_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str = "cargo test -p fcp-sonos --test live_verification -- --nocapture";

#[test]
fn sonos_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the Sonos lab network and room.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe Sonos device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the Sonos lab speaker.",
        )
        .with_account_setup(
            "Use a dedicated Sonos lab speaker or room; health-only proof must not alter playback.",
        )
        .with_budget(0.01)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "sonos_device_lab_verification",
    ...
    println!("SONOS_DEVICE_LAB_JSONL {serialized}");
}
