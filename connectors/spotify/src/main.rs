//! FCP `Spotify` Connector - Main entrypoint
//!
//! A `Spotify` Web API connector implementing the Flywheel Connector Protocol.
//! Provides access to search, metadata, playlists, recently played, top items,
//! and recommendations.
...
fn main() -> Result<()> {
    ...
    tracing::info!("FCP Spotify Connector starting");
    run_fcp_loop()?;
    Ok(())
}
...
async fn handle_message(connector: &mut SpotifyConnector, message: &str) -> serde_json::Value {
