...
#[fcp_async_core::runtime::test]
async fn client_send_message_posts_expected_payload() {
...
#[fcp_async_core::runtime::test]
async fn client_send_file_url_posts_checked_payload() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(body_json(json!({
            "file_url": "https://cdn.example.com/report.pdf",
...
#[fcp_async_core::runtime::test]
async fn client_send_payload_posts_raw_json() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(body_json(json!({
            "text": "hello",
...
#[fcp_async_core::runtime::test]
async fn client_send_payload_returns_text_body_metadata() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
    ...
    let client = SynologyChatClient::from_config(&config).expect("client should build");
    let payload =
        SynologyChatPayload::from_value(&json!({ "text": "hello" })).expect("payload should parse");
    let result = client
        .send_payload(&payload)
        .await
        .expect("payload send should succeed");
...
#[fcp_async_core::runtime::test]
async fn client_server_error_preserves_retry_after_hint() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .respond_with(
