...
const CONNECTOR_ID: &str = "synology-chat";
const PROVIDER: &str = "Synology Chat";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "SYNOLOGY_CHAT_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "SYNOLOGY_CHAT_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "SYNOLOGY_CHAT_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str =
    "cargo test -p fcp-synology-chat --test live_verification -- --nocapture";

#[test]
fn synology_chat_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the Synology Chat lab tenant.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe Synology Chat device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the Synology Chat lab tenant.",
        )
        .with_account_setup(
            "Use an isolated Synology Chat lab tenant/channel with synthetic users and redacted webhook payloads.",
        )
        .with_budget(0.01)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "synology_chat_device_lab_verification",
    ...
    println!("SYNOLOGY_CHAT_DEVICE_LAB_JSONL {serialized}");
}
