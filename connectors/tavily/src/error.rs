...
pub enum TavilyError {
    ...
    /// API error response
    #[error("Tavily API error ({status_code}): {message}")]
...
impl ConnectorErrorMapping for TavilyError {
...
mod tests {
    ...
    fn rate_limited_maps_correctly() {
        let err = TavilyError::RateLimited {
            retry_after_ms: 5000,
        };
    ...
    fn not_configured_maps_correctly() {
        let err = TavilyError::NotConfigured;
    ...
    fn json_error_maps_to_internal() {
        ...
        let err = TavilyError::Json(json_err);
    ...
    fn from_async_cancelled() {
        let err = TavilyError::from_async_error(AsyncError::Cancelled);
        assert!(matches!(err, TavilyError::Api { status_code: 0, .. }));
    }
    ...
    fn display_formats() {
        ...
        assert_eq!(err.to_string(), "Tavily API error (404): Not Found");
    }
}
