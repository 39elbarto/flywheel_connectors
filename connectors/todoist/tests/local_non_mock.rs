//! Local loopback acceptance coverage for the `Todoist` connector.
...
const OP_PROJECTS_LIST: &str = "todoist.projects.list";
const ACCEPTANCE_SUITE_CLASS: &str = "local_non_mock";
const API_TOKEN: &str = "todoist_local_acceptance_pat";
...
async fn loopback_projects_list_uses_todoist_request_boundary() {
    ...
    connector
        .handle_handshake(json!({ "session_id": "todoist-local-non-mock" }))
        .await
        .expect("handshake connector");

    let result = connector
        .handle_invoke(json!({
            "operation_id": OP_PROJECTS_LIST,
            "input": {}
        }))
        .await
    ...
    assert_eq!(observation.request_line, "GET /projects HTTP/1.1");
    assert!(
        observation
            .headers
            .iter()
    ...
    assert!(
        observation
            .headers
            .iter()
            .any(|line| line.eq_ignore_ascii_case(&format!("authorization: Bearer {API_TOKEN}")))
    );
    assert!(observation.headers.iter().any(|line| {
        line.eq_ignore_ascii_case("user-agent: fcp-todoist/0.1.0 (FCP connector)")
    }));
    assert_eq!(result["projects"][0]["id"], "project_123");
    assert_eq!(result["projects"][0]["name"], "Acceptance Engineering");
    ...
    let artifact = json!({
        ...
        "command": "cargo test -p fcp-todoist --test local_non_mock -- --nocapture",
