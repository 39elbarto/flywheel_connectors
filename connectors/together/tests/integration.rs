...
use fcp_together::client::{
    DEFAULT_BASE_URL, DEFAULT_EMBEDDING_MODEL, DEFAULT_MODEL, TogetherAuth, TogetherClient,
    TogetherProvider, normalize_together_base_url,
};
use fcp_together::connector::{CONNECTOR_ID, test_handshake_request, test_invoke_request};
use fcp_together::types::{
    chat_request_from_value, embeddings_request_from_value, validate_together_model_id,
};
...
async fn configured_connector(
    ...
    extra_config: Value,
) -> (fcp_together::TogetherConnector, Ed25519SigningKey) {
    let mut connector = fcp_together::TogetherConnector::new();
    let mut config = serde_json::Map::new();
    config.insert("api_key".into(), json!("together-test-key"));
...
async fn invoke(
    connector: &fcp_together::TogetherConnector,
    signing_key: &Ed25519SigningKey,
...
fn provider_construction_auth_base_url_and_model_id_policy() {
    let provider = TogetherProvider::new(
        DEFAULT_BASE_URL,
        TogetherAuth::ApiKey("together-key".into()),
    );
    ...
    assert_eq!(
        normalize_together_base_url(None).unwrap(),
    ...
    let credential_provider = TogetherProvider::new(
        DEFAULT_BASE_URL,
        TogetherAuth::CredentialId("cred:together".into()),
    );
    ...
    assert!(validate_together_model_id("model", DEFAULT_MODEL).is_ok());
    assert!(validate_together_model_id("model", "namespace/name").is_ok());
    assert!(validate_together_model_id("model", "no-namespace").is_err());
    assert!(validate_together_model_id("model", "namespace/").is_err());
    assert!(validate_together_model_id("model", "namespace/model with space").is_err());
}

#[test]
fn request_builders_validate_models_embeddings_and_safety_model() {
    ...
    assert_eq!(
        chat.provider_extensions["safety_model"],
        "meta-llama/Llama-Guard-4-12B"
    );
    assert_eq!(chat.provider_extensions["reasoning_effort"], "low");
...
async fn chat_stream_embeddings_models_health_and_redaction_work_through_connector() {
    ...
    Mock::given(method("POST"))
        ...
        .and(header("authorization", "Bearer together-test-key"))
        ...
        .await;
    let sse = concat!(
        "data: {\"id\":\"chunk-1\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"openai/gpt-oss-20b\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"hel\"},\"finish_reason\":null}]}\n\n",
        "data: {\"id\":\"chunk-2\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"openai/gpt-oss-20b\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"lo\"},\"finish_reason\":\"stop\"}]}\n\n",
        "data: [DONE]\n\n"
    );
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
    ...
    assert!(!doctor.to_string().contains("together-test-key"));
...
async fn rate_limit_retries_provider_errors_map_cancellation_and_shutdown_are_safe() {
    ...
    let client = TogetherClient::new(
        TogetherProvider::new(
            ...
            TogetherAuth::ApiKey("together-test-key".into()),
        ),
...
#[fcp_async_core::runtime::test]
async fn together_loopback_e2e_jsonl_matrix() {
    let git_revision =
        std::env::var("TOGETHER_E2E_GIT_REVISION").unwrap_or_else(|_| "unknown".to_string());
    let server = MockServer::start().await;
    mount_models_array(&server, DEFAULT_MODEL, 1).await;
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        ...
        .await;
    let sse = concat!(
        "data: {\"id\":\"chunk-a\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"openai/gpt-oss-20b\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"ok\"},\"finish_reason\":\"stop\"}]}\n\n",
        "data: [DONE]\n\n"
    );
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
...
fn connector_id_matches_manifest_contract() {
    assert_eq!(CONNECTOR_ID, "fcp.together");
}
...
fn emit_jsonl(
    ...
    record.insert("event".into(), json!("together_fixture_operation"));
    ...
    record.insert(
        "command_line".into(),
        json!("cargo test -p fcp-together --test integration together_loopback_e2e_jsonl_matrix -- --nocapture"),
    );
    ...
    println!("TOGETHER_E2E_JSONL {}", Value::Object(record));
}
