...
use fcp_together::connector::{CONNECTOR_ID, test_handshake_request};
...
async fn together_live_smoke_or_structured_skip_jsonl() {
    let git_revision =
    ...
    let Ok(api_key) = std::env::var("TOGETHER_API_KEY") else {
        emit_jsonl(
            ...
            "skipped",
            json!({
                "skip_reason": "TOGETHER_API_KEY not set",
    ...
    let chat_model =
        std::env::var("TOGETHER_LIVE_CHAT_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string());
    let embedding_model = std::env::var("TOGETHER_LIVE_EMBEDDING_MODEL")
...
fn emit_jsonl(
    ...
    record.insert("event".into(), json!("together_live_operation"));
    ...
    record.insert(
        "command_line".into(),
        json!("cargo test -p fcp-together --test live_verification together_live_smoke_or_structured_skip_jsonl -- --nocapture"),
    );
