...
/// Result alias for Trello operations.
pub type TrelloResult<T> = Result<T, TrelloError>;
...
pub enum TrelloError {
    ...
    /// Trello API returned an error
    #[error("Trello API error ({status_code}): {message}")]
    ...
    #[error("Authentication failed: invalid or expired API key/token")]
    Unauthorized,
...
impl ConnectorErrorMapping for TrelloError {
...
mod tests {
    ...
    fn unauthorized_not_retryable() {
        assert!(!TrelloError::Unauthorized.is_retryable());
    }

    #[test]
    fn forbidden_not_retryable() {
        assert!(!TrelloError::Forbidden.is_retryable());
    }

    #[test]
    fn not_found_not_retryable() {
        assert!(
            !TrelloError::NotFound {
                resource: "card".into()
            }
            .is_retryable()
        );
    ...
    fn retry_after_for_rate_limited() {
        let err = TrelloError::RateLimited {
    ...
    fn retry_after_none_for_unauthorized() {
        assert_eq!(TrelloError::Unauthorized.retry_after(), None);
    }

    #[test]
    fn retry_after_none_for_forbidden() {
        assert_eq!(TrelloError::Forbidden.retry_after(), None);
    }
    ...
    fn unauthorized_to_fcp_error() {
        match TrelloError::Unauthorized.to_fcp_error() {
            FcpError::External {
                service,
                status_code,
                retryable,
    ...
    fn forbidden_to_fcp_error() {
        match TrelloError::Forbidden.to_fcp_error() {
            FcpError::External {
                service,
                status_code,
                retryable,
    ...
    fn not_found_to_fcp_error() {
        match (TrelloError::NotFound {
            resource: "card_abc".into(),
        })
        .to_fcp_error()
        {
            FcpError::External {
                ...
                assert!(message.contains("card_abc"));
                assert!(!retryable);
            }
            other => panic!("expected External, got {other:?}"),
        }
    ...
    fn json_error_to_fcp_internal() {
        ...
        match TrelloError::Json(bad.unwrap_err()).to_fcp_error() {
    ...
    fn error_display_unauthorized() {
        assert_eq!(
            TrelloError::Unauthorized.to_string(),
    ...
    fn error_display_forbidden() {
        assert_eq!(
            TrelloError::Forbidden.to_string(),
    ...
    fn retry_after_none_for_json_error() {
        ...
        assert_eq!(TrelloError::Json(bad.unwrap_err()).retry_after(), None);
    }

    #[test]
    fn rate_limited_zero_retry() {
        let err = TrelloError::RateLimited { retry_after_ms: 0 };
    ...
    fn error_display_http() {
        ...
        let trello_err = TrelloError::Http(err);
        assert!(trello_err.to_string().starts_with("HTTP error:"));
    }

    #[test]
    fn error_display_json() {
        let bad: Result<serde_json::Value, _> = serde_json::from_str("{{bad");
        let err = TrelloError::Json(bad.unwrap_err());
    ...
    fn error_debug_variant_names() {
        assert!(format!("{:?}", TrelloError::Unauthorized).contains("Unauthorized"));
        assert!(format!("{:?}", TrelloError::Forbidden).contains("Forbidden"));
    }

    #[test]
    fn not_found_display_with_special_chars() {
        let err = TrelloError::NotFound {
            resource: "card/abc+123".into(),
        };
        assert!(err.to_string().contains("card/abc+123"));
    }
    ...
    fn api_error_display_empty_message() {
        ...
        assert_eq!(err.to_string(), "Trello API error (500): ");
    }
