...
fn emit_live_jsonl(status: &str, reason: &str, observed_count: usize, evidence: &Value) {
    eprintln!(
        ...
        json!({
            ...
            "required_env": [TEAM_ID_ENV, NAMESPACE_ENV],
            "defaulted_env": BASE_URL_ENV,
            "operation": OP_PROJECTS_LIST,
            "status": status,
            ...
            "call_ceiling": 1,
            "rate_limit_guidance": "Performs one read-only project listing against the sandbox team.",
...
async fn vercel_live_sandbox_project_listing_or_structured_skip_jsonl() {
    ...
    match client.list_projects(Some(1)).await {
        Ok(value) => {
            let observed_count = value.projects.len();
            emit_live_jsonl(
                "passed",
                "",
                observed_count,
                &json!({
                    ...
                    "operation_result": "projects.list completed",
                }),
            );
        }
        Err(error) => {
