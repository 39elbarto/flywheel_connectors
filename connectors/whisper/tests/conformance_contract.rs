...
use fcp_whisper::{connector::WhisperConnector, error::WhisperError};
use serde::Serialize;
...
fn manifest() -> ConnectorManifest {
    ConnectorManifest::parse_str_unchecked(MANIFEST_TOML)
        .expect("Whisper manifest TOML should deserialize")
}

async fn runtime_operations() -> Vec<Value> {
    let connector = WhisperConnector::new();
    let introspection = connector
        .handle_introspect()
        .await
        .expect("Whisper introspection should serialize");
...
fn manifest_network_constraints_are_complete_for_all_declared_operations() {
    ...
    for (id, op) in &manifest.provides.operations {
        ...
        assert_eq!(
            ...
            "{id} should only allow OpenAI API host egress"
        );
        assert_eq!(
        ...
        assert_eq!(constraints.max_redirects, 5, "{id} should bound redirects");
        assert!(
...
fn error_taxonomy_maps_to_fcp_error_classes() {
    let unauthorized = WhisperError::Auth("invalid key".to_string()).to_fcp_error();
    assert!(matches!(
        unauthorized,
        FcpError::External {
            service,
    ...
    let rate_limited = WhisperError::RateLimited {
    ...
    let provider_unavailable = WhisperError::Api {
        status_code: 503,
    ...
    let invalid_audio = WhisperError::InvalidAudio("empty audio".to_string()).to_fcp_error();
    assert!(matches!(
        invalid_audio,
        FcpError::External {
    ...
    let too_large = WhisperError::FileTooLarge {
    ...
    let missing_model = WhisperError::ModelNotFound("whisper-missing".to_string()).to_fcp_error();
    assert!(matches!(
        missing_model,
        FcpError::External {
    ...
    let timeout = WhisperError::Timeout("deadline exceeded".to_string()).to_fcp_error();
    assert!(matches!(
        timeout,
        FcpError::External {
            status_code: None,
