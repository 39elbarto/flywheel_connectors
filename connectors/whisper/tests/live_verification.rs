...
const PROVIDER: &str = "Whisper audio lab";
const LIVE_GATE_ENV: &str = "FCP_LIVE_DEVICE";
const LAB_INVENTORY_ENV: &str = "WHISPER_DEVICE_LAB_ID";
const LAB_EVIDENCE_ENV: &str = "WHISPER_DEVICE_LAB_EVIDENCE_JSONL";
const LAB_APPROVAL_ENV: &str = "WHISPER_DEVICE_LAB_APPROVAL";
const COMMAND_LINE: &str = "cargo test -p fcp-whisper --test live_verification -- --nocapture";

#[test]
fn whisper_device_lab_prerequisites_or_structured_skip_jsonl() {
...
fn device_manifest() -> EnvironmentManifest {
    EnvironmentManifest::device(CONNECTOR_ID, PROVIDER)
        .with_env_var(
            LAB_INVENTORY_ENV,
            "Redaction-safe identifier for the audio fixture lab host.",
        )
        .with_env_var(
            LAB_EVIDENCE_ENV,
            "Path or URI of the redaction-safe Whisper device-lab JSONL evidence bundle.",
        )
        .with_env_var(
            LAB_APPROVAL_ENV,
            "Set to yes only after the operator has reserved the audio fixture and live transcription lab.",
        )
        .with_account_setup(
            "Use a synthetic audio fixture and dedicated transcription credential; do not record or emit human speech content.",
        )
        .with_budget(0.05)
...
fn emit_jsonl(environment: &LiveEnvironment, status: &str, reason: &str) {
    let record = json!({
        "event": "whisper_device_lab_verification",
    ...
    println!("WHISPER_DEVICE_LAB_JSONL {serialized}");
}
