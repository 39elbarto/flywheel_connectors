...
const BOUNDARY: &str =
    "This first slice is a guarded helper-process contract. It does not bundle or emulate the upstream personal-account runtime.";
...
impl ZalouserConnector {
    ...
    pub async fn handle_introspect(&self) -> FcpResult<Value> {
        Ok(json!({
            ...
            "operations": [
                { "id": "zalouser.helper.exec", "summary": "Execute one guarded helper operation", "capability": "zalouser.helper", "risk_level": "high", "safety_tier": "risky", "idempotency": "none" }
            ],
            "events": [],
            "resource_types": []
        }))
    ...
    pub async fn handle_simulate(&self, params: Value) -> FcpResult<Value> {
        ...
        Ok(json!({
            "allowed": operation == "zalouser.helper.exec",
            "reason": BOUNDARY
        }))
    }
