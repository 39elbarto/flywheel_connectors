...
pub fn reconstruct_secret(shares: &[ShamirShare]) -> ShamirResult<ZeroizingSecret> {
    ...
    Ok(ZeroizingSecret::with_zeroize_drop(secret))
}
...
mod tests {
    ...
    fn zeroizing_secret_debug_redacts() {
        let secret = ZeroizingSecret::with_zeroize_drop(vec![0xDE, 0xAD, 0xBE, 0xEF]);
    ...
    fn zeroizing_secret_with_bytes_and_ct_eq_bytes_agree() {
        let s = ZeroizingSecret::with_zeroize_drop(vec![1, 2, 3, 4]);
