//! Cerebras connector e2e evidence.
//!
//! The default path is deterministic and uses a loopback OpenAI-compatible
//! fixture. Set `CEREBRAS_API_KEY` to additionally exercise the first-party
//! Cerebras API and record redaction-safe JSONL evidence without prompts,
//! completions, reasoning text, tool arguments, or bearer tokens.
...
use fcp_cerebras::connector::{test_handshake_request, test_invoke_request};
...
const ARTIFACT_PATH: &str = "target/fcp-cerebras/cerebras-live-e2e.jsonl";
...
fn valid_token(
    ...
    let cose = CapabilityTokenBuilder::new()
        ...
        .zone_id("z:work")
        .principal("user:cerebras-e2e")
        .operations(&[operation])
        .issuer("node:cerebras-e2e")
...
async fn configured_connector(config: Value, capabilities: &[&'static str]) -> ConfiguredCerebras {
    let mut connector = CerebrasConnector::new();
    connector
        .handle_configure(config)
        .await
        .expect("Cerebras connector should configure");
    ...
    connector
        ...
        .await
        .expect("Cerebras connector handshake should succeed");
...
async fn cerebras_connector_emits_redacted_e2e_evidence() {
    ...
    assert!(!jsonl.contains("cerebras-fixture-key"));
    assert!(!jsonl.contains("CEREBRAS_API_KEY"));
...
async fn run_fixture_script(records: &mut Vec<Value>) {
    ...
    records.push(evidence_record(
        ...
        json!({
            ...
            "completion_tokens": Value::Null,
            "stream_chunk_count": 0_u64,
            "rate_limit_remaining_requests": 0_u64
        }),
    ));
...
async fn run_live_script_or_record_skip(records: &mut Vec<Value>) {
    let live_bearer = std::env::var("CEREBRAS_API_KEY")
        .ok()
    ...
    match chat {
        ...
        Err(err) => {
            ...
            assert_eq!(
                classify_error(&err),
                "ok",
                "live Cerebras invocation failed after live credentials were provided: {err}"
            );
        }
    }
...
async fn mount_fixture_chat(server: &MockServer) {
    Mock::given(method("POST"))
        ...
        .and(header("authorization", "Bearer cerebras-fixture-key"))
        .and(body_partial_json(json!({
            "model": MODEL,
            "stream": false
        })))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "id": "chatcmpl-cerebras-e2e",
...
async fn mount_fixture_stream(server: &MockServer) {
    let sse = concat!(
        "data: {\"id\":\"chunk-1\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"llama3.1-8b\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"fi\"},\"finish_reason\":null}]}\n\n",
        "data: {\"id\":\"chunk-2\",\"object\":\"chat.completion.chunk\",\"created\":1,\"model\":\"llama3.1-8b\",\"choices\":[{\"index\":0,\"delta\":{\"content\":\"xture\"},\"finish_reason\":\"stop\"}]}\n\n",
        "data: [DONE]\n\n"
    );
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(429)
                ...
                .insert_header("x-ratelimit-reset-tokens-minute", "11.5")
                .set_body_json(json!({
                    "error": {
...
fn evidence_record(
    ...
    json!({
        "schema": "fcp.cerebras.e2e.v1",
        "command_line": "cargo test -p fcp-e2e --no-default-features --features cerebras --test cerebras_live_e2e -- --nocapture",
        ...
        "rate_limit_metadata": {
            "remaining_requests": details.get("rate_limit_remaining_requests").cloned().unwrap_or(Value::Null)
        },
...
fn write_jsonl_artifact(records: &[Value]) -> String {
    let jsonl = records
        ...
        .collect::<Vec<_>>()
        .join("\n");
    std::fs::create_dir_all("target/fcp-cerebras").expect("artifact directory should be writable");
