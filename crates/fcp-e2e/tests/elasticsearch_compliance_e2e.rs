...
impl ElasticsearchConnectorAdapter {
    ...
    fn instance_id_str(&self) -> String {
...
fn elasticsearch_manifest_toml() -> toml::Value {
    toml::from_str(include_str!(
        "../../../connectors/elasticsearch/manifest.toml"
    ))
...
fn build_token(
    ...
    let token = CapabilityTokenBuilder::new()
        ...
        // dja9u typestate ratchet: the adapter verifies with `verify_bound`,
...
async fn elasticsearch_default_deny_compliance_suite_passes() {
    ...
    let token = build_token(
        &signing_key,
        &connector.instance_id_str(),
        "elasticsearch.search.read",
        &["elasticsearch.search"],
    );
    let invoke = invoke_request(
        "elasticsearch.get_document",
        json!({"index": "logs", "id": "abc123"}),
        token,
    );
    ...
    let suite = ComplianceSuite::new(
        "elasticsearch_default_deny",
        elasticsearch_manifest_with_hash(),
        dynamic,
    );
...
fn elasticsearch_manifest_network_guard_allows_and_denies() {
    ...
    assert_eq!(
        operations.len(),
        7,
        "Elasticsearch manifest should declare 7 operations"
    );
    ...
    for operation_name in operations.keys() {
        ...
        assert_eq!(
            ...
            "operation {operation_name} should use Elasticsearch host allowlist"
        );
