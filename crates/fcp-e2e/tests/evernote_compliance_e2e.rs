...
impl FcpConnector for EvernoteConnectorAdapter {
    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        self.connector
            .handle_handshake(json!({"session_id": "evernote-e2e-session"}))
            .await?;
        ...
        Ok(HandshakeResponse {
            ...
            manifest_hash: "sha256:evernote-e2e".to_string(),
            nonce: req.nonce,
            event_caps: None,
            auth_caps: None,
...
fn evernote_manifest_toml() -> toml::Value {
    toml::from_str(include_str!("../../../connectors/evernote/manifest.toml"))
        .expect("Evernote manifest TOML")
}
...
fn invoke_request(
    ...
    InvokeRequest {
        ...
        id: RequestId::from("evernote-e2e"),
...
async fn evernote_default_deny_compliance_suite_passes() {
    ...
    let suite = ComplianceSuite::new(
        "evernote_default_deny",
        evernote_manifest_with_hash(),
        dynamic,
    );
...
fn evernote_manifest_network_guard_allows_and_denies() {
    ...
    assert_eq!(
        operations.len(),
        5,
        "Evernote manifest should declare 5 operations"
    );
    ...
    for operation_name in operations.keys() {
        ...
        assert_eq!(
            ...
            "operation {operation_name} should use Evernote host allowlist"
        );
