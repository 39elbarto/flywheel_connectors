    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        ...
        let protocol_version = response
            ...
            .ok_or_else(|| FcpError::Internal {
                message: "metabase handshake response missing protocol_version".into(),
            })?;
        if protocol_version != "2.0" {
            return Err(FcpError::Internal {
                message: format!(
                    "metabase handshake protocol_version expected 2.0, got {protocol_version}"
                ),
            });
        }
        let _connector_id = response
            ...
            .and_then(serde_json::Value::as_str)
            .ok_or_else(|| FcpError::Internal {
                message: "metabase handshake response missing connector_id".into(),
            })?;
        let connector_caps: std::collections::BTreeSet<String> = response
            .get("capabilities")
            .and_then(serde_json::Value::as_array)
            .ok_or_else(|| FcpError::Internal {
                message: "metabase handshake response missing capabilities array".into(),
            })?
            .iter()
    ...
    fn introspect(&self) -> Introspection {
        Introspection {
            operations: vec![OperationInfo {
                ...
                summary: "metabase.dashboards.list".to_string(),
                description: None,
...
#[fcp_async_core::runtime::test]
async fn metabase_happy_path_connector_suite_passes() {
    let mock = MockApiServer::start().await;

    // Mount mock for GET /dashboard (Metabase list_dashboards)
    Mock::given(method("GET"))
        .and(path_regex(r"^/dashboard"))
