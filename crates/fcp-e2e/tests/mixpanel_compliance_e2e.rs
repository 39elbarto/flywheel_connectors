...
impl FcpConnector for MixpanelConnectorAdapter {
    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        self.connector
            .handle_handshake(json!({"session_id": "mixpanel-e2e-session"}))
            .await?;
    ...
    async fn self_check(&self) -> fcp_core::FcpResult<fcp_core::SelfCheckReport> {
        ...
        serde_json::from_value(value).map_err(|err| FcpError::Internal {
            message: format!("failed to deserialize Mixpanel self_check: {err}"),
        })
    }
    ...
    async fn invoke(&self, req: InvokeRequest) -> fcp_core::FcpResult<InvokeResponse> {
        let verifier = self.verifier.as_ref().ok_or(FcpError::Internal {
            message: "Mixpanel verifier not initialized; handshake required".into(),
        })?;
        ...
        let value = self
            .connector
            .handle_invoke(json!({"operation_id": req.operation.as_str(), "input": req.input}))
            .await?;
...
fn mixpanel_manifest_toml() -> toml::Value {
    toml::from_str(include_str!("../../../connectors/mixpanel/manifest.toml"))
        .expect("Mixpanel manifest TOML")
}
...
fn invoke_request(
    ...
    InvokeRequest {
        ...
        id: RequestId::from("mixpanel-e2e"),
        connector_id: ConnectorId::from_static("mixpanel"),
...
fn operation_network_constraints<'a>(
    ...
    manifest
        ...
        .and_then(toml::Value::as_table)
        .and_then(|o| o.get("network_constraints"))
        .and_then(toml::Value::as_table)
...
async fn mixpanel_default_deny_compliance_suite_passes() {
    ...
    let suite = ComplianceSuite::new(
        "mixpanel_default_deny",
        mixpanel_manifest_with_hash(),
        dynamic,
    );
...
async fn mixpanel_happy_path_connector_suite_passes() {
    ...
    let hits = received
        .iter()
        .filter(|request| request.url.path() == "/funnels/list")
        .count();
    assert_eq!(hits, 1, "expected exactly one GET to /funnels/list");
...
fn mixpanel_manifest_network_guard_allows_and_denies() {
    ...
    assert_eq!(
        operations.len(),
        3,
        "Mixpanel manifest should declare 3 operations"
    );
    ...
    for operation_name in operations.keys() {
        ...
        assert_eq!(
            ...
            "operation {operation_name} should use exact Mixpanel host allowlist"
        );
