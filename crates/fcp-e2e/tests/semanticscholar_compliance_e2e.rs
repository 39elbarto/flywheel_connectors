//! E2E Semantic Scholar connector compliance tests.
//!
//! Exercises the Semantic Scholar connector through the shared E2E harness:
...
//! Run: `cargo test --package fcp-e2e --features semanticscholar`

#![cfg(feature = "semanticscholar")]
...
impl SemanticScholarConnectorAdapter {
    fn new() -> Self {
        Self {
            connector: SemanticScholarConnector::new(),
...
impl FcpConnector for SemanticScholarConnectorAdapter {
    ...
    async fn handshake(&mut self, req: HandshakeRequest) -> fcp_core::FcpResult<HandshakeResponse> {
        self.connector
            .handle_handshake(json!({
                "session_id": "semanticscholar-e2e-session",
            }))
            .await?;
        ...
        Ok(HandshakeResponse {
            ...
            manifest_hash: "sha256:semanticscholar-e2e".to_string(),
            nonce: req.nonce,
            event_caps: None,
            auth_caps: None,
        ...
        match self.connector.handle_health().await {
            Ok(payload) => {
                ...
                match status {
                    ...
                    other => HealthSnapshot::degraded(format!("semanticscholar_status:{other}")),
                }
            }
    ...
    async fn self_check(&self) -> fcp_core::FcpResult<fcp_core::SelfCheckReport> {
        ...
        serde_json::from_value(value).map_err(|err| FcpError::Internal {
            message: format!("failed to deserialize Semantic Scholar self_check: {err}"),
        })
    }
    ...
    async fn invoke(&self, req: InvokeRequest) -> fcp_core::FcpResult<InvokeResponse> {
        let verifier = self.verifier.as_ref().ok_or(FcpError::Internal {
            message: "Semantic Scholar verifier not initialized; handshake required".into(),
        })?;
...
fn required_capability(operation: &str) -> fcp_core::FcpResult<CapabilityId> {
    let capability = match operation {
        ...
        | "semanticscholar.paper.recommendations" => "semanticscholar.papers.read",
        "semanticscholar.author.get" | "semanticscholar.author.papers" => {
...
fn semanticscholar_manifest_with_hash() -> String {
    let raw = include_str!("../../../connectors/semanticscholar/manifest.toml");
...
fn semanticscholar_manifest_toml() -> toml::Value {
    toml::from_str(include_str!(
        "../../../connectors/semanticscholar/manifest.toml"
    ))
    .expect("Semantic Scholar manifest TOML")
}

fn semanticscholar_config(base_url: &str) -> serde_json::Value {
    json!({
        "api_key": "semanticscholar-test-api-key",
        "base_url": base_url,
    })
}
...
fn invoke_request(
    ...
    InvokeRequest {
        ...
        id: RequestId::from("semanticscholar-e2e"),
        connector_id: ConnectorId::from_static("semanticscholar"),
...
async fn semanticscholar_default_deny_compliance_suite_passes() {
    ...
    let suite = ComplianceSuite::new(
        "semanticscholar_default_deny",
        semanticscholar_manifest_with_hash(),
        dynamic,
    );
...
#[test]
fn semanticscholar_manifest_network_guard_allows_and_denies() {
    let manifest = semanticscholar_manifest_toml();
    ...
    assert_eq!(
        operations.len(),
        7,
        "Semantic Scholar manifest should declare 7 operations"
    );

    let expected_hosts = vec!["api.semanticscholar.org".to_string()];

    for operation_name in operations.keys() {
        let host_allow = operation_host_allow_list(&manifest, operation_name);
        assert_eq!(
            host_allow, expected_hosts,
            "operation {operation_name} should use exact Semantic Scholar API host allowlist"
        );

        assert!(host_allowed("api.semanticscholar.org", &host_allow));
        assert!(!host_allowed("semanticscholar.org", &host_allow));
