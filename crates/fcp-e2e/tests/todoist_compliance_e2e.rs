//! E2E Todoist connector compliance tests.
//!
//! Exercises the Todoist connector through the shared E2E harness:
...
//! Run: `cargo test --package fcp-e2e --features todoist`
...
struct TodoistConnectorAdapter {
...
impl TodoistConnectorAdapter {
    fn new() -> Self {
        Self {
            connector: TodoistConnector::new(),
...
impl FcpConnector for TodoistConnectorAdapter {
        ...
        match self.connector.handle_health().await {
            Ok(payload) => {
                ...
                match status {
                    ...
                    other => HealthSnapshot::degraded(format!("todoist_status:{other}")),
                }
            }
    ...
    async fn self_check(&self) -> fcp_core::FcpResult<fcp_core::SelfCheckReport> {
        ...
        serde_json::from_value(value).map_err(|err| FcpError::Internal {
            message: format!("failed to deserialize Todoist self_check: {err}"),
        })
    }
    ...
    fn introspect(&self) -> Introspection {
        Introspection {
            operations: vec![OperationInfo {
                id: OperationId::from_static("todoist.projects.list"),
                summary: "todoist.projects.list".to_string(),
                description: None,
                input_schema: json!({"type": "object"}),
                output_schema: json!({"type": "object"}),
                capability: CapabilityId::from_static("todoist.projects.read"),
...
fn required_capability(operation: &str) -> fcp_core::FcpResult<CapabilityId> {
    let capability = match operation {
        "todoist.projects.list" => "todoist.projects.read",
        "todoist.tasks.list" => "todoist.tasks.read",
        "todoist.tasks.create" | "todoist.tasks.complete" | "todoist.tasks.delete" => {
            "todoist.tasks.write"
        }
        _ => {
...
fn todoist_manifest_with_hash() -> String {
    let raw = include_str!("../../../connectors/todoist/manifest.toml");
...
fn todoist_config(base_url: &str) -> serde_json::Value {
    json!({
        "api_token": "todoist-test-api-token",
        "base_url": base_url,
    })
}
...
fn invoke_request(
    ...
    InvokeRequest {
        ...
        id: RequestId::from("todoist-e2e"),
        connector_id: ConnectorId::from_static("todoist"),
...
async fn todoist_default_deny_compliance_suite_passes() {
    let mock = MockApiServer::start().await;

    let mut connector = TodoistConnectorAdapter::new();
    ...
    let suite = ComplianceSuite::new(
        "todoist_default_deny",
        todoist_manifest_with_hash(),
        dynamic,
    );

    let mut runner = E2eRunner::new("fcp-e2e-todoist");
    let report = runner
...
#[fcp_async_core::runtime::test]
async fn todoist_allow_valid_token_connector_suite_passes() {
    let mock = MockApiServer::start().await;
...
#[test]
fn todoist_manifest_network_guard_allows_and_denies() {
    let manifest = todoist_manifest_toml();
    ...
    assert_eq!(
        operations.len(),
        5,
        "Todoist manifest should declare 5 operations"
    );

    let expected_hosts = vec!["api.todoist.com".to_string()];

    for operation_name in operations.keys() {
        let host_allow = operation_host_allow_list(&manifest, operation_name);
        assert_eq!(
            host_allow, expected_hosts,
            "operation {operation_name} should use exact Todoist API host allowlist"
        );

        assert!(host_allowed("api.todoist.com", &host_allow));
        assert!(!host_allowed("todoist.com", &host_allow));
        assert!(!host_allowed("evil.api.todoist.com", &host_allow));
