...
//!
//! Measures append throughput at concurrency 1 / 8 / 32 / 128 across
...
//! contention (same-zone CAS retries) while the "multi-zone" group
//! surfaces the load-bearing parallelism win.
//!
...
/// Single-zone contention — N threads, each appending APPENDS_PER
/// events to the SAME zone. The per-zone Mutex still serialises
/// these but the canonical-CBOR + BLAKE3 work runs lock-free under
/// the optimistic-CAS pattern; retries fire on race-with-write.
/// Measures the contention floor.
