...
impl MockOrigin {
    fn start() -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind github mock origin");
    ...
    fn url(&self) -> String {
        format!(
            "http://127.0.0.1:{}/repos/octocat/hello-world",
            self.addr.port()
        )
    }
    ...
    fn observed_request(self) -> String {
        let observed = self
            ...
            .expect("github mock origin must capture a request")
            .expect("github mock origin request must parse");
...
fn send_authorized_request(request: &EgressHttpRequest) {
    ...
    let response = outbound
        .send()
        .expect("authorized github request reaches mock origin");
...
fn runtime_context<'a>(
    ...
    RuntimeEgressDecisionContext {
        ...
        operation,
        zone_id: "z:work",
        request_id: "req-secretless-github",
        correlation_id: Some("corr-secretless-github"),
        execution_mode: RuntimeNetworkEnforcement::HostEgressProxy,
        constraint_source: "secretless-github-smoke",
        credential_allow,
    }
}

#[test]
fn github_credential_id_request_gets_bearer_at_egress_boundary() {
    let origin = MockOrigin::start();
    let credential_id = "11111111-1111-4111-8111-111111111111";
    let secret = "ghp_secretless_smoke";
    ...
    let decision = authorize_runtime_http_egress(&context, &constraints, &mut request, &injector)
        .expect("github egress request is authorized");
    ...
    assert!(observed_lower.contains("authorization: bearer ghp_secretless_smoke"));
