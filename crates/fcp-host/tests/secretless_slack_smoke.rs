...
impl MockOrigin {
    fn start() -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").expect("bind slack mock origin");
    ...
    fn url(&self) -> String {
        format!("http://127.0.0.1:{}/api/chat.postMessage", self.addr.port())
    }
    ...
    fn observed_request(self) -> String {
        let observed = self
            ...
            .expect("slack mock origin must capture a request")
            .expect("slack mock origin request must parse");
...
fn send_authorized_request(request: &EgressHttpRequest) {
    ...
    let response = outbound
        .send()
        .expect("authorized slack request reaches mock origin");
...
fn runtime_context<'a>(
    ...
    RuntimeEgressDecisionContext {
        ...
        operation,
        zone_id: "z:work",
        request_id: "req-secretless-slack",
        correlation_id: Some("corr-secretless-slack"),
        execution_mode: RuntimeNetworkEnforcement::HostEgressProxy,
        constraint_source: "secretless-slack-smoke",
        credential_allow,
    }
}

#[test]
fn slack_credential_id_request_gets_bot_bearer_at_egress_boundary() {
    let origin = MockOrigin::start();
    let credential_id = "22222222-2222-4222-8222-222222222222";
    let secret = "xoxb-secretless-smoke";
    ...
    let mut request = EgressHttpRequest {
        ...
        body: Some(br#"{"channel":"C123","text":"hello"}"#.to_vec()),
    ...
    let decision = authorize_runtime_http_egress(&context, &constraints, &mut request, &injector)
        .expect("slack egress request is authorized");
    ...
    assert!(observed_lower.contains("authorization: bearer xoxb-secretless-smoke"));
