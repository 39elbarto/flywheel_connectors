...
pub fn recommend_capabilities(
    ...
    for aggregate in aggregates {
        let entry = by_zone.entry(aggregate.key.zone_id.as_str()).or_insert_with(|| {
            ZoneRiskSummary {
...
mod tests {
    ...
    fn recommendations_flag_unused_and_risky() {
        ...
        let report = recommend_capabilities(&[aggregate], 100, RecommendationConfig {
    ...
    fn recommendations_keep_active_safe_caps() {
        ...
        assert_eq!(report.recommendations[0].suggestion, CapabilitySuggestionKind::Keep);
    }
}
