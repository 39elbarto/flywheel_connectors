...
type RuleSpec = (CoverageCategory, &'static str, bool, u32, bool, bool);

const KERNEL_RULE_SPECS: &[RuleSpec] = &[
    (
        CoverageCategory::PositivePath,
...
const HOST_RULE_SPECS: &[RuleSpec] = &[
    (
        CoverageCategory::PositivePath,
...
const MANIFEST_RULE_SPECS: &[RuleSpec] = &[
    (
        CoverageCategory::PositivePath,
...
const POLICY_RULE_SPECS: &[RuleSpec] = &[
    (
        CoverageCategory::PositivePath,
...
const PLACEMENT_RULE_SPECS: &[RuleSpec] = &[
    (
        CoverageCategory::PositivePath,
...
const PROVISIONING_RULE_SPECS: &[RuleSpec] = &[
    (
        CoverageCategory::PositivePath,
...
fn build_rules(subsystem: SubsystemClass, specs: &[RuleSpec]) -> Vec<CoverageRule> {
    specs
        .iter()
        .map(
            |&(category, description, required, min_cases, requires_host, requires_fuzz)| {
                rule(
                    subsystem,
                    category,
                    description,
...
pub fn default_policy(subsystem: SubsystemClass) -> SubsystemCoveragePolicy {
    let rules = match subsystem {
        SubsystemClass::Kernel => build_rules(subsystem, KERNEL_RULE_SPECS),
        SubsystemClass::Host => build_rules(subsystem, HOST_RULE_SPECS),
        SubsystemClass::Abi => build_rules(subsystem, ABI_RULE_SPECS),
        SubsystemClass::Sdk => build_rules(subsystem, SDK_RULE_SPECS),
        SubsystemClass::Manifest => build_rules(subsystem, MANIFEST_RULE_SPECS),
        SubsystemClass::Policy => build_rules(subsystem, POLICY_RULE_SPECS),
        SubsystemClass::Placement => build_rules(subsystem, PLACEMENT_RULE_SPECS),
        SubsystemClass::Provisioning => build_rules(subsystem, PROVISIONING_RULE_SPECS),
    };
