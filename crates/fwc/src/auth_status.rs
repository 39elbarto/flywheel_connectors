...
fn expiry_warning_message(meta: &AuthMetadata, status: ExpiryStatus) -> String {
    match status {
        ...
        ExpiryStatus::ExpiringImminently => {
            let remaining = meta.time_to_expiry().map_or_else(|| "< 1h".to_owned(), format_duration);
            format!(
        ...
        ExpiryStatus::ExpiringSoon => {
            let remaining = meta.time_to_expiry().map_or_else(|| "< 7d".to_owned(), format_duration);
            format!(
...
mod tests {
    ...
    fn expiry_status_labels() {
        ...
        assert_eq!(ExpiryStatus::ExpiringImminently.label(), "expiring imminently");
    ...
    fn rotation_reason_labels() {
        ...
        assert_eq!(RotationReason::SecurityIncident.label(), "security incident");
