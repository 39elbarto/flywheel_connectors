...
mod tests {
    ...
    const KNOWN_CANONICAL_COMMANDS: &[&str] = &[
        "guide",
        "task",
        "plan",
        "explain",
    ...
    fn assert_commands_resolve_to_known(inputs: &[&str], kind: &str) {
        for input in inputs {
            let res = resolve_command(input);
            assert!(
                res.is_some(),
                "{kind} `{input}` did not resolve to any command"
            );
            let canonical = res.unwrap().canonical;
            assert!(
                KNOWN_CANONICAL_COMMANDS.contains(&canonical),
                "{kind} `{input}` resolved to unknown command `{canonical}`"
            );
        }
    }
    ...
    fn all_aliases_resolve_to_known_canonical_commands() {
        assert_commands_resolve_to_known(TEST_ALIASES, "alias");
    }

    #[test]
    fn all_typos_resolve_to_known_canonical_commands() {
        assert_commands_resolve_to_known(TEST_TYPOS, "typo");
    }
