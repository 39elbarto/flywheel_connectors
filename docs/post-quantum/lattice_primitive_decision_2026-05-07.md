...
| --- | --- | --- | --- | --- |
| `fcp-crypto-pq` current scaffold | API shape exists, but `trap_gen` and `delegate` create deterministic SHAKE seed bundles; `sample_pre` and `verify` intentionally return `NotImplemented`. | In-tree, `unsafe_code` forbidden, small dependency set. | Redacted debug and zeroization exist for current secret blobs, but the blobs are not real trapdoor bases. | Keep as fixture/API scaffold only. |
