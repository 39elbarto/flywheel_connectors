...
must be a redaction-safe relative `target/fcp-crypto-pq/*.jsonl` path, never an
absolute local/private path, arbitrary repo-relative path, or a `..` traversal;
an unsafe `ARTIFACT` override is rejected before the script creates the artifact
directory or JSONL file. The final scan must appear immediately after the
