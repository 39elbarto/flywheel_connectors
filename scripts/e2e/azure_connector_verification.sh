...
require_rch_remote_proof() {
  ...
  if command_uses_rch_exec "$@" && ! rch_remote_summary_present "${log_path}"; then
    echo "[azure-verification] ${name}: rch command did not produce remote proof" >&2
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration lifecycle_health_unconfigured_includes_guidance -- --nocapture
then
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration doctor_unconfigured_reports_remediation -- --nocapture
then
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration self_check_ready_with_local_management_override_and_evidence -- --nocapture
then
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration self_check_retryable_management_failure_reports_degraded -- --nocapture
then
...
if run_logged \
  blob_put_evidence \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration invoke_blob_put_preserves_artifact_evidence -- --nocapture
then
  blob_put_status="passed"
else
  blob_put_status="$(classify_step_failure "${OUT_ROOT}/logs/blob_put_evidence.log")"
  promote_overall_status "${blob_put_status}"
fi

if run_logged \
  keyvault_set_secret_evidence \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration invoke_keyvault_set_secret_preserves_artifact_evidence -- --nocapture
then
  keyvault_set_secret_status="passed"
else
  keyvault_set_secret_status="$(classify_step_failure "${OUT_ROOT}/logs/keyvault_set_secret_evidence.log")"
  promote_overall_status "${keyvault_set_secret_status}"
fi

if run_logged \
  compliance_evidence \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration introspection_emits_v3_compliance_evidence -- --nocapture
then
...
if run_logged \
  integration_suite \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-azure --test integration -- --nocapture
then
...
if run_logged \
  clippy \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo clippy -p fcp-azure --all-targets -- -D warnings
then
...
jq -n \
  ...
  --arg blob_put_evidence "${blob_put_status}" \
  --arg keyvault_set_secret_evidence "${keyvault_set_secret_status}" \
  ...
  --arg blob_put_evidence_log "${OUT_ROOT}/logs/blob_put_evidence.log" \
  --arg keyvault_set_secret_evidence_log "${OUT_ROOT}/logs/keyvault_set_secret_evidence.log" \
  ...
  '{
    ...
    steps: {
      ...
      blob_put_evidence: $blob_put_evidence,
      keyvault_set_secret_evidence: $keyvault_set_secret_evidence,
    ...
    artifacts: {
      ...
      blob_put_evidence_log: $blob_put_evidence_log,
      keyvault_set_secret_evidence_log: $keyvault_set_secret_evidence_log,
