...
SCRIPT_PATH="scripts/e2e/circleci_connector_verification.sh"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/circleci_connector/${RUN_ID}}"
LOG_JSONL="${LOG_JSONL:-${OUT_ROOT}/circleci_connector_verification.jsonl}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_REQUIRE_REMOTE="${RCH_REQUIRE_REMOTE:-1}"
RCH_VISIBILITY="${RCH_VISIBILITY:-verbose}"
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-circleci-${RUN_ID}-target}"
...
emit_step() {
  ...
  jq -cn \
    --arg record_type "circleci_connector_verification_step" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.circleci" \
    ...
    '{
      ...
      redaction: {
        circle_token_logged: false,
        project_slug_logged: false,
        branch_logged: false,
        pipeline_parameters_logged: false,
...
run_logged() {
  ...
  echo "[circleci-verification] ${step}: ${command_text}"
  if (
    cd "${REPO_ROOT}"
    "$@"
  ) >"${log_path}" 2>&1; then
...
redaction_scan() {
  ...
  local forbidden=(
    "local-circleci-api-token"
    "CIRCLECI_PARAMETER_SHOULD_NOT_APPEAR_IN_EVIDENCE"
  ...
  emit_step "redaction_scan" "${status}" "${scan_log}" "grep forbidden CircleCI fixture material"
  promote_status "${status}"
}
...
emit_summary() {
  ...
  jq -cn \
    --arg record_type "circleci_connector_verification_summary" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.circleci" \
    --arg git_revision "$(git_revision)" \
    ...
    '{
      ...
      git_revision: $git_revision,
      status: $status,
      local_non_mock: "connectors/circleci/tests/local_non_mock.rs",
      live_verification: "connectors/circleci/tests/live_verification.rs",
      fixture_mode: "loopback HTTP CircleCI fixtures plus sandbox-gated read-only live suite",
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo check -p fcp-circleci --locked --all-targets
...
run_logged \
  ...
  env -u RCH_FORCE_REMOTE -u RCH_REQUIRE_REMOTE "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo fmt -p fcp-circleci -- --check
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-circleci --locked --test local_non_mock -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-circleci --locked --tests -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo clippy -j 2 -p fcp-circleci --locked --all-targets -- -D warnings

redaction_scan
emit_summary

echo "CIRCLECI_CONNECTOR_VERIFICATION_JSONL=${LOG_JSONL}"
exit "${EXIT_CODE}"
