...
{"event":"fireworks_fixture_missing_jsonl","status":"failed","reason":"loopback test emitted no FIREWORKS_E2E_JSONL records","git_revision":"${git_revision}","fixture_mode":"wiremock","log":"${OUT_ROOT}/logs/loopback_jsonl.log"}
EOF
...
if grep -a '^FIREWORKS_E2E_JSONL ' "${OUT_ROOT}/logs/live_jsonl.log" \
  | sed 's/^FIREWORKS_E2E_JSONL //' \
...
{"event":"fireworks_live_missing_jsonl","status":"failed","reason":"live test emitted no FIREWORKS_E2E_JSONL records","git_revision":"${git_revision}","fixture_mode":"live","log":"${OUT_ROOT}/logs/live_jsonl.log"}
EOF
...
cat >"${OUT_ROOT}/environment.json" <<EOF
{
  ...
  "repo_root": "${REPO_ROOT}",
  "verification_script": "scripts/e2e/fireworks_connector_verification.sh",
  ...
  "live_mode": "FIREWORKS_API_KEY gated",
...
echo "Fireworks verification artifacts written to ${OUT_ROOT}"
exit "${EXIT_CODE}"
