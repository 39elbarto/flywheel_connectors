...
classify_failure() {
  ...
  if grep -Eqi 'RCH-E|remote required; refusing local fallback|rch command did not produce remote proof|\[RCH\] local|No space left on device|connection reset by peer|Backend unavailable|unable to update registry|spurious network error|failed to get successful HTTP response|missing worker system package|timeout: failed to execute process|failed to load manifest for dependency|toon_rust/Cargo\.toml' "${log_path}"; then
    echo "infra_blocked"
  else
    echo "failed"
  fi
