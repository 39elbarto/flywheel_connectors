...
run_logged() {
  ...
  echo "[google-docs-verification] ${name}: $*" >&2
  (
    cd "${REPO_ROOT}" || exit
    "$@"
  ) >"${log_path}" 2>&1
...
run_graduation_gauntlet() {
  local connector_path="connectors/google-docs"
  ...
  : >"${jsonl_path}"
  echo "[google-docs-verification] graduation_gauntlet: scripts/graduation/run_gauntlet.sh ${connector_path}" >&2
  (
    cd "${REPO_ROOT}" || exit
...
if grep -a '"suite_class":"local_non_mock"' "${OUT_ROOT}/logs/local_non_mock.log" >"${OUT_ROOT}/evidence/local_non_mock.jsonl"; then
  if jq -s -e '
    length >= 3
    and all(.[]; .connector == "google-docs")
    and all(.[]; .acceptance_suite_class == "local_non_mock")
    and all(.[]; .fixture_mode == "loopback_http")
    and all(.[]; .result == "passed")
    and any(.[]; .operation == "docs.get" and .method == "GET")
    and any(.[]; .operation == "docs.get" and .auth_gate.authorization_header_verified == true and .headers.accept_json_seen == true and .headers.user_agent_seen == true)
...
{"event":"google_docs_local_non_mock_missing_jsonl","status":"${local_non_mock_jsonl_status}","reason":"local_non_mock test emitted no extractable local_non_mock JSONL records","git_revision":"${git_revision}","fixture_mode":"loopback_http","log":"${OUT_ROOT}/logs/local_non_mock.log"}
EOF
  if [[ "${local_non_mock_status}" == "passed" ]]; then
    local_non_mock_jsonl_status="failed"
    promote_overall_status failed
...
if grep -qE 'ya29\.|local-loopback-auth-value|Connector Suite Notes|Hello from Docs|doc_test_123|docs\.google\.com|127\.0\.0\.1:[0-9]+|Authorization: Bearer|refresh_token|client_secret' "${OUT_ROOT}/evidence/local_non_mock.jsonl"; then
  local_non_mock_jsonl_status="failed"
  promote_overall_status failed
fi
...
echo "Google Docs verification artifacts written to ${OUT_ROOT}"
echo "GOOGLE_DOCS_E2E_JSONL=${OUT_ROOT}/evidence/local_non_mock.jsonl"
echo "GOOGLE_DOCS_E2E_SUMMARY=${OUT_ROOT}/summary.json"
exit "${EXIT_CODE}"
