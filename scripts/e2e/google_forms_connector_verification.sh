...
OUT_ROOT="${OUT_ROOT:-/tmp/fcp-google-forms-e2e/${RUN_ID}}"
...
run_logged() {
  ...
  echo "[google-forms-verification] ${name}: $*" >&2
  (
    cd "${REPO_ROOT}" || exit
    "$@"
  ) >"${log_path}" 2>&1
...
run_graduation_gauntlet() {
  local connector_path="connectors/google-forms"
  ...
  : >"${jsonl_path}"
  echo "[google-forms-verification] graduation_gauntlet: scripts/graduation/run_gauntlet.sh ${connector_path}" >&2
  (
    cd "${REPO_ROOT}" || exit
...
if run_capture_stdout \
  ...
  env CARGO_BUILD_JOBS="${BUILD_JOBS}" CARGO_TARGET_DIR="${TARGET_DIR}" cargo run -q --locked -p fwc -- manifest fix connectors/google-forms/manifest.toml --check --json
then
...
run_step cargo_check env CARGO_BUILD_JOBS="${BUILD_JOBS}" CARGO_TARGET_DIR="${TARGET_DIR}" cargo check --locked -p fcp-google-forms --all-targets
cargo_check_status="${LAST_STEP_STATUS}"

# `cargo fmt --check` validates source state; it is not accepted remote Cargo proof.
run_step format_check cargo fmt -p fcp-google-forms -- --check
format_check_status="${LAST_STEP_STATUS}"

run_step connector_suite env CARGO_BUILD_JOBS="${BUILD_JOBS}" CARGO_TARGET_DIR="${TARGET_DIR}" cargo test --locked -p fcp-google-forms --test connector_suite_happy_path -- --nocapture
connector_suite_status="${LAST_STEP_STATUS}"

run_step local_non_mock env CARGO_BUILD_JOBS="${BUILD_JOBS}" CARGO_TARGET_DIR="${TARGET_DIR}" GIT_REVISION="${git_revision}" cargo test --locked -p fcp-google-forms --test local_non_mock -- --nocapture
...
if grep -a '"suite_class":"local_non_mock"' "${OUT_ROOT}/logs/local_non_mock.log" >"${OUT_ROOT}/evidence/local_non_mock.jsonl"; then
  if jq -s -e '
    length >= 3
    and all(.[]; .connector == "google-forms")
    and all(.[]; .acceptance_suite_class == "local_non_mock")
    and all(.[]; .fixture_mode == "loopback_http")
    and all(.[]; .result == "passed")
    and any(.[]; .operation == "forms.get" and .method == "GET")
    and any(.[]; .operation == "forms.get" and .auth_gate.authorization_header_verified == true and .headers.accept_json_seen == true and .headers.user_agent_seen == true)
...
{"event":"google_forms_local_non_mock_missing_jsonl","status":"${local_non_mock_jsonl_status}","reason":"local_non_mock test emitted no extractable local_non_mock JSONL records","git_revision":"${git_revision}","fixture_mode":"loopback_http","log":"${OUT_ROOT}/logs/local_non_mock.log"}
EOF
  if [[ "${local_non_mock_status}" == "passed" ]]; then
    local_non_mock_jsonl_status="failed"
    promote_overall_status failed
...
if grep -qE 'ya29\.|local-loopback-auth-value|Private local fixture|Local item|form_test_123|private@example|private answer|private-response-id|private-next-token|forms\.googleapis\.com|127\.0\.0\.1:[0-9]+|Authorization: Bearer|refresh_token|client_secret|respondentEmail|fileUploadAnswers' "${OUT_ROOT}/evidence/local_non_mock.jsonl"; then
  local_non_mock_jsonl_status="failed"
  promote_overall_status failed
fi

run_step clippy env CARGO_BUILD_JOBS="${BUILD_JOBS}" CARGO_TARGET_DIR="${TARGET_DIR}" cargo clippy --locked -p fcp-google-forms --all-targets --no-deps -- -D warnings
clippy_status="${LAST_STEP_STATUS}"

cat >"${OUT_ROOT}/environment.json" <<EOF
{
  "run_id": "${RUN_ID}",
  "connector": "fcp-google-forms",
  "repo_root": "${REPO_ROOT}",
  "verification_script": "scripts/e2e/google_forms_connector_verification.sh",
  ...
  "redaction": "no Google Forms access token, loopback endpoint, form or response ID, question or answer text, respondent email, file-upload reference, page token, live credential secret, provider payload, or provider error body is emitted in extracted evidence"
}
EOF
...
env CARGO_BUILD_JOBS="\${BUILD_JOBS}" CARGO_TARGET_DIR="\${TARGET_DIR}" cargo run -q --locked -p fwc -- manifest fix connectors/google-forms/manifest.toml --check --json
env CARGO_BUILD_JOBS="\${BUILD_JOBS}" CARGO_TARGET_DIR="\${TARGET_DIR}" cargo check --locked -p fcp-google-forms --all-targets
cargo fmt -p fcp-google-forms -- --check
env CARGO_BUILD_JOBS="\${BUILD_JOBS}" CARGO_TARGET_DIR="\${TARGET_DIR}" cargo test --locked -p fcp-google-forms --test connector_suite_happy_path -- --nocapture
env CARGO_BUILD_JOBS="\${BUILD_JOBS}" CARGO_TARGET_DIR="\${TARGET_DIR}" GIT_REVISION="\${git_revision}" cargo test --locked -p fcp-google-forms --test local_non_mock -- --nocapture
env CARGO_BUILD_JOBS="\${BUILD_JOBS}" CARGO_TARGET_DIR="\${TARGET_DIR}" cargo clippy --locked -p fcp-google-forms --all-targets --no-deps -- -D warnings
EOF
...
echo "Google Forms verification artifacts written to ${OUT_ROOT}"
echo "GOOGLE_FORMS_E2E_JSONL=${OUT_ROOT}/evidence/local_non_mock.jsonl"
echo "GOOGLE_FORMS_E2E_SUMMARY=${OUT_ROOT}/summary.json"
exit "${EXIT_CODE}"
