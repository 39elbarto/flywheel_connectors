...
manifest_check_cmd=(
  ...
  manifest
  fix
  connectors/hackernews/manifest.toml
  --check
  --json
)
...
cat > "${OUT_ROOT}/summary.json" <<EOF
{
  ...
  "steps": {
    ...
    "item_invoke_evidence": "${item_invoke_status}",
  ...
  "artifacts": {
    ...
    "item_invoke_evidence_log": "${OUT_ROOT}/logs/item_invoke_evidence.log",
...
echo "Hacker News verification artifacts written to ${OUT_ROOT}"
exit "${EXIT_CODE}"
