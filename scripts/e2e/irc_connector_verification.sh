...
SCRIPT_PATH="scripts/e2e/irc_connector_verification.sh"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/irc_connector/${RUN_ID}}"
LOG_JSONL="${LOG_JSONL:-${OUT_ROOT}/irc_connector_verification.jsonl}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_REQUIRE_REMOTE="${RCH_REQUIRE_REMOTE:-1}"
RCH_VISIBILITY="${RCH_VISIBILITY:-verbose}"
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-irc-${RUN_ID}-target}"
...
emit_step() {
  ...
  jq -cn \
    --arg record_type "irc_connector_verification_step" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.irc" \
...
run_logged() {
  ...
  echo "[irc-verification] ${step}: ${command_text}"
  if (
    cd "${REPO_ROOT}"
    "$@"
  ) >"${log_path}" 2>&1; then
...
redaction_scan() {
  ...
  emit_step "redaction_scan" "${status}" "${scan_log}" "grep forbidden IRC fixture material"
  promote_status "${status}"
}
...
emit_summary() {
  ...
  jq -cn \
    --arg record_type "irc_connector_verification_summary" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.irc" \
    --arg git_revision "$(git_revision)" \
    ...
    '{
      ...
      git_revision: $git_revision,
      status: $status,
      local_non_mock: "connectors/irc/tests/local_non_mock.rs",
      fixture_mode: "loopback TCP IRC server; no live external service",
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo check -p fcp-irc --locked --all-targets
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-irc --locked --test local_non_mock -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-irc --locked --tests -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo clippy -j 2 -p fcp-irc --locked --all-targets -- -D warnings

redaction_scan
emit_summary

echo "IRC_CONNECTOR_VERIFICATION_JSONL=${LOG_JSONL}"
exit "${EXIT_CODE}"
