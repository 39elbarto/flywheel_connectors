...
SCRIPT_PATH="scripts/e2e/paypal_connector_verification.sh"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/paypal_connector/${RUN_ID}}"
LOG_JSONL="${LOG_JSONL:-${OUT_ROOT}/paypal_connector_verification.jsonl}"
COMMAND_LINE="${COMMAND_LINE:-bash ${SCRIPT_PATH}}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_REQUIRE_REMOTE="${RCH_REQUIRE_REMOTE:-1}"
RCH_VISIBILITY="${RCH_VISIBILITY:-verbose}"
REMOTE_TARGET_BASE="${REMOTE_TARGET_BASE:-/tmp/rch-fcp-paypal-${RUN_ID}}"
...
emit_step() {
  ...
  jq -cn \
    --arg record_type "paypal_connector_verification_step" \
    --arg command_line "${COMMAND_LINE}" \
    --arg git_revision "${git_revision}" \
    --arg connector_id "fcp.paypal" \
    ...
    '{
      ...
      redaction: {
        ...
        order_id_logged: false,
        capture_id_logged: false,
        invoice_id_logged: false,
        buyer_email_logged: false,
        refund_note_logged: false
      }
    }' >>"${LOG_JSONL}"
}
...
run_logged() {
  ...
  echo "[paypal-verification] ${name}: ${command_text}"
  if (
    cd "${REPO_ROOT}"
    "$@"
  ) >"${log_path}" 2>&1; then
...
redaction_scan() {
  ...
  local forbidden=(
    "paypal-local-client-id"
    "paypal-local-client-secret"
    "paypal-local-access-token"
    "ORDER-LOCAL-1"
    "CAPTURE-LOCAL-1"
    "REFUND-LOCAL-1"
    "INV-LOCAL-1"
    "buyer-local@example.invalid"
    "refund note that must stay out of evidence"
  )
...
emit_summary() {
  ...
  jq -cn \
    --arg record_type "paypal_connector_verification_summary" \
    --arg command_line "${COMMAND_LINE}" \
    --arg git_revision "${git_revision}" \
    --arg connector_id "fcp.paypal" \
    ...
    '{
      ...
      connector_id: $connector_id,
      outcome: $outcome,
      local_non_mock: "connectors/paypal/tests/local_non_mock.rs",
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo check -p fcp-paypal --locked --all-targets
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-paypal --locked --test local_non_mock -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-paypal --locked --tests -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo clippy -j 2 -p fcp-paypal --locked --all-targets -- -D warnings

redaction_scan
emit_summary

echo "PAYPAL_CONNECTOR_VERIFICATION_JSONL=${LOG_JSONL}"
exit "${EXIT_CODE}"
