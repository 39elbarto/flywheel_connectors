...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/posthog_connector/${RUN_ID}}"
...
run_logged() {
  ...
  echo "[posthog-verification] ${name}: $*"
  previous_pwd="$(pwd)"
  cd "${REPO_ROOT}" || return
  "$@" >"${log_path}" 2>&1
  rc="$?"
...
require_rch_remote_proof() {
  ...
  if command_uses_rch_exec "$@" && ! rch_remote_summary_present "${log_path}"; then
    echo "[posthog-verification] ${name}: rch command did not produce remote proof" >&2
...
if run_logged \
  cargo_check \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo check -p fcp-posthog --all-targets
then
...
if run_logged \
  format_check \
  env -u RCH_FORCE_REMOTE -u RCH_REQUIRE_REMOTE RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo fmt --manifest-path connectors/posthog/Cargo.toml --check
then
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog --test integration health_unconfigured_includes_guidance -- --nocapture
then
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog --test integration doctor_unconfigured_reports_operator_guidance -- --nocapture
then
...
if run_logged \
  self_check_evidence \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog --test integration self_check_ready_with_mock_posthog_api_and_evidence -- --nocapture
then
...
if run_logged \
  ...
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog --test integration self_check_retryable_posthog_failure_reports_degraded -- --nocapture
then
...
if run_logged \
  compliance_evidence \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog --test integration introspection_emits_v3_compliance_evidence -- --nocapture
then
...
if run_logged \
  integration_suite \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog --test integration -- --nocapture
then
...
if run_logged \
  crate_suite \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo test -p fcp-posthog -- --nocapture
then
...
if run_logged \
  clippy \
  env RCH_VISIBILITY=verbose rch exec -- env "RUSTUP_TOOLCHAIN=${REPO_TOOLCHAIN}" cargo clippy -p fcp-posthog --all-targets -- -D warnings
then
...
jq -n \
  --arg run_id "${RUN_ID}" \
  --arg connector "fcp-posthog" \
  --arg repo_root "${REPO_ROOT}" \
  --arg verification_script "scripts/e2e/posthog_connector_verification.sh" \
  --arg artifact_root "${OUT_ROOT}" \
  --arg manifest_check_runner "${manifest_check_runner}" \
  --arg toolchain "${REPO_TOOLCHAIN}" \
  --arg scope_note "read-only PostHog retrofit: HogQL query, insight listing, feature flag listing, and readiness evidence" \
  '{
    run_id: $run_id,
    connector: $connector,
    repo_root: $repo_root,
...

{
  ...
  printf '%s\n' "env RCH_VISIBILITY=verbose rch exec -- env \"RUSTUP_TOOLCHAIN=\${REPO_TOOLCHAIN}\" cargo run -q -p fwc -- manifest fix connectors/posthog/manifest.toml --check --json"
  printf '%s\n' "env -u RCH_FORCE_REMOTE -u RCH_REQUIRE_REMOTE RCH_VISIBILITY=verbose rch exec -- env \"RUSTUP_TOOLCHAIN=\${REPO_TOOLCHAIN}\" cargo fmt --manifest-path connectors/posthog/Cargo.toml --check"
  printf '%s\n' "env RCH_VISIBILITY=verbose rch exec -- env \"RUSTUP_TOOLCHAIN=\${REPO_TOOLCHAIN}\" cargo check -p fcp-posthog --all-targets"
  printf '%s\n' "env RCH_VISIBILITY=verbose rch exec -- env \"RUSTUP_TOOLCHAIN=\${REPO_TOOLCHAIN}\" cargo test -p fcp-posthog --test integration health_unconfigured_includes_guidance -- --nocapture"
  printf '%s\n' "env RCH_VISIBILITY=verbose rch exec -- env \"RUSTUP_TOOLCHAIN=\${REPO_TOOLCHAIN}\" cargo test -p fcp-posthog --test integration doctor_unconfigured_reports_operator_guidance -- --nocapture"
...
echo "PostHog verification artifacts written to ${OUT_ROOT}"
exit "${EXIT_CODE}"
