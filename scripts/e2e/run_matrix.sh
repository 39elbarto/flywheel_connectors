...
run_scenario() {
  ...
  if [[ ! -x "${resolved_script_path}" ]]; then
    status="skipped"
    reason="script_missing"
    record_result "${scenario}" "${resolved_script_path}" "${description}" "${required}" "${status}" 0 "${log_jsonl}" "${reason}"
    return 0
  fi
  ...
  start_ms="$(now_ms)"
  set +e
  OUT_DIR="${out_dir}" LOG_JSONL="${log_jsonl}" "${resolved_script_path}"
  rc=$?
  set -e
  end_ms="$(now_ms)"
  ...
  record_result "${scenario}" "${resolved_script_path}" "${description}" "${required}" "${status}" "${duration_ms}" "${log_jsonl}" "${reason}"
}
