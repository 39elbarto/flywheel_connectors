...
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-searxng-manifest-ops-${RUN_ID}-target}"
LOG_DIR="${LOG_DIR:-$(dirname "${LOG_JSONL}")/searxng-manifest-ops-logs}"
RCH_BIN="${RCH_BIN:-rch}"
...
echo "[searxng-manifest] log_dir=${LOG_DIR}"
...
run_cargo() {
  ...
  echo "[searxng-manifest] ${name}: cargo $*"
  ...
  then
    ...
    echo "[searxng-manifest] ${name}: ${status}; see ${log_path}" >&2
    exit "$([[ "${status}" == "infra_blocked" ]] && echo 2 || echo 1)"
  fi

  if ! rch_remote_summary_present "${log_path}"; then
    echo "[searxng-manifest] ${name}: rch command did not produce remote proof; see ${log_path}" >&2
...
run_cargo manifest_runtime_contract test --locked -p fcp-searxng manifest --lib -- --nocapture

echo "[searxng-manifest] running deterministic no-live-provider HTTP connector suite"
run_cargo loopback_integration test --locked -p fcp-searxng --test integration -- --nocapture

echo "[searxng-manifest] running redaction-safe cross-connector audit JSONL lane"
run_cargo manifest_ops_audit run --locked -p fcp-conformance --bin fcp-manifest-ops-audit -- \
  --repo-root . \
  --allow-findings \
  --log-jsonl "${LOG_JSONL}"
