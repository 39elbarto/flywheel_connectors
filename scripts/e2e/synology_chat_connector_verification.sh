...
SCRIPT_PATH="scripts/e2e/synology_chat_connector_verification.sh"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/synology_chat_connector/${RUN_ID}}"
LOG_JSONL="${LOG_JSONL:-${OUT_ROOT}/synology_chat_connector_verification.jsonl}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_REQUIRE_REMOTE="${RCH_REQUIRE_REMOTE:-1}"
RCH_VISIBILITY="${RCH_VISIBILITY:-verbose}"
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-synology-chat-${RUN_ID}-target}"
...
emit_step() {
  ...
  jq -cn \
    --arg record_type "synology_chat_connector_verification_step" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.synology-chat" \
    ...
    '{
      ...
      redaction: {
        incoming_webhook_url_logged: false,
        outgoing_token_logged: false,
        message_body_logged: false,
        forwarded_payload_logged: false,
        device_lab_env_logged: false
      }
    }' >>"${LOG_JSONL}"
}
...
run_logged() {
  ...
  echo "[synology-chat-verification] ${step}: ${command_text}"
  if (
    cd "${REPO_ROOT}"
    "$@"
  ) >"${log_path}" 2>&1; then
...
redaction_scan() {
  ...
  local forbidden=(
    ...
    "wrong-secret"
    "Ignore previous"
    "SYNOLOGY_CHAT_DEVICE_LAB_ID="
    "SYNOLOGY_CHAT_DEVICE_LAB_EVIDENCE_JSONL="
  )
  ...
  emit_step "redaction_scan" "${status}" "${scan_log}" "grep forbidden Synology Chat fixture material"
  promote_status "${status}"
}
...
emit_summary() {
  ...
  jq -cn \
    --arg record_type "synology_chat_connector_verification_summary" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.synology-chat" \
    --arg git_revision "$(git_revision)" \
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo check -p fcp-synology-chat --locked --all-targets
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-synology-chat --locked --tests -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo clippy -j 2 -p fcp-synology-chat --locked --all-targets -- -D warnings

redaction_scan
emit_summary

echo "SYNOLOGY_CHAT_CONNECTOR_VERIFICATION_JSONL=${LOG_JSONL}"
exit "${EXIT_CODE}"
