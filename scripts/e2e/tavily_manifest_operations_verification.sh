...
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-tavily-manifest-ops-${RUN_ID}-target}"
LOG_DIR="${LOG_DIR:-$(dirname "${LOG_JSONL}")/tavily-manifest-ops-logs}"
RCH_BIN="${RCH_BIN:-rch}"
...
echo "[tavily-manifest] log_dir=${LOG_DIR}"
...
run_cargo() {
  ...
  echo "[tavily-manifest] ${name}: cargo $*"
  ...
  then
    ...
    echo "[tavily-manifest] ${name}: ${status}; see ${log_path}" >&2
    exit "$([[ "${status}" == "infra_blocked" ]] && echo 2 || echo 1)"
  fi

  if ! rch_remote_summary_present "${log_path}"; then
    echo "[tavily-manifest] ${name}: rch command did not produce remote proof; see ${log_path}" >&2
...
run_cargo manifest_runtime_contract test -p fcp-tavily manifest --lib -- --nocapture

echo "[tavily-manifest] running deterministic no-live-provider HTTP connector suite"
run_cargo loopback_connector_suite test -p fcp-tavily --test connector_suite_happy_path -- --nocapture
