...
if command -v "${FWC_MANIFEST_BIN}" >/dev/null 2>&1; then
  ...
  if run_logged \
    manifest_check \
    "${FWC_MANIFEST_BIN}" manifest fix connectors/together/manifest.toml --check --json
  then
...
EOF
  ...
  if run_logged \
    manifest_check \
    "${RCH_BIN}" exec -- env CARGO_TARGET_DIR="${TARGET_DIR}" cargo run -q -p fwc -- manifest fix connectors/together/manifest.toml --check --json
  then
    manifest_status="passed"
    cp "${OUT_ROOT}/logs/manifest_check.log" "${OUT_ROOT}/evidence/manifest_check.json"
  else
...
cargo_check_status="$(run_step cargo_check "${RCH_BIN}" exec -- env CARGO_TARGET_DIR="${TARGET_DIR}" cargo check -p fcp-together --all-targets)"
format_check_status="$(run_step format_check "${RCH_BIN}" exec -- env CARGO_TARGET_DIR="${TARGET_DIR}" cargo fmt --package fcp-together --check)"
loopback_status="$(run_step loopback_jsonl "${RCH_BIN}" exec -- env CARGO_TARGET_DIR="${TARGET_DIR}" TOGETHER_E2E_GIT_REVISION="${git_revision}" cargo test -p fcp-together --test integration together_loopback_e2e_jsonl_matrix -- --nocapture)"
live_status="$(run_step live_jsonl "${RCH_BIN}" exec -- env CARGO_TARGET_DIR="${TARGET_DIR}" TOGETHER_E2E_GIT_REVISION="${git_revision}" cargo test -p fcp-together --test live_verification together_live_smoke_or_structured_skip_jsonl -- --nocapture)"
clippy_status="$(run_step clippy "${RCH_BIN}" exec -- env CARGO_TARGET_DIR="${TARGET_DIR}" cargo clippy -p fcp-together --all-targets --no-deps -- -D warnings)"
...
"\${RCH_BIN}" exec -- env CARGO_TARGET_DIR="\${TARGET_DIR}" cargo check -p fcp-together --all-targets
"\${RCH_BIN}" exec -- env CARGO_TARGET_DIR="\${TARGET_DIR}" cargo fmt --package fcp-together --check
git_revision="\$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"
"\${RCH_BIN}" exec -- env CARGO_TARGET_DIR="\${TARGET_DIR}" TOGETHER_E2E_GIT_REVISION="\${git_revision}" cargo test -p fcp-together --test integration together_loopback_e2e_jsonl_matrix -- --nocapture
"\${RCH_BIN}" exec -- env CARGO_TARGET_DIR="\${TARGET_DIR}" TOGETHER_E2E_GIT_REVISION="\${git_revision}" cargo test -p fcp-together --test live_verification together_live_smoke_or_structured_skip_jsonl -- --nocapture
"\${RCH_BIN}" exec -- env CARGO_TARGET_DIR="\${TARGET_DIR}" cargo clippy -p fcp-together --all-targets --no-deps -- -D warnings
EOF
