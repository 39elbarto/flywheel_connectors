...
SCRIPT_PATH="scripts/e2e/twitch_connector_verification.sh"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/twitch_connector/${RUN_ID}}"
LOG_JSONL="${LOG_JSONL:-${OUT_ROOT}/twitch_connector_verification.jsonl}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_REQUIRE_REMOTE="${RCH_REQUIRE_REMOTE:-1}"
RCH_VISIBILITY="${RCH_VISIBILITY:-verbose}"
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-twitch-${RUN_ID}-target}"
...
redaction_scan() {
  ...
  local forbidden=(
    "local_twitch_acceptance_secret"
    "local-twitch-access-token"
  ...
  emit_step "redaction_scan" "${status}" "${scan_log}" "grep forbidden Twitch fixture material"
  promote_status "${status}"
}
...
emit_summary() {
  ...
  jq -cn \
    --arg record_type "twitch_connector_verification_summary" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.twitch" \
    --arg git_revision "$(git_revision)" \
    ...
    '{
      ...
      git_revision: $git_revision,
      status: $status,
      local_non_mock: "connectors/twitch/tests/local_non_mock.rs",
      conformance_contract: "connectors/twitch/tests/conformance_contract.rs",
      fixture_mode: "loopback HTTP Twitch OAuth/Helix fixtures; no live external service",
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo check -p fcp-twitch --locked --all-targets
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-twitch --locked --test local_non_mock -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-twitch --locked --tests -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo clippy -j 2 -p fcp-twitch --locked --all-targets -- -D warnings

redaction_scan
emit_summary

echo "TWITCH_CONNECTOR_VERIFICATION_JSONL=${LOG_JSONL}"
exit "${EXIT_CODE}"
