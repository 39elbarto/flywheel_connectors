...
SCRIPT_PATH="scripts/e2e/wecom_connector_verification.sh"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/artifacts/e2e/wecom_connector/${RUN_ID}}"
LOG_JSONL="${LOG_JSONL:-${OUT_ROOT}/wecom_connector_verification.jsonl}"
RCH_BIN="${RCH_BIN:-rch}"
RCH_REQUIRE_REMOTE="${RCH_REQUIRE_REMOTE:-1}"
RCH_VISIBILITY="${RCH_VISIBILITY:-verbose}"
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/rch-fcp-wecom-${RUN_ID}-target}"
...
emit_step() {
  ...
  jq -cn \
    --arg record_type "wecom_connector_verification_step" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.wecom" \
    ...
    '{
      ...
      redaction: {
        agent_secret_logged: false,
        access_token_logged: false,
        message_body_logged: false,
        tenant_user_logged: false,
        callback_plaintext_logged: false
      }
    }' >>"${LOG_JSONL}"
}
...
run_logged() {
  ...
  echo "[wecom-verification] ${step}: ${command_text}"
  if (
    cd "${REPO_ROOT}"
    "$@"
  ) >"${log_path}" 2>&1; then
...
redaction_scan() {
  ...
  emit_step "redaction_scan" "${status}" "${scan_log}" "grep forbidden WeCom fixture material"
  promote_status "${status}"
}
...
emit_summary() {
  ...
  jq -cn \
    --arg record_type "wecom_connector_verification_summary" \
    --arg script "${SCRIPT_PATH}" \
    --arg run_id "${RUN_ID}" \
    --arg connector_id "fcp.wecom" \
    --arg git_revision "$(git_revision)" \
    ...
    '{
      ...
      git_revision: $git_revision,
      status: $status,
      local_non_mock: "connectors/wecom/tests/local_non_mock.rs",
      host_forwarded_callback_e2e: "connectors/wecom/tests/host_forwarded_callback_e2e.rs",
      fixture_mode: "loopback HTTP WeCom token/send/user fixtures plus host-forwarded callback e2e",
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo check -p fcp-wecom --locked --all-targets
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-wecom --locked --test local_non_mock -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo test -j 2 -p fcp-wecom --locked --tests -- --nocapture
...
run_logged \
  ...
  "${RCH_BIN}" exec -- env \
    ...
    CARGO_INCREMENTAL=0 \
    CARGO_BUILD_JOBS=2 \
    cargo clippy -j 2 -p fcp-wecom --locked --all-targets -- -D warnings

redaction_scan
emit_summary

echo "WECOM_CONNECTOR_VERIFICATION_JSONL=${LOG_JSONL}"
exit "${EXIT_CODE}"
