...
Usage:
  ...
  scripts/graduation/run_gauntlet.sh [--jsonl <path>] --batch batch1 --status-md <path>
...
write_batch_status_markdown() {
  ...
  {
    printf '# Batch 1 Graduation Status\n\n'
    printf "Generated by \`scripts/graduation/run_gauntlet.sh --batch %s --status-md docs/graduation/batch1_status.md\`.\n\n" "${batch}"
    ...
    printf "Summary: \`%s/%s\` Batch 1 connectors currently pass the graduation gauntlet.\n\n" "${passing}" "${total}"
    if [[ "${passing}" -ne "${total}" && "${proven_status_blocked}" -eq 1 ]]; then
      printf "Pre-promotion status: \`%s/%s\` Batch 1 connectors pass every check before \`readme_status_match\`; none should be called PROVEN until the full proof bundle lands.\n\n" "${total}" "${total}"
    fi
...
run_batch_status() {
  ...
  for connector in "${BATCH1_CONNECTORS[@]}"; do
